 ...
 *
 * This is the CJS backend copy.  The ES module (browser) copy lives at
 * lib/public/modules/model-context-windows.js.  A CI test
 * (test/model-context-windows-parity.test.js) asserts both files produce
 * identical resolution results so they cannot silently drift.
 *
 * When adding a new model: update BOTH this file and
 * lib/public/modules/model-context-windows.js.
 ...
 *
 * Fallback-only (lr-d91ecf): this map is consulted only when the vendor
 * hasn't reported a context window for the active model at runtime
 * (resolveModelContextWindow returns 0 for anything not listed here, and
 * callers degrade gracefully — see resolveContextWindow in app-panels.js,
 * which falls back further to the SDK-reported value before a last-resort
 * default). A vendor-reported window always wins when present; this table
 * exists only to cover models/betas the vendor's runtime response doesn't
 * carry a window for.
 */
...
module.exports = { KNOWN_CONTEXT_WINDOWS: KNOWN_CONTEXT_WINDOWS, resolveModelContextWindow: resolveModelContextWindow };
