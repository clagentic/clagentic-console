...
var { dropSkillsCache } = require("./server-skills");
var { readInstalledBuildSha, checkForNewerInstalledBuild, waitForIdleThenAct } = require("./build-update-check");
...
// lr-dc9a3b: the SHA this process actually loaded lib/build-sha.json with at
// startup, captured once here so a later re-read of the on-disk file (after
// an install replaces it) can never be confused with what THIS process is
// running. Read defensively — a dev checkout that has never run `npm pack`
// (prepack writes this file, and it is gitignored) will not have it, and
// that is a legitimate "no build-identity signal available" state, not an
// error.
var loadedBuildSha = null;
try {
  var _buildShaContents = fs.readFileSync(path.join(__dirname, 'build-sha.json'), 'utf8');
  var _buildShaParsed = JSON.parse(_buildShaContents);
  if (_buildShaParsed && typeof _buildShaParsed.sha === 'string') {
    loadedBuildSha = _buildShaParsed.sha;
  }
} catch (_) {
  // lib/build-sha.json missing/unreadable/malformed — loadedBuildSha stays
  // null, which get_build_status reports honestly rather than guessing.
}
...
var relay = createServer({
  ...
  // lr-2ea2a7: optional daemon.json override for the in-heap session-history
  // tail cap (default 1000 entries, see lib/sessions.js HISTORY_INMEM_MAX_DEFAULT).
  historyInMemMax: typeof config.historyInMemMax === "number" ? config.historyInMemMax : undefined,
  ...
  onRemoveProject: function (slug, userId) {
    ...
    if (isWorktreeSlug(slug)) {
      ...
      if (!parentProject) {
        // Parent project is gone from config (removed/renamed) — we cannot
        // resolve the worktree's on-disk path, so `git worktree remove` never
        // runs and the checkout stays on disk. Surface this as a failure
        // instead of silently reporting success (lr-fc2818): previously this
        // branch fell through to relay.removeProject/unregisterWorktreeSlug
        // and returned {ok:true}, unregistering the slug while leaving the
        // worktree directory and branch behind with no way to retry from the UI.
        console.log("[daemon] Cannot remove worktree, parent project not found:", slug);
        return { ok: false, error: "Parent project not found; worktree left on disk. Remove it manually with 'git worktree remove'." };
      }
  ...
  // lr-20e71c: `tls` stays the accurate name for daemon-internal TLS
  // termination (tlsOptions !== null) — it must NOT be repurposed to mean
  // "reachable over HTTPS". effectiveProtocol is the separate, request-scoped
  // source of truth for that: server.js resolves it per-WS-connection via
  // lib/effective-protocol.js (trustedProxy-gated X-Forwarded-Proto) and
  // passes it in here so the settings badge and any other consumer render
  // the real external protocol, not the internal-termination flag.
  onGetDaemonConfig: function (effectiveProtocol) {
    return {
      port: config.port,
      tls: !!tlsOptions,
      tlsEffective: effectiveProtocol ? effectiveProtocol.protocol === "https" : !!tlsOptions,
      tlsState: effectiveProtocol ? effectiveProtocol.state : (tlsOptions ? "direct" : "disabled"),
      debug: !!config.debug,
      ...
      // lr-9e6569: config.memAvailableMinMB absent means the value below is a
      // fallback default, not something the operator ever saved -- renders
      // identically to a persisted value equal to the default without this
      // flag, which is part of why a silently-discarded edit went unnoticed.
      memAvailableMinMBIsDefault: config.memAvailableMinMB === undefined,
      tokensPerMbHeadroom: config.tokensPerMbHeadroom !== undefined ? config.tokensPerMbHeadroom : DEFAULT_TOKENS_PER_MB_HEADROOM,
      tokensPerMbHeadroomIsDefault: config.tokensPerMbHeadroom === undefined,
  ...
  // lr-e85fec: operator clicked "Restart to apply update" on the
  // build-adoption diagnostic above. Unlike onRestart (immediate —
  // restart_server is already an explicit admin action with no reason to
  // delay further), this waits for in-flight sessions to finish first, the
  // same signal drain.js's memory-pressure path already keys off
  // (getActiveLiveCount()) — an operator asking to "apply the update" is not
  // also asking to drop whatever is running right now. Still bounded: a
  // stuck session cannot hold this open forever. See
  // build-update-check.js's waitForIdleThenAct for the (unit-tested) timing
  // logic itself.
  onRestartForBuildUpdate: function () {
    console.log("[daemon] Restart-for-build-update requested via web UI — waiting for in-flight sessions to finish");
    waitForIdleThenAct({
      getActiveCount: getActiveLiveCount,
      onIdle: function () {
        console.log("[daemon] Restarting to adopt new build (activeCount=" + getActiveLiveCount() + ")");
        spawnAndRestart();
      },
    });
  },
...
var ipc = createIPCServer(socketPath(), function (msg) {
  ...
  switch (msg.cmd) {
    ...
    // lr-dc9a3b: makes the running-vs-installed mismatch a first-class,
    // queryable signal instead of a journalctl-only warning nothing
    // consumes. verify-installed-build.js (the post-merge gate) already
    // compares the ON-DISK build-sha.json against merged HEAD -- that
    // proves the ARTIFACT is right, not that the RUNNING PROCESS loaded
    // it. This reports the SHA this process actually loaded at startup
    // (loadedBuildSha, captured once above -- never re-read, so an install
    // replacing the file on disk after startup cannot change what this
    // reports) plus the existing checkStaleInodes() detector's live
    // result, so a caller can tell "process never picked up the new
    // build" apart from "process picked it up but its deleted-inode
    // handles say the file has since moved again".
    //
    // Read-only, mutates nothing, same OS-level socket gate as every other
    // command on this connection (CONFIG_DIR is chmod 0700).
    case "get_build_status":
      return {
        ok: true,
        loadedBuildSha: loadedBuildSha,
        staleInodes: checkStaleInodes(),
        pid: process.pid,
      };
...
function startListening() {
  relay.server.listen(config.port, listenHost, function () {
    // Startup has no live request, so there is no X-Forwarded-Proto to read —
    // resolveEffectiveProtocol's "proxy" state requires one. Reflect the
    // operator's trustedProxy declaration directly instead of silently
    // printing http:// for a deployment they've told us sits behind a
    // TLS-terminating proxy (lr-20e71c).
    ...
    console.log("[daemon] Listening on " + protocol + "://" + listenHost + ":" + config.port + protocolNote);
    ...
    // onCrossing runs an in-process shedding pass (lr-5e70) FIRST — an
    // immediate attempt to reduce RSS via retrim/evict/cache-drop — then
    // notifies the drain controller (lr-6b30), which independently decides
    // whether to refuse new connections and eventually exit. Shedding and
    // drain are deliberately independent responses to the same signal: a
    // shedding pass that fails or is rate-limited must never block drain
    // from entering, and drain entering must never skip a shedding attempt.
    if (process.platform === "linux") {
      _memoryWatcher = startMemoryHighWatcher(config, {
        onCrossing: function (detail) {
          try {
            var shedProjects = [];
            if (relay && typeof relay.forEachProject === "function") {
              relay.forEachProject(function (ctx) {
                if (ctx && ctx.sm) shedProjects.push({ slug: ctx.slug, sm: ctx.sm, send: ctx.send });
              });
            }
            shedMemory({
              projects: shedProjects,
              caches: { skills: { drop: dropSkillsCache } },
            });
          } catch (e) {
            console.error("[daemon] Memory shedding pass failed:", e && e.message ? e.message : e);
          }
          _drain.onMemoryHighCrossing(detail);
        },
      });

      // Startup sanity check (lr-c10f6d): verify the applied cgroup memory.max
      // ceiling is actually below the container's true RAM. A `%` unit-file
      // directive is resolved by systemd against the wrong MemTotal inside an
      // LXC container (only lxcfs-backed /proc/meminfo sees the true value),
      // so a stale drop-in or an un-applied postinstall can leave the ceiling
      // silently inoperative — this turns that into a visible fault.
      var ceilingCheck = checkAppliedMemoryCeiling();
      if (ceilingCheck.ok && ceilingCheck.inoperative) {
        var ceilingEvent = {
          event: "memory_ceiling_inoperative",
          timestamp: new Date().toISOString(),
          appliedMaxRaw: ceilingCheck.appliedMaxRaw,
          appliedMaxBytes: ceilingCheck.appliedMaxBytes,
          memTotalBytes: ceilingCheck.memTotalBytes,
          reason: ceilingCheck.reason,
        };
        process.stderr.write("[daemon] WARN memory_ceiling_inoperative " + JSON.stringify(ceilingEvent) + "\n");
        setTimeout(function () {
          relay.broadcastAll({
            type: "toast",
            level: "warn",
            message: "Memory ceiling exceeds container RAM — OOM protection inoperative.",
            detail: ceilingCheck.reason,
          });
        }, 3000);
      }
    }

    // --- lr-e85fec: build-adoption diagnostic ---
    // Polls whether the on-disk build (lib/build-sha.json, rewritten by every
    // `npm install -g`) has moved past what THIS process loaded at startup
    // (loadedBuildSha, captured once above and never re-read). lr-22e8's
    // stale-inode/journald warning already surfaces this on the log, but
    // nothing acted on it (see lr-e85fec's task description — the same
    // warning fired 403 times before anyone noticed). This turns the
    // detector into an OFFERED, operator-initiated affordance instead of a
    // passive log line: a diagnostic with an actionable "restart to apply"
    // button the operator can act on when THEY choose, never an automatic
    // restart (see onRestartForBuildUpdate below — restart only ever fires
    // in direct response to that click).
    //
    // Only fires once per detected staleness (edge-triggered on
    // _buildUpdateAnnounced), not on every poll — a 5-minute interval would
    // otherwise re-toast the same condition indefinitely until the operator
    // actually restarts.
    var _buildUpdateAnnounced = false;
    var BUILD_UPDATE_POLL_MS = 5 * 60 * 1000;
    var _buildShaPath = path.join(__dirname, "build-sha.json");
    function _pollBuildUpdate() {
      try {
        var installedSha = readInstalledBuildSha(_buildShaPath);
        var result = checkForNewerInstalledBuild({
          loadedBuildSha: loadedBuildSha,
          installedBuildSha: installedSha,
        });
        if (result.stale && !_buildUpdateAnnounced) {
          _buildUpdateAnnounced = true;
          console.log("[daemon] Newer build installed on disk (" + result.installedBuildSha +
            ") than this process loaded (" + result.loadedBuildSha + ") — offering restart in UI.");
          relay.broadcastAll({
            type: "diagnostic",
            severity: "warning",
            source: "build-update",
            message: "A newer build has been installed but is not yet running. Restart to apply it.",
            actionable: { label: "Restart to apply update", action: "restart_for_build_update" },
          });
        } else if (!result.stale) {
          // Covers the (rare) case of a manual downgrade/rollback reinstalling
          // the same SHA this process already has — re-arm so a subsequent
          // genuine update is announced again rather than staying suppressed
          // forever from one earlier announcement.
          _buildUpdateAnnounced = false;
        }
      } catch (e) {
        // A detection failure must never crash the daemon — this is a
        // best-effort UI nudge, not a load-bearing control path.
        console.error("[daemon] Build-update check failed (non-fatal):", e && e.message ? e.message : e);
      }
    }
    var _buildUpdateTimer = setInterval(_pollBuildUpdate, BUILD_UPDATE_POLL_MS);
    if (_buildUpdateTimer && typeof _buildUpdateTimer.unref === "function") _buildUpdateTimer.unref();
    // Run once shortly after startup too — otherwise an install that lands
    // moments after this process started would sit undetected for a full
    // poll interval before the operator sees anything.
    setTimeout(_pollBuildUpdate, 10000);
