...
var { DEFAULT_MEM_AVAILABLE_MIN_MB } = require("./sdk-bridge");
...
try {
  ...
  if (e.code === "ENOENT") {
    // Guard: detect CLAGENTIC_HOME mis-pointed at the socket subdirectory.
    //
    // Symptom: CLAGENTIC_HOME=~/.clagentic/console → CONFIG_DIR ends in
    // ".clagentic/console" and configFile = CONFIG_DIR/daemon.json (absent),
    // but the real config lives one level up at ../daemon.json. If we
    // bootstrap here we silently write projects:[] and the user loses all
    // their projects with no obvious error. Refuse loudly instead. (lr-dec3)
    var _configBasename = path.basename(path.dirname(configFile));
    var _configGrandparent = path.basename(path.dirname(path.dirname(configFile)));
    var _siblingConfig = path.join(path.dirname(path.dirname(configFile)), path.basename(configFile));
    if (_configBasename === "console" && _configGrandparent === ".clagentic" && fs.existsSync(_siblingConfig)) {
      console.error("[daemon] ERROR: config file not found at " + configFile + " but a config exists at " + _siblingConfig + ".");
      console.error("[daemon] CLAGENTIC_HOME appears to be set to the socket subdirectory (~/.clagentic/console).");
      console.error("[daemon] Do NOT set CLAGENTIC_HOME to ~/.clagentic/console — that directory holds the");
      console.error("[daemon] Unix socket, not the config. Relocating CLAGENTIC_HOME there moves daemon.json");
      console.error("[daemon] out of reach and causes your project list to be wiped on next bootstrap.");
      console.error("[daemon] Fix: unset CLAGENTIC_HOME (or set it back to ~/.clagentic) and restart.");
      process.exit(78); // EX_CONFIG — fatal config error, don't auto-restart
    }
...
var relay = createServer({
  ...
  onCloneProject: function (cloneUrl, wsUser, callback) {
    ...
    // Reject leading-dash values (option injection) and any scheme not in the allow-list.
    // The ext:: and file:: transports are also disabled at the git level via GIT_ALLOW_PROTOCOL.
    var ALLOWED_SCHEMES = ["https://", "http://", "git://", "ssh://", "git@"];
    if (cloneUrl.startsWith("-")) {
      callback({ ok: false, error: "Invalid clone URL: URL must not start with '-'" });
      return;
    }
    var schemeAllowed = false;
    for (var si = 0; si < ALLOWED_SCHEMES.length; si++) {
      if (cloneUrl.startsWith(ALLOWED_SCHEMES[si])) {
        schemeAllowed = true;
        break;
      }
    }
    if (!schemeAllowed) {
      callback({ ok: false, error: "Invalid clone URL: unsupported scheme. Allowed: https, http, git, ssh, git@" });
      return;
    }
    // Enforce protocol restrictions at the git level to prevent ext:: and file:: transports
    // even if future validation is bypassed.
    spawnOpts.env = Object.assign({}, spawnOpts.env || process.env, {
      GIT_ALLOW_PROTOCOL: "https:http:git:ssh",
    });
    // The "--" terminator prevents cloneUrl from being parsed as a git option.
