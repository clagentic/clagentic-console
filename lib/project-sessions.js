...
function attachSessions(ctx) {
  ...
  function handleSessionsMessage(ws, msg) {
    ...
    if (msg.type === "new_session") {
      ...
      // lr-7db0 — optional: cast new session as a named agent (SDK `agent` option).
      // Hidden in the UI when active project is a Mate; server-side we still
      // accept the field but skip-set when the project is a Mate to avoid
      // mixing identity layers in the POC.
      if (msg.agentName && typeof msg.agentName === "string" && !isMate) {
    ...
    if (msg.type === "set_session_agent") {
      ...
      // Hide for Mate sessions per POC scope decision (lr-7db0 answer 2).
