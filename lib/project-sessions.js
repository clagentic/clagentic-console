  ...
  function handleSessionsMessage(ws, msg) {
    ...
    if (msg.type === "new_session") {
      ...
      if (msg.agentName && typeof msg.agentName === "string") {
        ...
        try { agentsFavorites.touchRecent({ name: msg.agentName, kind: msg.agentKind || "user", pluginName: msg.agentPluginName || null }); }
      ...
      // uses the right values even before the user changes anything.
    ...
    if (msg.type === "list_agents") {
      var allAgents;
      try { allAgents = agentsModule.discoverAllAgents(); }
      catch (e) {
        console.error("[project-sessions] list_agents discovery failed:", e.message);
        allAgents = [];
      }
      var favorites = [];
      var recents = [];
    ...
    // Required fields: name, kind. Optional: pluginName (for kind=plugin).
    if (msg.type === "toggle_agent_favorite") {
      if (!msg.name || !msg.kind) return true;
      var entry = { name: msg.name, kind: msg.kind, pluginName: msg.pluginName || null };
      var nowFav;
      ...
      sendTo(ws, {
        type: "agents_list",
        agents: agentsModule.discoverAllAgents(),
        favorites: agentsFavorites.listFavorites(),
        recents: agentsFavorites.listRecents(),
      });
      sendTo(ws, { type: "agent_favorite_toggled", name: msg.name, kind: msg.kind, pluginName: msg.pluginName || null, favorite: nowFav });
      return true;
    }
    ...
    if (msg.type === "set_session_agent") {
      ...
      if (nextAgent) {
        try { agentsFavorites.touchRecent({ name: nextAgent, kind: msg.agentKind || "user", pluginName: msg.agentPluginName || null }); }
        catch (e) { /* best-effort */ }
      }
    ...
    if (msg.type === "set_mate_dm") {
      // set_mate_dm is a no-op now that mates are removed (lr-316f)
      return true;
    }
    ...
    // Update checks disabled — fork, not on chadbyte/clay release track.
    if (msg.type === "set_update_channel") { return true; }
    ...
    if (msg.type === "set_permission_mode" && msg.mode) {
      ...
      if (session) {
        sdk.setPermissionMode(session, msg.mode).then(function () {
          ...
          sendTo(ws, { type: "config_state", model: session.model || sm.currentModel || "", mode: session.permissionMode || "default", effort: sm.currentEffort || "medium", betas: sm.currentBetas || [], thinking: sm.currentThinking || "adaptive", thinkingBudget: sm.currentThinkingBudget || 10000 });
        });
      }
      return true;
    }
    ...
    if (msg.type === "ask_user_response") {
      ...
      if (pending.mode === "mcp") {
        ...
        // included: the SDK spreads this object as-is into the control
        // response, and the Claude CLI reads the snake_case key. Without
        // updated_input, the answers are silently dropped and Claude sees
        // an empty tool result.
        var updatedInput = Object.assign({}, pending.input, { answers: answers });
        pending.resolve({
          behavior: "allow",
