...
function attachSessions(ctx) {
  ...
  function handleSessionsMessage(ws, msg) {
      ...
      try {
        ...
        if (_fsResume.existsSync(metaPath)) {
          ...
          try {
            ...
            if (metaObj && metaObj.type === "meta" && metaObj.allowedTools) persistedAllowedTools = metaObj.allowedTools;
          } catch (e) {}
        }
      } catch (e) {}
    ...
    if (msg.type === "process_stats") {
      ...
      // lr-2d91: Include memory pressure fields so the admin panel can show
      // pressure building before the MemAvailable gate fires.
