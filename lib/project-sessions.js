  ...
  function handleSessionsMessage(ws, msg) {
    ...
    if (msg.type === "process_stats") {
      ...
      if (process.platform === "linux") {
        try {
          var procMem = require("fs").readFileSync("/proc/meminfo", "utf8");
          var memMatch = procMem.match(/^MemAvailable:\s+(\d+)\s+kB/m);
          if (memMatch) memAvailMB = Math.floor(parseInt(memMatch[1], 10) / 1024);
        } catch (e) { /* non-fatal */ }
      }
      sendTo(ws, {
        type: "process_stats",
