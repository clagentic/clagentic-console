  ...
  function handleSessionsMessage(ws, msg) {
    ...
    if (msg.type === "set_session_visibility") {
      if (typeof msg.sessionId === "number" && (msg.visibility === "shared" || msg.visibility === "private")) {
        ...
        if (ws._clayUser) {
          if (!usersModule.canAccessSession(ws._clayUser.id, visTarget, { visibility: "public" })) return true;
        }
    ...
    if (msg.type === "update_now") {
      if (!ws._clayUser || ws._clayUser.role !== "admin") return true;
    ...
    if (msg.type === "process_stats") {
      ...
      // lr-2016fe: activeLiveCount/maxConcurrentSessions/activityDivergence*
      // come from sdk.getMemoryStats(), which is daemon-wide (module-scope
      // counters shared across every project's bridge instance, not scoped
      // to this project) and, per its own doc comment, is the field group a
      // future author is most likely to enrich with per-session identifying
      // data (that already happened once — PR #403 briefly added
      // sessionId here, caught by BOBBIE, and removed rather than gated).
      // This handler has no admin/role gate, unlike its update_now/
      // kill_process siblings in this file, but the base fields above ARE
      // consumed by non-admin surfaces (the `/status` panel and the
      // Settings "Server Status" card, both open to any authenticated
      // user — see app-panels.js/server-settings.js), so a blanket admin
      // gate on the whole handler would break them. Restricting only this
      // diagnostic subgroup to admins keeps the existing non-admin
      // consumers working while closing the channel a future change could
      // otherwise use to leak cross-project session data to any
      // authenticated client.
      if (ws._clayUser && ws._clayUser.role === "admin") {
        var memStats = (sdk && typeof sdk.getMemoryStats === "function") ? sdk.getMemoryStats() : {};
        statsResp.activeLiveCount = memStats.activeLiveCount !== undefined ? memStats.activeLiveCount : null;
        statsResp.maxConcurrentSessions = memStats.maxConcurrentSessions !== undefined ? memStats.maxConcurrentSessions : null;
        statsResp.activityDivergenceCount = memStats.activityDivergenceCount !== undefined ? memStats.activityDivergenceCount : null;
        statsResp.activityDivergenceRecentSamples = memStats.activityDivergenceRecentSamples || [];
      }
      sendTo(ws, statsResp);
      return true;
    }
    ...
    if (msg.type === "schedule_move") {
      if (ws._clayUser) {
        var schMovePerms = usersModule.getEffectivePermissions(ws._clayUser, osUsers);
    ...
    if (msg.type === "get_daemon_config") {
      if (typeof opts.onGetDaemonConfig === "function") {
        // lr-20e71c: ws._clayEffectiveProtocol was resolved once at WS-upgrade
        // time (server.js), where X-Forwarded-Proto is still in scope — this
        // per-message dispatch only has ws/msg, not the original req.
        var daemonConfig = opts.onGetDaemonConfig(ws._clayEffectiveProtocol);
    ...
    if (msg.type === "set_mem_available_threshold") {
      if (typeof opts.onSetMemAvailableThreshold === "function") {
        ...
        sendTo(ws, { type: "set_mem_available_threshold_result", ok: matResult.ok, memAvailableMinMB: matResult.memAvailableMinMB });
    ...
    if (msg.type === "set_tokens_per_mb_headroom") {
      if (typeof opts.onSetTokensPerMbHeadroom === "function") {
        ...
        sendTo(ws, { type: "set_tokens_per_mb_headroom_result", ok: tpmResult.ok, tokensPerMbHeadroom: tpmResult.tokensPerMbHeadroom });
