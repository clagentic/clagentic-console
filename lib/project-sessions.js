...
function attachSessions(ctx) {
  ...
  function handleSessionsMessage(ws, msg) {
    ...
    if (msg.type === "get_daemon_config" || msg.type === "set_pin" || msg.type === "set_keep_awake" ||
        ...
        msg.type === "shutdown_server" || msg.type === "restart_server" || msg.type === "restart_for_build_update") {
    ...
    // lr-e85fec: operator-initiated adoption of an installed-but-not-yet-
    // running build (the "Restart to apply update" action offered by the
    // build-update diagnostic). Same admin gate and same in-process wiring
    // pattern as restart_server above — the only difference is
    // onRestartForBuildUpdate waits for in-flight sessions to finish (bounded)
    // before restarting, since this restart is a maintenance action the
    // operator chose at a moment of their own choosing, not an emergency.
    if (msg.type === "restart_for_build_update") {
      if (typeof opts.onRestartForBuildUpdate === "function") {
        sendTo(ws, { type: "restart_for_build_update_result", ok: true });
        send({ type: "toast", level: "info", message: "Restarting to apply the installed update..." });
        setTimeout(function () {
          opts.onRestartForBuildUpdate();
        }, 500);
      } else {
        sendTo(ws, { type: "restart_for_build_update_result", ok: false, error: "Restart not supported" });
      }
      return true;
    }
