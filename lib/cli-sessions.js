...
// Maximum number of sessions returned by listCliSessions().
// Prevents OOM on installs with large session archives (e.g. 30K+ files).
// Tunable: raise if users need deeper history in the session picker.
var MAX_CLI_SESSIONS = 200;

// Sessions older than this are excluded before any file I/O.
// 90 days keeps the session picker relevant without opening stale files.
var CLI_SESSION_MAX_AGE_MS = 90 * 24 * 60 * 60 * 1000;

// Max simultaneous parseSessionFile() calls inside listCliSessions().
// Caps readline stream count; too high → fd exhaustion; too low → slow.
var PARSE_CONCURRENCY = 20;
 ...
 *
 * Two-stage to prevent OOM on large session archives:
 *   Stage 1 — stat + cap: filter by recency (CLI_SESSION_MAX_AGE_MS), sort by
 *             mtime descending, slice to MAX_CLI_SESSIONS.
 *   Stage 2 — promise pool: parse the capped list with at most
 *             PARSE_CONCURRENCY simultaneous parseSessionFile() calls.
 */
function listCliSessions(cwd) {
  ...
  return new Promise(function (resolve) {
    fs.readdir(projectDir, { withFileTypes: true }, function (err, entries) {
      ...
      var now = Date.now();
      var cutoff = now - CLI_SESSION_MAX_AGE_MS;

      // Stage 1: stat every .jsonl file, drop stale entries, sort and cap.
      var candidates = [];
      for (var i = 0; i < entries.length; i++) {
        if (!entries[i].isFile() || !entries[i].name.endsWith(".jsonl")) continue;
        var filePath = path.join(projectDir, entries[i].name);
        try {
          var st = fs.statSync(filePath);
          if (st.mtimeMs >= cutoff) {
            candidates.push({ filePath: filePath, mtimeMs: st.mtimeMs });
          }
        } catch (e) {
      ...
      if (candidates.length > MAX_CLI_SESSIONS) {
        candidates = candidates.slice(0, MAX_CLI_SESSIONS);
      }

      // Stage 2: promise pool — at most PARSE_CONCURRENCY concurrent parses.
      var results = [];
      var index = 0;      // next candidate to start
      var active = 0;     // currently running parseSessionFile calls
      var total = candidates.length;
      var done = 0;       // completed calls
      ...
      function schedule() {
        while (active < PARSE_CONCURRENCY && index < total) {
          var item = candidates[index++];
          active++;
          parseSessionFile(item.filePath).then(function (session) {
