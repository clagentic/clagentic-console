...
function createProjectContext(opts) {
  ...
  // In single-user mode the terminal manager falls back to the bare send() broadcast,
  // so this closure is only exercised when isMultiUser() is true at startup.
  var tm;
  ...
  tm = createTerminalManager({
    ...
    broadcastTermList: usersModule.isMultiUser() ? broadcastTermListToAll : null,
  });
