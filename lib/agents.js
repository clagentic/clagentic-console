...
// globalAgentsDir: optional override of AGENTS_SOURCE_DIR. On a shared
// multi-user daemon, callers resolve the SESSION OWNER's home
// (os-users.getLinuxUserHome(linuxUser)) and pass
// path.join(thatHome, ".claude", "agents") here so the on-disk agent
// definition is read from the invoking user's own home, not the daemon
// process's REAL_HOME (lr-3ccc78 twin of the skill-discovery fix).
...
function readAgentToolsFromFile(agentName, globalAgentsDir) {
  if (!agentName || typeof agentName !== "string") return null;
  var slug = slugifyAgentName(agentName);
  if (!slug) return null;
  var baseDir = (typeof globalAgentsDir === "string" && globalAgentsDir) ? globalAgentsDir : AGENTS_SOURCE_DIR;
  var filePath = path.join(baseDir, slug + ".md");
  var raw;
  try {
...
// _globalAgentsDir is an optional override of the global agents directory.
// Tests use it to point at a fixture dir; production callers on a shared
// multi-user daemon pass the session owner's own ~/.claude/agents (see
// readAgentToolsFromFile above, lr-3ccc78).
