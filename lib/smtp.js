...
function sendTestEmail(cfg, toEmail) {
  ...
  return t.sendMail({
    from: cfg.from,
    to: toEmail,
    subject: "Clagentic:Console SMTP Test",
    html: '<div style="font-family:system-ui,sans-serif;max-width:400px;margin:0 auto;padding:24px">' +
      '<h2 style="color:#DA7756;margin:0 0 16px">Clagentic:Console</h2>' +
...
function sendOtpEmail(email, code) {
  var subject = "Your Clagentic login code: " + code;
...
function sendInviteEmail(email, inviteUrl, inviterName) {
  var subject = "You've been invited to Clagentic:Console";
  var html = '<div style="font-family:system-ui,sans-serif;max-width:400px;margin:0 auto;padding:24px">' +
    '<h2 style="color:#DA7756;margin:0 0 16px">Clagentic:Console</h2>' +
    '<p style="color:#333;margin:0 0 16px">' + (inviterName || "An admin") + ' has invited you to join Clagentic:Console.</p>' +
