...
export function resetClientState() {
  ...
  // activity-state.js's sessionActivity()), and resetClientState() runs
  // from app-messages.js's session_switched handler AFTER that handler has
  // already reconciled 'processing' from the incoming session's
  // authoritative isProcessing — unconditionally zeroing it here stomped
  // that just-set correct value back to false on every single session
  // switch (a pre-existing latent defect this removal also fixes, not only
  // a latch-deletion). setActivityEl(null) above still nulls the DOM ref
  // so the footer's own reactive subscriber (initActivityFooter) cannot
  // find a stale element if it fires after this reset.
  setTurnCounter(0);
