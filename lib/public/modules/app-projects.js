...
export function resetClientState() {
  ...
  // resetClientState() BEFORE applying its own 'processing' projection
  // (reordered, PEACHES finding 1 fix) precisely so setActivityEl(null)
  // below nulls the footer widget's DOM ref before that projection's
  // store.set() can synchronously fire initActivityFooter's subscriber —
  // otherwise the subscriber could raise the widget against the outgoing
  // session's transcript DOM right before this function's own
  // innerHTML="" wipes it, stranding a stale ref that a later unchanged
  // value would never repair (store.set()'s changed-flag bounding,
  // lr-9bcd7b). See app-messages.js's session_switched handler for the
