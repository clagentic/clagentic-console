...
// renderUserStrip is now reactive via store subscriber in sidebar-users.js
import { renderIconStrip } from './sidebar-projects.js';
import { updateCrossProjectBlink, stopUrgentBlink } from './app-favicon.js';
...
import { setTurnCounter, setPrependAnchor, setActivityEl, setIsUserScrolledUp, hideSuggestionChips, resetCurrentFullText } from './app-rendering.js';
import { resetToolState, enableMainInput, resetTurnMetaCost } from './tools.js';
import { clearPendingImages } from './input.js';
import { clearAllMentionActive } from './sidebar-users.js';
...
// --- Module-owned state ---
var _onProjectListUpdated = null;
export function onProjectListUpdated(fn) { _onProjectListUpdated = fn; }

var _onResetClientState = null;
export function onResetClientState(fn) { _onResetClientState = fn; }
...
export function updateProjectList(msg) {
  ...
  if (msg.folderMeta) cachedFolderMeta = msg.folderMeta;
  ...
  if (projectsChanged || slugChanged) {
    ...
    if (count === 1 && projectHint) {
      try {
        if (!localStorage.getItem("clagentic-project-hint-dismissed") && !localStorage.getItem("clay-project-hint-dismissed")) {
  ...
  if (msg.allUsers) {
    ...
    var st2 = store.snap();
    if (document.body.classList.contains("wide-view")) {
      ...
      if (refreshedMyUser) {
        ...
        try { localStorage.setItem("clagentic_my_user", JSON.stringify({ displayName: refreshedMyUser.displayName, username: refreshedMyUser.username, avatarStyle: refreshedMyUser.avatarStyle, avatarSeed: refreshedMyUser.avatarSeed, avatarCustom: refreshedMyUser.avatarCustom })); } catch(e) {}
      }
    }
...
export function renderProjectList() {
  // lr-66c118: activity-state.js is the sole reader of the processing flag
  // (CI-enforced) — Object.assign carries it through untouched rather than
  // this mapper naming/reading the property itself.
  ...
  var st = store.snap();
  var iconStripActiveSlug = st.currentSlug;
  renderIconStrip(iconStripProjects, iconStripActiveSlug, cachedFolderMeta);
  // Update mobile Projects tab badge (called via registered callback to avoid circular import)
  if (typeof _onProjectListUpdated === "function") _onProjectListUpdated(iconStripProjects);
    ...
    for (var pi = 0; pi < cachedProjects.length; pi++) {
      if (cachedProjects[pi].slug === st.currentSlug) {
        ...
        if (tbIcon) {
          ...
          if (pIcon) {
            renderProjectIcon(pIcon, tbIcon, parseEmojis);
            tbIcon.classList.add("has-icon");
            try { localStorage.setItem("clagentic-project-icon-" + (st.currentSlug || "default"), pIcon); } catch (e) {}
          } else {
            tbIcon.textContent = "";
            tbIcon.classList.remove("has-icon");
            try { localStorage.removeItem("clagentic-project-icon-" + (st.currentSlug || "default")); localStorage.removeItem("clay-project-icon-" + (st.currentSlug || "default")); } catch (e) {}
          }
        }
        // Input bar project indicator
        var inputIndicator = document.getElementById("input-project-indicator");
        if (inputIndicator) {
          var indName = cachedProjects[pi].title || cachedProjects[pi].project || "";
          var indIcon = cachedProjects[pi].icon || null;
          if (indIcon) {
            renderProjectIcon(indIcon, inputIndicator, parseEmojis);
            inputIndicator.removeAttribute("data-abbrev");
          } else {
            // Abbreviation fallback — first two initials
            var words = indName.replace(/[^a-zA-Z0-9\s]/g, "").trim().split(/\s+/);
            var abbrev = words.length >= 2
              ? (words[0][0] + words[1][0]).toUpperCase()
              : indName.substring(0, 2).toUpperCase();
            inputIndicator.textContent = abbrev;
            inputIndicator.setAttribute("data-abbrev", "1");
          }
          inputIndicator.setAttribute("data-tip", indName);
        }
        break;
      }
    }
  }
  // Re-apply current socket status to the active icon's dot. st.processing
  // is always tone 'self' — it can only ever be about the viewing user's
  // own connection. lr-66c118: routed through the same derivation as every
  // other dot rather than writing '.processing' as an independent 3rd
  // state source. lr-5edd64: st.processing is now a projection of the
  // focused session's server-authoritative isProcessing (app-messages.js's
  // session_list/session_switched handlers), not a client-local latch.
  var dot = getStatusDot();
  if (dot) {
    if (st.connected) {
      dot.classList.add("connected");
      var localActivityCls = indicatorClass(sessionActivity({ isProcessing: st.processing }));
      if (localActivityCls) dot.classList.add(localActivityCls);
    }
  }
...
export function resetClientState() {
  ...
  setActivityEl(null);
  // lr-5edd64: no longer force-zeroes 'processing' here. It is a
  // PROJECTION now (session_list/session_switched handlers derive it via
  // activity-state.js's sessionActivity()) — unconditionally zeroing it
  // here would stomp a correctly-projected value. session_switched calls
  // resetClientState() BEFORE applying its own 'processing' projection so
  // setActivityEl(null) above nulls the footer widget's DOM ref before the
  // incoming session's transcript DOM exists, and before this function's
  // own innerHTML="" wipes the outgoing one.
  //
  // The DOM ref this line nulls is repaired by an explicit, UNCONDITIONAL
  // call (app-favicon.js's reconcileActivityFooter()) that each projection
  // caller makes right after applying its own 'processing' projection — not
  // by relying on initActivityFooter's ['processing'] value-change
  // subscriber to fire (lr-5edd64, 9th recurrence: a session switch where
  // the value is UNCHANGED across the destroy, e.g. true->true, would never
  // fire that subscriber, leaving this ref permanently un-repaired — see
  // app-messages.js's session_switched handler and app-favicon.js's
  // reconcileActivityFooter() doc for the full mechanism).
  setTurnCounter(0);
  ...
  setRewindMode(false);
  // lr-66c118 / lr-6e20f7: the setActivity clear call is removed here —
  // setActivityEl(null) above already nulls the activity-widget ref, and
  // getMessagesEl().innerHTML = "" already wiped the DOM element itself, so
  // an explicit setActivity(null) call here would be redundant. The widget's
  // OWN reactive clear (app-favicon.js's initActivityFooter, subscribed to
  // store.processing) additionally still needs the DOM ref nulled out here —
  // setActivityEl(null) above is exactly what makes that safe, since a
  // reactive clear firing after this reset would otherwise find a stale ref
  // pointing at an element already removed from the DOM by innerHTML="".
  setStatus("connected");
  // Always re-enable input on session switch. A loop running in another session
  // has its own dedicated session and must not lock unrelated sessions.
  // updateLoopInputVisibility (called from switched_session) will re-hide the
  // input area only when the target session is itself a loop coder/judge session.
  enableMainInput();
  resetUsage();
  resetTurnMetaCost();
  resetContext();
  ...
  stopUrgentBlink();
  // Clear vendor state so the incoming model_info for the new project is never
  // blocked by a lock that was set during the previous project's session.
  store.set({ currentVendor: null, vendorSelectionLocked: false, codexApproval: null, codexSandbox: null, codexWebSearch: null });
  // NOTE: session list reset is NOT done here. resetClientState() is also called
  // from session_switched (within-project session switching) where the session
  // list must be preserved. Session list clearing is done only in switchProject()
  // and the popstate handler via _onResetClientState, which are the only paths
  // that actually change projects.
}
...
export function switchProject(slug) {
  ...
  // Clear the session list immediately so the sidebar shows blank (not the
  // previous project's sessions) while waiting for the new project's
  // session_list. Must happen here, NOT in resetClientState(), because
  // resetClientState() is also called from session_switched (within-project
  // session switch) where the session list must stay intact.
  if (typeof _onResetClientState === "function") _onResetClientState();
  resetClientState();
  connect();
}
...
function renderRemovedProjectsList() {
  ...
  for (var ri = 0; ri < cachedRemovedProjects.length; ri++) {
    ...
    if (rp.icon) {
      // renderProjectIcon() routes custom (:slug:) icons to <img>; the prior
      // textContent assignment showed ':slug:' as literal text (lr-a6da).
      renderProjectIcon(rp.icon, iconEl, parseEmojis);
    } else {
      iconEl.textContent = "\uD83D\uDCC1";
    }
