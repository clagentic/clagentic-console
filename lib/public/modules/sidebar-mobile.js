...
import { switchProject, getCachedProjects, openAddProjectModal } from './app-projects.js';
import { openDm } from './app-dm.js';
import { showHomeHub, hideHomeHub, isHomeHubVisible } from './app-home-hub.js';
import { openTerminal } from './terminal.js';
import { loadRootDirectory } from './filebrowser.js';
import { renderSheetTeam as renderTeamView } from './team-panel.js';
...
// Duration must match the .closing animation in mobile-nav.css
var SHEET_CLOSE_MS = 230;
...
// Close the sheet and run fn after the close animation completes.
// Use this for session/project switches so the history replay flood
// doesn't begin while the sheet slide-down is still animating.
function closeMobileSheetThen(fn) {
  var sheet = document.getElementById("mobile-sheet");
...
function renderSheetProjects(listEl) {
  ...
  function renderProjectRow(p, indent) {
    ...
    var rowActivityCls = indicatorClass(sessionActivity(p, null, { ignoreUnread: true }));
    if (rowActivityCls) {
      var dot = document.createElement("span");
      dot.className = "mobile-project-processing " + rowActivityCls;
      el.appendChild(dot);
    }
  ...
  function renderFolderSection(fn, folderProjects) {
    ...
    var folderRowActivityCls = indicatorClass(rollupActivity(folderProjects, null, { ignoreUnread: true }));
    ...
    if (meta.icon) {
      ...
      renderProjectIcon(meta.icon, overlay, parseEmojis);
    ...
    if (folderRowActivityCls) {
      var fdot = document.createElement("span");
      fdot.className = "mobile-project-processing " + folderRowActivityCls;
...
function renderSheetSessions(listEl) {
  ...
  if (currentProject) {
    ...
    if (currentProject.icon) {
      renderProjectIcon(currentProject.icon, projIcon, parseEmojis);
    } else {
...
function createMobileSessionItem(s) {
  ...
  var msiActivityState = sessionActivity(s, store.get('myUserId'));
  var msiActivityCls = indicatorClass(msiActivityState);
  if (msiActivityCls) {
    var dot = document.createElement("span");
    dot.className = "mobile-session-processing " + msiActivityCls;
    dot.title = msiActivityState.label;
    el.appendChild(dot);
  }
...
function createMobileLoopChild(s) {
  ...
  var mlcActivityCls = indicatorClass(sessionActivity(s, store.get('myUserId')));
  if (mlcActivityCls) {
    var dot = document.createElement("span");
    dot.className = "mobile-session-processing " + mlcActivityCls;
    el.appendChild(dot);
  }
...
function createMobileLoopRun(parentGk, startedAtKey, sessions, isRalph) {
  ...
  // lr-66c118: replaces the open-coded anyProcessing OR-loop above.
  var runRowActivityCls = indicatorClass(rollupActivity(sessions, store.get('myUserId')));
  ...
  var labelHtml = "";
  if (runRowActivityCls) {
    labelHtml += '<span class="mobile-session-processing ' + runRowActivityCls + '"></span> ';
  }
...
function createMobileLoopGroup(loopId, children, groupKey) {
  ...
  // lr-66c118: replaces the open-coded anyProcessing OR-loop above.
  var groupRowActivityCls = indicatorClass(rollupActivity(children, store.get('myUserId')));
  ...
  if (groupRowActivityCls) {
    var dot = document.createElement("span");
    dot.className = "mobile-session-processing " + groupRowActivityCls;
...
function renderMobileSessionsInto(container) {
  ...
  // Agent Chat is Claude Code-only — the Codex adapter has no agent identity
  // injection API (no equivalent of the Claude Agent SDK `agent` option).
  var isCodexMode = (store.get('currentVendor') || 'claude') === 'codex';
  ...
  // Row 2: Agent chat (full picker) — hidden in dmMode and Codex mode
  if (!isDmMode && !isCodexMode) {
  ...
  for (var i = 0; i < chips.length; i++) {
    ...
    if (statusDot) {
      ...
      for (var pi = 0; pi < allProjects.length; pi++) {
        if (allProjects[pi].slug === lookupSlug) {
          chipProject = allProjects[pi];
          break;
        }
      }
      // lr-66c118: chip dot is project-level — isProcessing-only (no alert
      // tone here, matching every other project-level dot in this module).
      statusDot.classList.remove("processing", "alert", "other");
      var chipActivityCls = indicatorClass(sessionActivity(chipProject, null, { ignoreUnread: true }));
      if (chipActivityCls) statusDot.classList.add(chipActivityCls);
    }
  }
...
function renderSheetTeam(listEl) {
  renderTeamView(listEl);
}
...
export function initSidebarMobile() {
    ...
    (function (tab) {
      tab.addEventListener("click", function () {
        ...
        if (name === "projects") {
          ...
          openMobileSheet("team");
          setMobileTabActive("team");
        } else if (name === "more") {
  ...
  // Re-render the mobile session action grid when vendor changes so the Agent
  // Chat button appears/disappears correctly when the user switches vendors
  // while the mobile sheet is already open.
  store.subscribe(['currentVendor'], function (state, prev) {
    if (state.currentVendor !== prev.currentVendor && mobileChatSheetOpen) {
      refreshMobileChatSheet();
    }
  });
}
