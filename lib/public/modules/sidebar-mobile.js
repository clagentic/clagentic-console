...
function renderSheetProjects(listEl) {
  ...
  var currentSlug = getCachedCurrentSlug();

  // Group projects: collect folders and ungrouped
  var folders = {}; // folderName -> { projects: [], unread: 0, isProcessing: false }
  var folderOrder = [];
  var ungrouped = [];
  ...
  for (var i = 0; i < projects.length; i++) {
    var p = projects[i];
    if (p.folderName) {
      if (!folders[p.folderName]) {
        folders[p.folderName] = { projects: [], unread: 0, isProcessing: false };
        folderOrder.push(p.folderName);
      }
      folders[p.folderName].projects.push(p);
      folders[p.folderName].unread += (p.unread || 0);
      if (p.isProcessing) folders[p.folderName].isProcessing = true;
    } else {
      ungrouped.push(p);
    }
  }
  ...
  function renderFolderSection(fn) {
    var fd = folders[fn];
    var meta = folderMeta[fn] || {};
    var collapsed = isFolderCollapsed(fn);
    var isActiveFolder = !collapsed && fd.projects.some(function (fp) { return fp.slug === currentSlug; });
    ...
    if (fd.unread > 0) {
      ...
      fbadge.textContent = fd.unread > 99 ? "99+" : String(fd.unread);
    ...
    for (var fi = 0; fi < fd.projects.length; fi++) {
      childrenEl.appendChild(renderProjectRow(fd.projects[fi], true));
    }
  ...
  // Render folders first (in order), then ungrouped
  for (var fi = 0; fi < folderOrder.length; fi++) {
    renderFolderSection(folderOrder[fi]);
  }
  for (var ui = 0; ui < ungrouped.length; ui++) {
    listEl.appendChild(renderProjectRow(ungrouped[ui], false));
  }
}
...
function renderSheetSessions(listEl) {
  ...
  if (currentProject) {
    ...
    chatHeader.appendChild(projIcon);
    ...
    chatHeader.appendChild(projName);
  }
...
function renderMobileSessionsInto(container) {
  ...
  // Slot 2: Preferred agent (matches desktop slot 2)
  if (!store.get('dmMode')) {
    ...
    if (preferredAgentMobile) {
      ...
      var setPreferredBtn = document.createElement("button");
      setPreferredBtn.className = "mobile-session-new mobile-session-new--dim";
      setPreferredBtn.innerHTML = '<i data-lucide="bot" style="width:16px;height:16px"></i> Set preferred agent';
      setPreferredBtn.addEventListener("click", function () {
        closeMobileSheet();
        setTimeout(function () {
          if (currentSlugMobile) openAgentPickerForPreferred(currentSlugMobile);
        }, 250);
      });
      actionGrid.appendChild(setPreferredBtn);
    }
  }

  // Slot 3: Import session
  ...
  // Slot 4: Start agent chat
  if (!store.get('dmMode')) {
    var agentBtn = document.createElement("button");
    agentBtn.className = "mobile-session-new";
    agentBtn.innerHTML = '<i data-lucide="message-circle-plus" style="width:16px;height:16px"></i> Start agent chat';
...
function renderSheetMore(listEl) {
  ...
  addItem("calendar-clock", "Scheduled Tasks", function () {
    closeMobileSheet();
