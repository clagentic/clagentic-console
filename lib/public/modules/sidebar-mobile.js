...
function renderMobileSessionsInto(container) {
  ...
  // lr-7db0 — Start agent chat. Mirrors desktop sidebar-sessions
  // renderSessionTopActions; hidden in Mate DM views per POC scope decision.
  // Sheet closes first so the picker overlay isn't trapped behind it on
  // small viewports; opening is deferred via setTimeout to match the
  // import-session pattern above.
  if (!store.get('dmMode')) {
    // Preferred agent shortcut (mirrors desktop top-actions slot 2)
