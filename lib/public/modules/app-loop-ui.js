...
// lr-6fe5fc: single derivation of "should THIS session's composer present
// queue-mode UI", used by both call sites below. Takes the three server-
// pushed inputs directly (never a per-session `loop` object) — see the
// invariant note on recheckLoopQueueModeFromState() for why the per-session
// `loop.active`/`loop.role` fields the pre-lr-6fe5fc code additionally
// checked here are provably redundant with this three-input check and are
// no longer consulted: project-loop.js only ever sets loopCurrentSessionId
// to a session whose own loop.active is true and role is coder/judge, and
// finishLoop() atomically nulls loopCurrentSessionId in the same pass that
// clears every affected session's loop.active. So loopCurrentSessionId ===
// activeSessionId already implies everything the per-session object could
// have told us; carrying it as a second, separately-checked condition is
// exactly the shape of the bug this task is fixing (two things that are
// supposed to always agree, checked independently, able to drift).
//
// This is a client-side UX gate only — which WS message type the composer
// sends (`loop_message` vs `message`). It is not a permission check: the
// server's handleLoopMessage() gate independently and authoritatively
// decides whether to accept a queued `loop_message` off loopState.active,
// which is a coarser, correctly-project-scoped question (see that
// function's comment for why the two predicates are intentionally not the
// same one).
export function isLoopQueueSession(loopActive, loopCurrentSessionId, activeSessionId) {
  return !!(loopActive && loopCurrentSessionId != null && loopCurrentSessionId === activeSessionId);
}
...
//
// lr-fd38ac / lr-6fe5fc: the per-session `loop.active` marker (persisted on
// the session itself, see sessions.js:109/711) is not a reliable input to
// this gate — see isLoopQueueSession()'s comment for why the three
// server-pushed store values are sufficient on their own. The `loop`
// parameter is accepted for backward compatibility with callers that pass
// the per-session object from a WS message (session_switched et al.) but is
// intentionally unused in the derivation itself.
export function updateLoopInputVisibility(loop) {
  var inputArea = document.getElementById("input-area");
  if (!inputArea) return;
  inputArea.style.display = "";
  var s = store.snap();
  var queueMode = isLoopQueueSession(s.loopActive, s.loopCurrentSessionId, s.activeSessionId);
...
// lr-fd38ac follow-up / lr-6fe5fc: re-derive queue mode from store state
// alone, for the reactive subscriber in initLoopUi() — it fires on
// loopActive/loopCurrentSessionId/activeSessionId changes, none of which
// carry a per-session `loop` object the way session_switched's msg.loop
// does. Uses the same isLoopQueueSession() predicate as
// updateLoopInputVisibility() — see that function's comment for the
// server-side invariant that makes the three store values sufficient on
// their own, without a per-session `loop` object.
