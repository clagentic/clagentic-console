...
export function autoResize() {
  ...
  var lines = 1;
  for (var i = 0; i < val.length; i++) { if (val[i] === "\n") lines++; }

  // Check for soft-wrap growth: if scrollHeight > offsetHeight the content
  // already overflows visually even though newline count hasn't changed.
  var softWrapGrew = !autoResizeAtMax &&
                     ctx.inputEl.scrollHeight > ctx.inputEl.offsetHeight + 2;

  var skip = !softWrapGrew && (lines === autoResizeLastLines ||
             (autoResizeAtMax && lines > autoResizeLastLines));
  if (!skip) {
