...
import { getScheduleDelayMs, setScheduleDelayMs as setSessionScheduleDelayMs } from './rate-limit-state.js';
...
// lr-0827ba: schedule-delay arming is per-session state, owned by
// rate-limit-state.js and keyed by the currently active session (not a bare
// module-scoped variable — that was the root cause of arming in one project
// silently overwriting another project's armed state, since the SPA never
// reloads the page on project switch).
...
// Returns the armed schedule delay for the currently active session (0 = no
// schedule armed). Pass an explicit sessionId to check a session other than
// the active one (e.g. redrawing on switch-in).
export function getScheduleDelay(sessionId) {
  return getScheduleDelayMs(sessionId === undefined ? currentSessionId() : sessionId);
}

// Arms (or disarms, with ms=0) the schedule delay for a specific session.
// Only touches the visible schedule button when sessionId is the currently
// focused session — arming a background session (e.g. a rate-limit hit in a
// project the user isn't looking at) must not repaint the UI for whatever
// session IS focused (lr-0827ba).
export function setScheduleDelayForSession(sessionId, ms) {
  setSessionScheduleDelayMs(sessionId, ms);
  if (sessionId === currentSessionId() && _setScheduleDelayFn) _setScheduleDelayFn(ms);
}

// Arms (or disarms, with ms=0) the schedule delay for the currently active
// session and updates the visible button state.
export function setScheduleDelayMs(ms) {
  setScheduleDelayForSession(currentSessionId(), ms);
}

// Redraws the schedule button to reflect a session's already-armed delay
// without re-triggering any arm/disarm side effects — used when switching
// back into a session that has a background-armed schedule (lr-0827ba).
export function restoreScheduleDelayUi() {
  if (_setScheduleDelayFn) _setScheduleDelayFn(getScheduleDelay());
}

export function clearScheduleDelayForSession(sessionId) {
  setSessionScheduleDelayMs(sessionId, 0);
...
export function clearScheduleDelay() {
  clearScheduleDelayForSession(currentSessionId());
}
...
export var builtinCommands = [
  ...
  // Note: skills/workflows shipped with the CLI (code-review, workflow, loop, etc.)
  // are intentionally NOT listed here — they are discovered dynamically via
  // sdk-skill-discovery and sdk-workflow-discovery and will appear with real
  // descriptions through the slash_commands WS message (lr-7d8d, lr-7bd3, lr-1c7f).
  // Adding them here would filter them OUT of the dynamic discovery path.
];
...
export function sendMessage() {
  ...
  // lr-e31b: Viewing an active (non-crafting) loop iteration session — the
  // SDK input stream for that iteration is already closed (single-turn), so
  // the message cannot be injected live. Queue it server-side for delivery
  // at the next iteration boundary instead of sending a normal "message".
  // Images/pastes are not supported on this path (loop_message is text-only
  // server-side) — fold paste text into the message body, drop images.
  if (store.get("loopQueueMode")) {
    var queueText = text;
    if (pastes.length > 0) {
      queueText = queueText ? queueText + "\n\n" + pastes.join("\n\n") : pastes.join("\n\n");
    }
    if (!queueText) return;
    ctx.ws.send(JSON.stringify({ type: "loop_message", text: queueText }));
    ctx.addSystemMessage("Queued for the next loop iteration: " + queueText, false);
    ctx.inputEl.value = "";
    sendInputSync();
    clearPendingImages();
    autoResize();
  ...
  var _armedDelayMs = getScheduleDelay();
  if (_armedDelayMs > 0) {
    var resetsAt = Date.now() + _armedDelayMs;
...
var INPUT_MIN_HEIGHT = 24; // matches #input's CSS min-height (input.css)
...
var autoResizeLastGrowth = -1; // last value written to --input-growth, avoids redundant writes

// Publish how far the textarea has grown past its single-line height as a
// CSS var so #new-msg-btn (messages.css) can anchor above the composer
// instead of colliding with it when the composer expands (lr-4a3d). Runs
// through the same rAF-batched autoResize() path already used for typing,
// so this adds no extra observers/listeners.
function publishComposerGrowth(newH) {
  var growth = Math.max(0, newH - INPUT_MIN_HEIGHT);
  if (growth === autoResizeLastGrowth) return;
  autoResizeLastGrowth = growth;
  document.documentElement.style.setProperty("--input-growth", growth + "px");
}
...
export function resetAutoResize() {
  ...
  autoResizeLastGrowth = -1;
  publishComposerGrowth(INPUT_MIN_HEIGHT);
}
...
export function initInput(_ctx) {
  ...
  function setScheduleDelay(ms) {
    setSessionScheduleDelayMs(currentSessionId(), ms);
    if (!scheduleBtn) return;
    collapseScheduleBtn();
    if (ms > 0) {
  ...
  if (scheduleBtn) {
    scheduleBtn.addEventListener("click", function (e) {
      e.stopPropagation();
      if (getScheduleDelayMs(currentSessionId()) > 0) {
        setScheduleDelay(0);
        return;
      }
      if (scheduleInlineInput) {
