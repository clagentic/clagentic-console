...
function renderAgentFavorites() {
  ...
  // Un-hide the container if we added agent icons (renderUserStrip may have hidden it
  // when there are no other human users, but agent favorites should still show).
  if (favorites.length > 0) {
    container.classList.remove("hidden");
  } else if (!container.querySelector(".icon-strip-user")) {
