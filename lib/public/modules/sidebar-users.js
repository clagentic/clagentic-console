...
// Replacement for sidebar-mates.js after mates removal (lr-316f).
// Contains only human-user functionality; all mate-specific rendering is gone.
...
export function setCurrentDmUser(userId) {
  ...
  var items = document.querySelectorAll("#icon-strip-users .icon-strip-user");
...
export function updateDmBadge(userId, count) {
  var badge = document.querySelector("#icon-strip-users .icon-strip-user-badge[data-user-id='" + userId + "']");
  if (!badge) return;
  if (count && count > 0) {
