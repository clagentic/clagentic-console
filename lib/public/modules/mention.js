...
var mentionMenuBound = false;
var mentionCandidatesCache = null;
var mentionCandidatesCacheKey = "";

// --- Init ---
export function initMention(_ctx) {
  ctx = _ctx;
  if (!mentionMenuBound) {
    mentionMenuBound = true;
    document.addEventListener("click", function (e) {
      var menuEl = document.getElementById("mention-menu");
      if (!menuEl || !menuEl.contains(e.target)) return;

      var closeBtn = e.target.closest(".mention-close-btn");
      if (closeBtn) {
        e.preventDefault();
        e.stopPropagation();
        hideMentionMenu();
      ...
      e.preventDefault();
      e.stopPropagation();
      selectMentionItem(parseInt(item.dataset.idx, 10));
    });
  }
}
...
function buildMentionCandidates() {
  ...
  if (mentionCandidatesCache && mentionCandidatesCacheKey === cacheKey) {
    return mentionCandidatesCache;
  }
  ...
  // Users: every other signed-in user (always available).
  ...
  mentionCandidatesCache = candidates;
  mentionCandidatesCacheKey = cacheKey;
  return candidates;
}
...
export function showMentionMenu(query) {
  ...
  menuEl.innerHTML = '<div class="mention-hint">Mention a teammate or side channel &mdash; the coding agent stays out of this exchange until your next message<button class="mention-close-btn" aria-label="Close">&times;</button></div>' +
...
export function hideMentionMenu() {
  if (!mentionActive && mentionFiltered.length === 0) return; // already hidden
...
function getMyAvatarSrc() {
  ...
  var myUser = null;
  try { myUser = JSON.parse(localStorage.getItem("clagentic_my_user") || localStorage.getItem("clay_my_user") || "null"); } catch (e) {}
