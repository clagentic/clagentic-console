...
import { isSonnetModel, isOpusModel, isHaikuModel } from './settings-defaults.js';
  ...
  if (isRejected && msg.resetsAt) {
    ...
    if (delayUntilReset > 0 && currentVendor === "claude") {
      ...
      if (msg.rateLimitType === "seven_day_sonnet") {
        // Haiku usage is metered against the Sonnet 7-day bucket on Pro/Max plans,
        // so a seven_day_sonnet rejection also blocks Haiku.
        shouldSchedule = isSonnetModel(currentModel) || isHaikuModel(currentModel);
