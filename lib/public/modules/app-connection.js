...
export function connect() {
  ...
  fetchWsTicket().then(function (result) {
    if (result.unauthenticated) {
      // Ticket fetch itself came back 401: the session is gone (expired /
      // revoked), not just cookie-less-on-upgrade. Route to sign-in instead
      // of opening a socket that would only 401 on the upgrade too — ties
      // into the Surface-1 unauth-vs-unreachable split below.
...
export function scheduleReconnect() {
  ...
  reconnectTimer = setTimeout(function () {
    ...
    fetch("/info").then(function (res) {
      if (res.status === 401) { handleUnauthenticated(); return null; }
