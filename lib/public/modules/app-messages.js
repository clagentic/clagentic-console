...
import { sessionActivity } from './activity-state.js';
...
registerHandlers({
  ...
  session_list: function (msg) {
    ...
    var _sessionListSessions = msg.sessions || [];
    var _activeId = store.get('activeSessionId');
    if (_activeId != null) {
      for (var _sli = 0; _sli < _sessionListSessions.length; _sli++) {
        if (_sessionListSessions[_sli].id === _activeId) {
          store.set({ processing: sessionActivity(_sessionListSessions[_sli]).active });
          break;
        }
      }
    }
