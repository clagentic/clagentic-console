...
import { startUrgentBlink, stopUrgentBlink, blinkSessionDot, updateCrossProjectBlink, reconcileActivityFooter } from './app-favicon.js';
...
import { deriveSessionListProcessing } from './activity-state.js';
...
registerHandlers({
  ...
  session_list: function (msg) {
    ...
    // lr-5edd64: 'processing' (the transcript footer's field) is now a pure
    // PROJECTION of server-authoritative per-session state — nothing writes
    // it independently anymore (see activity-latch.js's header comment for
    // what this replaced). session_list already carries per-session
    // isProcessing to EVERY client regardless of which session that client
    // has focused (lib/sessions.js broadcastSessionList/mapSessionForClient
    // is not scoped by ws._clagenticActiveSession the way status/done/
    // auth_required sends are) — this is what makes re-deriving here, on
    // every list broadcast, correct for a session whose activity edge this
    // client's own WS connection never received (MILLER lr-5edd64 comment
    // #3, candidate 1: the server-side ws._clagenticActiveSession send filter,
    // not the client-side latch, was dropping the edge before it ever
    // arrived). The other query/snapshot half of the projection is
    // session_switched (below), covering reconnect/switch/hydration.
    // Bounded to the entry matching activeSessionId; store.set() and the
    // footer's own subscriber both already no-op on an unchanged value
    // (lr-9bcd7b's changed-flag bounding), so this cannot introduce a
    // spurious render on every roster broadcast.
    //
    // { ignoreUnread: true } (PEACHES finding 2, lr-5edd64 8th recurrence):
    // the transcript footer is a "a turn is running" indicator, not an
    // unread-alert surface — it has no unread badge affordance of its own,
    // unlike the sidebar/hub dots this same sessionActivity() call also
    // drives. Without this, sessionActivity()'s alert>processing precedence
    // (activity-state.js's LOCKED rule) makes .active true whenever
    // unread>0 REGARDLESS of isProcessing, so a session with isProcessing
    // false and unread>0 pins the footer on with no turn running — the
    // exact "processing dot when idle" symptom this task's operator
    // report described. session_switched's own projection (below) reads
    // sessionIsProcessing directly, which is already immune to this
    // conflation; this call site now matches it. This narrows
    // activity-state.js's documented ignoreUnread contract ("session-level
    // dots never set this" — see its header) for this ONE session-level
    // consumer, since the footer is a processing indicator, not a dot with
    // its own alert affordance; see that header comment for the updated
    // contract.
    // Derivation itself lives in activity-state.js's deriveSessionListProcessing
    // (extracted, not inlined here) specifically so a test can import and
    // exercise this exact production logic directly — app-messages.js is
    // not importable under plain Node (see that function's own doc comment
    // for why), so leaving this inline would leave only a hand-copied
    // reimplementation testable, not the shipped code.
    var _projection = deriveSessionListProcessing(msg.sessions || [], store.get('activeSessionId'));
    if (_projection.found) {
      store.set({ processing: _projection.active });
      // lr-5edd64 (9th recurrence): reconcile the footer's DOM ref
      // unconditionally, not only when store.set() actually changed the
      // value — see app-favicon.js's reconcileActivityFooter() doc for why
      // an unchanged-but-still-true value can otherwise leave a ref nulled
      // by resetClientState() permanently unrepaired.
  ...
  session_switched: function (msg) {
    ...
    // NOTE: the 'processing' projection itself is applied further down,
    // AFTER resetClientState() (PEACHES finding 1, lr-5edd64 8th
    // recurrence) — see the comment down there for why the ordering
    // matters. Do not move this store.set({ processing: ... }) back up
    // here without re-reading that comment.
    if (msg.vendor) {
    ...
    clearRemoteCursors();
    resetClientState();
    // Re-derive 'processing' from sessionIsProcessing (set above), NOT from
    // a second raw msg.isProcessing read — this file's CI invariant test
    // locks .isProcessing reads to exactly the one documented site above,
    // so every switch/reconnect/hydration re-derives 'processing' from that
    // already-read, already-reconciled value instead of trusting whatever
    // the client-local latch happened to be left at.
    //
    // Applied HERE, AFTER resetClientState(), not right after
    // sessionIsProcessing is set above (PEACHES finding 1, lr-5edd64 8th
    // recurrence): resetClientState() calls setActivityEl(null), nulling the
    // footer widget's DOM ref, and wipes the transcript DOM outright. Running
    // the projection after the reset means the incoming session's own
    // transcript DOM already exists by the time the footer is (re)rendered.
    //
    // The render call itself is UNCONDITIONAL (lr-5edd64, 9th recurrence —
    // PEACHES BLOCKING on PR #410 + HOLDEN's trace), not left to
    // initActivityFooter's ['processing'] value-change subscriber: on a
    // true->true session switch (outgoing session processing, incoming
    // session also processing) store.set() below does not change the value,
    // so a value-change subscriber alone would never fire and the ref
    // resetClientState() just nulled would never get repaired — stuck OFF
    // forever, the exact inverse of the stuck-ON failure this redesign
    // exists to close (see finding 1's fix above, which fixed false->true
    // and left true->true broken this same way). reconcileActivityFooter()
    // renders unconditionally from current store.processing instead of
    // depending on a transition firing — see its doc comment in
    // app-favicon.js. This does not defeat lr-9bcd7b's changed-flag
    // bounding on store.set()/the subscriber, which still exists and still
    // suppresses spurious renders on unrelated store updates.
  ...
  status: function (msg) {
    // lr-5edd64: this handler no longer writes 'processing' (the transcript
    // footer's field) — that is now a pure projection derived from
    // session_list/session_switched (see those handlers). Session-scoping
    // still gates the OTHER, non-render side effects below (sessionIsProcessing
    // for dead-session-todo-compaction, and the pre-thinking-dots clear),
    // mirroring the lr-0827ba pattern already used by scheduled_message_*
    // handlers (msg.localId == null || msg.localId === activeSessionId) —
    // a background session's status:"processing" must not touch state
    // scoped to the FOCUSED session. msg.localId is always populated
    // server-side (see sessions.js stampActivityLocalId / project.js
    // sendToSession); a missing localId (e.g. an older server) falls back
    // to applying unconditionally, same as before this fix.
  ...
  // lr-5edd64: this handler no longer writes 'processing' — it is now a
  // pure projection derived from session_list/session_switched (see those
  // handlers). Every cleanup call below is unconditional: it targets
  // DOM/tool state for whatever session this client's own WS connection is
  // bound to server-side (ws._clagenticActiveSession), so it is never
  // cross-session — there was no other session-scoped state to guard here
  // once the store-wide latch write is gone.
  done: function (msg) {
    removePreThinking();
  ...
  // lr-5edd64: no longer session-scope-guards a 'processing' write here —
  // that field is now a pure projection (see session_list/session_switched
  // handlers), and nothing else in this handler was gated by the guard.
  ...
  loop_iteration: function (msg) {
    ...
    //
    // lr-fd38ac follow-up: loopCurrentSessionId (set just above) is what
    // makes updateLoopInputVisibility()'s gate condition
    // (loopCurrentSessionId === activeSessionId) newly true when a
    // client is already viewing the loop's new session — the
    // session_switched broadcast that forced every client onto this
    // session fired earlier with loop: null (session.loop is assigned
    // server-side only after createSession() returns; see
    // project-loop.js runNextIteration()), so that call armed nothing.
    // The re-invocation itself lives in app-loop-ui.js's reactive
    // store.subscribe(), keyed on loopActive/loopCurrentSessionId, so
    // every writer of those two fields (this handler, loop_judging,
    // loop_finished) gets the re-check for free instead of each call
  ...
  loop_judging: function (msg) {
    ...
    // lr-fd38ac follow-up: same re-arm requirement as loop_iteration —
    // loopCurrentSessionId changes to the judge session here, and the
    // reactive subscriber in app-loop-ui.js re-checks the gate.
  },
