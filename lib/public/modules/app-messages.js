    ...
    switch (msg.type) {
      ...
      case "history_done":
        ...
        if (messagesEl) {
          ...
          // Re-arm after highlight batch completes — Highlight.js defers work
          // via requestAnimationFrame, so the actual DOM updates may happen
          // after the synchronous call returns. This re-arm catches the deferred
          // work on slow browsers.
          setTimeout(function () {
            if (!store.get('processing')) {
              armStickyBottom(500, true); // force: deferred post-highlight pin
            }
          }, 300);
        }
