...
registerHandlers({
  ...
  delta: function (msg) {
    ...
    stopThinking();
    resetThinkingGroup();
    // lr-1317b8 (Defect 2): this was an ungated clear-site, missed by the
    // lr-255e clear-site audit. Any parent-turn assistant text delta after a
    // subagent dispatch (e.g. the model narrating between tool calls) killed
    // the indicator while the subagent was still genuinely running. Same
    // guard as the 'result'/'done' handlers below.
  ...
  result: function (msg) {
    // lr-255e: the parent turn's SDK stream ends with a normal 'result' as
    // soon as a Task tool call is dispatched -- the Task itself keeps
    // running as a background subagent. Clearing the activity indicator
    // unconditionally here made the "thinking dots" vanish while a
    // dispatched subagent was still genuinely working. Only clear it if no
    // subagent is currently live; markSubagentDone() clears it once the
    // last live subagent actually finishes.
    if (!hasActiveSubagents()) {
      setActivity(null);
    }
    stopThinking();
  ...
  context_usage: function (msg) {
    ...
    removePreThinking();
    // lr-255e: 'done' can also fire while a backgrounded Task subagent is
    // still genuinely running (task-stop/error/abort paths that bypass the
    // 'result' handler never clear activeTaskToolIds). Same rule as the
    // 'result' handler: don't kill the indicator out from under a live
    // subagent. resetToolState() below intentionally still clears all
    // per-turn tool tracking (including subagent liveness) once this
    // handler is done deciding whether to clear the indicator.
    if (!hasActiveSubagents()) {
      setActivity(null);
    }
    stopThinking();
