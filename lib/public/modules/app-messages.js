...
registerHandlers({
  ...
  session_switched: function (msg) {
    ...
    // recurrence): resetClientState() calls setActivityEl(null), nulling
    // the footer widget's DOM ref. app-favicon.js's initActivityFooter
    // subscriber fires SYNCHRONOUSLY inside this store.set() call — if it
    // ran before resetClientState(), an incoming already-processing session
    // would raise the widget against the OUTGOING session's transcript DOM
    // (about to be wiped by resetClientState()'s innerHTML=""), leaving
    // setActivityEl pointing at a node that's already gone. A LATER
    // session_list broadcast carrying the same `true` value for this
    // session would then no-op (store.set()'s changed-flag bounding,
    // lr-9bcd7b) and never re-fire the subscriber to repair it — the
    // footer would be stuck OFF forever, the exact inverse of the stuck-ON
    // failure this redesign exists to close. Running the projection after
    // the reset means the subscriber (if it fires) always sees a fresh,
    // valid DOM ref.
