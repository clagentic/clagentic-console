...
registerHandlers({
  ...
  session_list: function (msg) {
    ...
    // is not scoped by ws._clayActiveSession the way status/done/
    ...
    // #3, candidate 1: the server-side ws._clayActiveSession send filter,
  ...
  // bound to server-side (ws._clayActiveSession), so it is never
