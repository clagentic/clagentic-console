...
export function processMessage(msg) {
    ...
    switch (msg.type) {
      ...
      case "history_done":
        ...
        // Restore cumulative usage totals from server-supplied authoritative data
        if (msg.cumulativeUsage) {
          var cu = msg.cumulativeUsage;
          setSessionUsage({
            cost:       msg.lastCost != null ? msg.lastCost : 0,
            input:      cu.input      || 0,
            output:     cu.output     || 0,
            cacheRead:  cu.cacheRead  || 0,
            cacheWrite: cu.cacheWrite || 0,
            turns:      cu.turns      || 0,
          });
        }
        updateContextPanel();
