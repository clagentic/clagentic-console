...
function pinToBottomNow() {
  // Suppress scroll-during-replay: history messages render fast enough that
  // every per-message pin is visible as a janky scroll on mobile. We do one
  // instant jump at history_done instead (replayingHistory is cleared before
  // armStickyBottom fires there, so that final pin goes through unguarded).
...
export function armStickyBottom(durationMs, force) {
  if (prependAnchor) return; // never fight pagination
  // If the user has intentionally scrolled up to read history, don't yank them
  // back to the bottom. History replay callers pass force=true to override this.
  ...
  if (stickyBottomQuietTimer) clearTimeout(stickyBottomQuietTimer);
  stickyBottomQuietTimer = setTimeout(disarmStickyBottom, stickyBottomQuietMs);
  // Hard ceiling: use at least the quiet window + 5s headroom so a long
  // history replay arm (durationMs=5000) can't be cut short by a stale
  // ceiling set from a prior shorter arm.
  var ceiling = Math.max(stickyBottomCeilingMs, (durationMs || 750) + 5000);
  if (stickyBottomCeilingTimer) clearTimeout(stickyBottomCeilingTimer);
  stickyBottomCeilingTimer = setTimeout(disarmStickyBottom, ceiling);
}
...
export function scrollToBottom() {
  ...
  // During history replay, every appended message calls scrollToBottom(), which
  // queues a rAF. On mobile these all fire visibly, making it look like the page
  // is scrolling through history. Suppress — history_done does one final jump.
  if (store.get('replayingHistory')) return;
  var newMsgBtn = document.getElementById("new-msg-btn");
  if (isUserScrolledUp) {
...
export function forceScrollToBottom(quietMs) {
  ...
  // Always force=true here: caller explicitly wants to jump to bottom (user sent
  // a message, clicked "↓ Latest", or equivalent intentional action).
  armStickyBottom(quietMs || 750, true);
}
...
export function finalizeAssistantBlock() {
  ...
  // Capture replay state synchronously before any async work — by the time
  // the async callback fires, replayingHistory may have already been cleared
  // by history_done. wasReplaying lets the callback know not to arm
  // sticky-bottom (history_done owns that arm during replay).
  var wasReplaying = store.get('replayingHistory');
      ...
      renderMarkdownAsync(textToRender).then(function (html) {
        setTimeout(function () {
          if (contentEl.isConnected) {
            ...
            // Only re-arm sticky-bottom for live turns. During replay,
            // history_done owns the sticky-bottom arm. Per-turn arms from
            // replayed finalizeAssistantBlock callbacks keep extending the
            // quiet window after history is done, leaving the user scroll-
            // pinned for several extra seconds on large sessions.
            if (!wasReplaying) {
              armStickyBottom(500);
            }
          }
...
export function addUserMessage(text, images, pastes, fromUserId, fromUserName) {
  ...
  var isOtherUser = myUserId && fromUserId && fromUserId !== myUserId;
