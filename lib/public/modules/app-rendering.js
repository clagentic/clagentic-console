...
export function appendDelta(text) {
  ...
  streamBuffer += text;
  // During history replay, skip per-delta rendering entirely. The text
  // accumulates in streamBuffer and is flushed by finalizeAssistantBlock()
  // at each turn boundary (result/done). Skipping intermediate renders
  // eliminates hundreds of worker round-trips (one per delta event) and
  // makes replay 10-25x faster on long sessions.
