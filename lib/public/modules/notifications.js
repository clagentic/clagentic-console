...
export function initNotifications(_ctx) {
  ...
  (function () {
    ...
    if (copyBtn) {
      copyBtn.addEventListener("click", function (e) {
        ...
        var cmdText = cmdEl ? cmdEl.textContent : "npx @clagentic/console@latest";
  ...
  (function initServiceWorker() {
    ...
    navigator.serviceWorker.register("/sw.js")
      .then(function (reg) {
        // sw.js is served no-cache (server.js serveStatic), so reg.update()
        // always re-fetches it from the network and picks up a post-deploy
        // change. Without an explicit update() call the browser only
        // re-checks sw.js on its own heuristic schedule (typically once per
        // navigation, ~24h max) — a foregrounded mobile PWA that never
        // navigates can hold a pre-deploy SW+cache indefinitely (lr-e0ec47).
        reg.update().catch(function () {});

        // Re-check for a newer sw.js whenever the PWA is foregrounded, so a
        // backgrounded-then-resumed mobile client re-validates promptly
    ...
    // New service worker took control — notify user rather than reloading blindly
    navigator.serviceWorker.addEventListener("message", function (event) {
      if (!event.data || event.data.type !== "sw-updated") return;

      // If page is hidden (backgrounded PWA, other tab) reload immediately — user won't notice
      if (document.visibilityState === "hidden") {
        window.location.reload();
        return;
      }

      // Page is visible: show a persistent banner so the user can reload when ready
      ...
      banner.innerHTML =
        '<span class="sw-update-banner-text">New version available</span>' +
        '<button class="sw-update-banner-btn" id="sw-update-reload">Reload</button>' +
        '<button class="sw-update-banner-dismiss" id="sw-update-dismiss" aria-label="Dismiss">&#x2715;</button>';
