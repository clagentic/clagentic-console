...
import { sendWs } from './ws-ref.js';

// lr-e85fec: allowlist of actionable.action values this client will act on.
// A diagnostic's `actionable.action` string is never sent to the server
// verbatim as a WS message type — only a value in this map is ever wired to
// a click, so the backend cannot steer the client into sending an arbitrary
// WS command by way of a diagnostic message.
var ACTIONABLE_HANDLERS = {
  restart_for_build_update: function () { sendWs({ type: 'restart_for_build_update' }); },
};

// lr-e85fec: shared by both render sites below (panel entry + toast). Returns
// a clickable button when entry.actionable.action is a known action, or null
// when there is no label or no known handler — callers render their own
// existing icon+text hint in the null case, so this only owns the "known
// action -> real button" half of the branch, not the fallback shape.
function _buildActionableEl(entry, btnClass) {
  if (!entry.actionable || !entry.actionable.label) return null;
  var handler = entry.actionable.action ? ACTIONABLE_HANDLERS[entry.actionable.action] : null;
  if (!handler) return null;
  var btn = document.createElement("button");
  btn.className = btnClass;
  btn.textContent = entry.actionable.label;
...
function _showDiagnosticToast(entry) {
  ...
  // lr-e85fec: known actionable.action -> real button; else the plain hint.
  var actionBtn = _buildActionableEl(entry, "toast-diagnostic-action-btn");
  if (actionBtn) {
  ...
  // Auto-dismiss after 6s — EXCEPT when an action button is present: an
  // operator deciding whether to restart must not have the offer vanish out
  // from under them before they've read it (lr-e85fec).
  var timer = actionBtn ? null : setTimeout(function () { _removeDiagnosticToast(el); }, 6000);
...
function _appendEntry(entry) {
  ...
  // Actionable hint: surface label when present. lr-e85fec: a known action
  // renders as a real button (_buildActionableEl), not just a hint icon — a
  // detector with no attached actuator is the failure mode this closes.
  if (entry.actionable && entry.actionable.label) {
    var actionEl = document.createElement("div");
    actionEl.className = "diagnostics-actionable";

    var panelBtn = _buildActionableEl(entry, "diagnostics-actionable-btn");
    if (panelBtn) {
      actionEl.appendChild(panelBtn);
    } else {
