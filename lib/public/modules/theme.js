...
function themeIdForMode(mode) {
  ...
  // Default skin pair: clagentic-dark / clagentic-light
  if (!skin) {
    return mode === "light" ? "clagentic-light" : "clagentic-dark";
  }

  // Custom skin — try to find the counterpart
  var current = getTheme(skin);
  if (!current) return mode === "light" ? "clagentic-light" : "clagentic-dark";
  ...
  if (mode === "light") {
    ...
    return "clagentic-light";
  } else {
    if (darkId && themes[darkId]) return darkId;
    return "clagentic-dark";
  }
}
