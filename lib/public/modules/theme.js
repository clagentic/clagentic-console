...
var defaultExactVars = {
  ...
  "--warning": "#4A7FE8",
  "--sidebar-bg": "#04060B",
  "--sidebar-hover": "#0A0E19",
  "--sidebar-active": "#162134",
  "--filebrowser-bg": "#09090D",
  "--filebrowser-border": "#121B2C",
...
var defaultFallback = {
  ...
  base00: "060810", base01: "111827", base02: "1E2E48", base03: "3D5068",
...
function computeVars(theme) {
  ...
  return {
    ...
    "--success":        b.base0B,
    "--warning":        b.base0A,
    "--sidebar-bg":     isLight ? darken(b.base00, 0.03) : darken(b.base00, 0.30),
    "--sidebar-hover":  isLight ? darken(b.base00, 0.07) : mixColors(b.base00, b.base01, 0.4),
    "--sidebar-active": isLight ? darken(b.base01, 0.06) : mixColors(b.base01, b.base02, 0.4),
...
var changeCallbacks = [];
var STORAGE_KEY = "clagentic-theme";
var MODE_KEY = "clagentic-mode";        // "light" | "dark" | null (system)
var SKIN_KEY = "clagentic-skin";        // theme id within current variant pair
var WIDE_KEY = "clagentic-wide-view";   // cached from server, "bubble" | "channel"
var BRAND_KEY = "clagentic-brand";      // "clagentic" | "classic"
...
function getEffectiveMode() {
  var saved = null;
  try { saved = localStorage.getItem(MODE_KEY) || localStorage.getItem("clay-mode"); } catch (e) {}
...
export function getBrand() {
  var b = null;
  try { b = localStorage.getItem(BRAND_KEY) || localStorage.getItem("clay-brand"); } catch (e) {}
...
export function setBrand(brand) {
  ...
  applyTheme(tid, true);
  // Persist to server so mobile/cross-device stays in sync
  fetch("/api/user/theme-brand", {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ themeBrand: b }),
  }).catch(function () {});
}
...
export function isWideView() {
  try {
    var v = localStorage.getItem(WIDE_KEY) || localStorage.getItem("clay-wide-view");
...
// Apply server-supplied theme preferences (themeMode, themeBrand) from /api/profile.
// Call this after profile loads. Server values take precedence over localStorage.
export function applyServerThemePrefs(themeMode, themeBrand) {
  var changed = false;
  if (themeBrand && (themeBrand === "classic" || themeBrand === "clagentic")) {
    var local = null;
    try { local = localStorage.getItem(BRAND_KEY) || localStorage.getItem("clay-brand"); } catch (e) {}
    if (local !== themeBrand) {
      try { localStorage.setItem(BRAND_KEY, themeBrand); } catch (e) {}
      changed = true;
    }
  }
  if (themeMode && (themeMode === "light" || themeMode === "dark")) {
    var localMode = null;
    try { localMode = localStorage.getItem(MODE_KEY) || localStorage.getItem("clay-mode"); } catch (e) {}
    if (localMode !== themeMode) {
      try { localStorage.setItem(MODE_KEY, themeMode); } catch (e) {}
      changed = true;
    }
  }
  if (changed) {
    var mode = getEffectiveMode();
    var tid = themeIdForMode(mode);
    applyTheme(tid);
    updateToggleIcon();
  }
}
...
export function initTheme() {
  ...
  var saved = null;
  try { saved = localStorage.getItem(STORAGE_KEY) || localStorage.getItem("clay-theme"); } catch (e) {}
  ...
  if (window.matchMedia) {
    ...
    var handler = function () {
      var userMode = null;
      try { userMode = localStorage.getItem(MODE_KEY) || localStorage.getItem("clay-mode"); } catch (e) {}
      if (!userMode) {
