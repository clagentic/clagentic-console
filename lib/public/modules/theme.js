...
var themes = {};
var customSet = {};   // ids that came from ~/.clagentic/themes/
...
function buildPickerContent() {
  ...
  // Default themes always first in their section
...
export function applyServerThemePrefs(themeMode, themeBrand) {
  ...
  if (themeBrand && (themeBrand === "classic" || themeBrand === "clagentic")) {
    var local = null;
    try { local = localStorage.getItem(BRAND_KEY); } catch (e) {}
    if (local !== themeBrand) {
  ...
  if (themeMode && (themeMode === "light" || themeMode === "dark")) {
    var localMode = null;
    try { localMode = localStorage.getItem(MODE_KEY); } catch (e) {}
...
export function initTheme() {
  ...
  var islandToggle = document.getElementById("user-theme-toggle-btn");
  if (islandToggle && !islandToggle._clagenticThemeBound) {
    islandToggle._clagenticThemeBound = true;
