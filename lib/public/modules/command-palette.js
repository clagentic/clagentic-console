...
import { projectIconHtml } from './project-icon.js';
...
// Read-only accessor for the version this client's WS connection last
// reported as running (set from the "info" handshake message). Used by the
// stale-bundle watchdog in app-connection.js to detect a version mismatch
// against the server's currently served build (lr-e0ec47).
export function getPaletteVersion() {
...
function updateFooter() {
  ...
  footerEl.innerHTML =
    '<a href="https://github.com/clagentic/clagentic-console" target="_blank" rel="noopener" class="cmd-palette-brand"><img src="favicon-banded.png" width="13" height="13" alt="">Clagentic: Console ' + versionText + '</a>' +
...
function renderHome(filter) {
  ...
  if (recentSessions.length > 0) {
    ...
    for (var r = 0; r < recentSessions.length; r++) {
      ...
      // emoji to escaped text — the prior raw s.projectIcon interpolation
      // showed ':slug:' as literal text (lr-a6da).
      var iconHtml = projectIconHtml(s.projectIcon, '<i data-lucide="message-square"></i>');
      var projLabel = projectIconHtml(s.projectIcon) + " " + escapeHtml(s.projectTitle || s.projectSlug);
  ...
  if (filteredProjects.length > 0) {
    ...
    for (var j = 0; j < filteredProjects.length; j++) {
      ...
      // projectIconHtml() routes custom (:slug:) icons to a safe <img> tag;
      // the prior raw proj.icon interpolation (via refreshIcons(), which only
      // converts <i data-lucide> placeholders) showed ':slug:' as literal
      // text (lr-a6da).
      html += renderItem(flatIndex, projectIconHtml(proj.icon, '<i data-lucide="box"></i>'), pName, sessLabel, null);
      flatIndex++;
    }
  }
...
function appendHomeSearchResults(results, query) {
  ...
  for (var i = 0; i < newResults.length; i++) {
    ...
    // projectIconHtml() routes custom (:slug:) icons to a safe <img> tag and
    // emoji to escaped text — the prior raw r.projectIcon interpolation
    // showed ':slug:' as literal text (lr-a6da).
    var srIconHtml = projectIconHtml(r.projectIcon, '<i data-lucide="message-square"></i>');
    var projLabel = projectIconHtml(r.projectIcon) + " " + escapeHtml(r.projectTitle || r.projectSlug);
...
function renderItem(index, iconContent, title, desc, snippet, timestamp) {
  var dateStr = relativeTime(timestamp);
