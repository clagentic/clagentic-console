  ...
  for (var i = 0; i < editBtns.length; i++) {
    (function (btn) {
      btn.addEventListener("click", function (e) {
        ...
        // Setting sidebarListEl.innerHTML during renderSidebar() detaches this
        // <input> from the DOM, which the browser treats as a blur — the
        // "blur" listener below still fires and would call finishEdit() and
        // commit the rename even though the user pressed Escape to cancel.
        // cancelled short-circuits that so Escape truly discards the edit.
        var cancelled = false;
        function finishEdit() {
          if (cancelled) return;
        ...
        input.addEventListener("keydown", function (ev) {
          ...
          if (ev.key === "Escape") { ev.preventDefault(); cancelled = true; renderSidebar(); }
        });
...
function renderModelTab(bodyEl, rec) {
  ...
  bodyEl.innerHTML =
    '<div class="scheduler-model-settings">' +
      '<div class="settings-card"><div class="settings-field">' +
        ...
        '<div id="ls-model-scope" class="config-scope-label"></div>' +
  ...
  var opts = {
    ...
    // lr-db0437: loop-scoped — this picker configures the specific scheduled/
    // loop task being edited, not the current session or any project/server
    // default.
    scopeLabel: "Task default (this scheduled task only)",
  };
  ...
  for (var w = 0; w < 6; w++) {
    ...
    for (var d = 0; d < 7; d++) {
      ...
      for (var e = 0; e < events.length && e < 3; e++) {
        ...
        html += '<span class="scheduler-event-time">' + esc(ev.timeStr) + '</span> ' + esc(ev.name);
        html += '</div>';
      }
      if (events.length > 3) {
...
function renderWeekView() {
  ...
  for (var d = 0; d < 7; d++) {
    ...
    for (var e = 0; e < events.length; e++) {
      ...
      if (ev.intervalBadge) {
        var badgeStyle = "";
        var badgeColor = safeColor(ev.color);
        if (badgeColor) badgeStyle = "background:" + badgeColor;
      ...
      var evColor = safeColor(ev.color) || "";
...
function showPopover(recId, anchorEl) {
  ...
  html += '<div class="schedule-popover-meta">' + esc(cronToHuman(rec.cron)) + '</div>';
...
function showMovePopover(recId, anchorEl) {
  ...
  if (projects.length === 0) {
    ...
    popoverEl.classList.remove("hidden");
    positionPopover(popoverEl, anchorEl, { placement: "below" });
    return;
  }
  ...
  // Four-edge clamp (lr-a10a): the prior left-only clamp could still leave
  // the popover with its bottom edge off-screen on short mobile viewports.
  positionPopover(popoverEl, anchorEl, { placement: "below" });
...
// Server-controlled schedule color values are interpolated into string-built
// style="" attributes. Only accept a strict hex color; anything else (e.g. an
// injected attribute breakout) is dropped rather than rendered.
