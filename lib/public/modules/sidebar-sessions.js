...
function renderSessionTopActions() {
  ...
  if (currentSlug && !store.get('dmMode')) {
  ...
  // Slot 2 (top-right): preferred agent quick-launch.
  // When set: launches a session with that agent directly.
  // When unset: shown only once dmMode is off; renders as a muted "set agent…" affordance.
  if (!store.get('dmMode')) {
    ...
    if (preferredAgent) {
      ...
      // No preferred agent yet — show a dimmed placeholder so the slot isn't empty.
      favBtn.className = "session-top-action session-top-action--set-preferred";
      favBtn.innerHTML = iconHtml("bot") + '<span>Set preferred agent</span>';
      favBtn.title = 'Set a preferred agent for this project';
      favBtn.addEventListener("click", function () {
        var slug = store.get('currentSlug');
        if (slug) openAgentPickerForPreferred(slug);
      });
    }
  ...
  // Slot 4 (bottom-right): always "Start agent chat" (full picker) — hidden in dmMode.
  if (!store.get('dmMode')) {
    var agentBtn = document.createElement("button");
    agentBtn.className = "session-top-action";
    agentBtn.type = "button";
    agentBtn.innerHTML = iconHtml("message-circle-plus") + '<span>Start agent chat</span>';
