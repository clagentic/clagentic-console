...
// firing a real user "blur". Committing on every rebuild (the original
// design here) turned out to be wrong: an actively-streaming session
// broadcasts session_list many times per turn, so the rename committed
// within a keystroke or two, sending a partial title as a real
// rename_session (durable write + permanent titleManuallySet=true — see
// lr-16b88d). Instead, `suspend()` captures the in-progress edit (value +
// caret) without committing or cancelling it, so renderSessionList can
// preserve it across a real rebuild by re-opening the rename afterward
// (see the suspendedRename re-open block at the end of renderSessionList).
// Precedent: scheduler.js's cancelled short-circuit flag / detach-as-blur
// handling (lr-fb49-E/F family).
...
function startInlineRename(sessionId, currentTitle, resume) {
  ...
  // Capture the live edit without settling it (no commit, no cancel, no WS
  // send) — used by renderSessionList() ahead of a real rebuild so the edit
  // can be re-opened afterward instead of force-committed. Does not flip
  // `settled`: the DOM node this closure holds (input/textSpan) is about to
  // be torn down by the rebuild regardless, so there is nothing left here
  // for a stray blur/keydown to settle against once suspend() returns.
...
// resume: see startInlineRename's matching parameter.
function startLoopInlineRename(loopId, currentName, resume) {
  ...
  // See startInlineRename's suspendRename — same non-settling capture.
  function suspendRename() {
    return { value: input.value, selectionStart: input.selectionStart, selectionEnd: input.selectionEnd };
  }
...
export function renderSessionList(sessions) {
  ...
  // lr-16b88d. Instead, capture the in-progress edit via suspend() (no
  // commit, no cancel, no WS send) so it can be re-opened after the rebuild
  // below, once we know whether one is actually going to run.
  var suspendedRename = activeRename ? {
    type: activeRename.type,
    id: activeRename.id,
    currentTitle: activeRename.currentTitle,
    snapshot: activeRename.suspend(),
  } : null;
  clearArmedSessionDelete();
  ...
  if (fp === _sessionListFingerprint) {
    // No-op render: nothing was torn down, so the suspended rename's input
    // is still live in the DOM — leave it alone (activeRename is unchanged).
