...
// --- Web Worker for off-main-thread markdown parsing ---
// The worker handles marked.parse() (the heavy part). DOMPurify.sanitize()
// runs on the main thread after the worker returns, since it needs DOM access.
// Falls back to synchronous renderMarkdown() if workers are unavailable.
var _mdWorker = null;
var _mdWorkerReady = false;
var _mdPending = {}; // id -> { resolve, reject }
var _mdIdCounter = 0;

function _initMdWorker() {
  if (_mdWorker || typeof Worker === "undefined") return;
  try {
    _mdWorker = new Worker("/markdown-worker.js");
    _mdWorker.onmessage = function (e) {
      var id = e.data.id;
      var cb = _mdPending[id];
      if (cb) {
        delete _mdPending[id];
        cb.resolve(e.data.html);
      }
    };
    _mdWorker.onerror = function (err) {
      // Worker failed — clear it so future calls fall back to sync
      _mdWorker = null;
      _mdWorkerReady = false;
      // Reject all pending
      for (var k in _mdPending) {
        _mdPending[k].reject(err);
        delete _mdPending[k];
      }
    };
    _mdWorkerReady = true;
...
// Async render: uses worker when available, falls back to sync.
// Returns a Promise<string> of sanitized HTML.
export function renderMarkdownAsync(text) {
  if (!_mdWorkerReady || !_mdWorker) {
    return Promise.resolve(renderMarkdown(text));
  }
  var id = ++_mdIdCounter;
  return new Promise(function (resolve, reject) {
    _mdPending[id] = {
      ...
      reject: function (err) {
        // Worker failed mid-flight — fall back to sync
        resolve(renderMarkdown(text));
      }
    };
    _mdWorker.postMessage({ id: id, text: text });
  });
}
...
// Inject copy buttons into all <pre> blocks in el that don't have one yet.
// Shared by the sync highlight path (per-message streaming) and the batched
// path (post-history_done full-container pass).
export function injectCopyButtons(el) {
...
export function highlightCodeBlocksBatched(container, onDone) {
  ...
  if (blocks.length === 0) {
    // No unhighlighted code blocks, but there may be pre blocks without copy
    // buttons (e.g. plain code fences that were already highlighted). Inject now.
  ...
  function processBatch(deadline) {
    ...
    if (idx < blocks.length) {
      ...
      // All blocks highlighted — inject copy buttons in one pass over the
      // whole container. This is the post-history_done batch path; the sync
      // highlightCodeBlocks() path handles per-message streaming injection.
      injectCopyButtons(container);
      if (onDone) onDone();
    }
  }
...
export function showMermaidModal(svgHtml) {
  ...
  // svgHtml originates from container.innerHTML after mermaid render — sanitize
  // defensively here too since showMermaidModal is also exported and callable
  // from other paths.
  body.innerHTML = DOMPurify.sanitize(svgHtml, { USE_PROFILES: { svg: true, svgFilters: true } });
