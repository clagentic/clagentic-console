...
function enableMultiUserMode() {
  var modal = document.createElement("div");
  modal.className = "admin-modal-overlay";
  modal.innerHTML = '<div class="admin-modal">' +
    '<div class="admin-modal-body" style="padding:20px 16px 16px">' +
    '<p class="admin-modal-desc" style="margin:0;font-size:14px;color:var(--text)">Enable multi-user mode? You will be directed to set up the admin account. Your current PIN will become the admin PIN.</p>' +
    '</div>' +
    '<div class="admin-modal-footer">' +
    '<button class="admin-modal-save" id="confirm-enable-multiuser">Enable</button>' +
    '<button class="admin-modal-cancel">Cancel</button>' +
    '</div></div>';
  document.body.appendChild(modal);
  refreshIcons(modal);
  modal.querySelector(".admin-modal-cancel").addEventListener("click", function () { modal.remove(); });
  modal.addEventListener("click", function (e) { if (e.target === modal) modal.remove(); });
  modal.querySelector("#confirm-enable-multiuser").addEventListener("click", function () {
    modal.remove();
    apiPost("/api/settings/enable-multiuser").then(function (data) {
      if (data.ok && data.setupCode) {
        showToast("Multi-user mode enabled. Redirecting to setup...");
        setTimeout(function () {
          window.location.href = "/auth?setupCode=" + encodeURIComponent(data.setupCode);
        }, 500);
      } else {
        showToast(data.error || "Failed to enable multi-user mode");
      }
    }).catch(function () {
      showToast("Failed to enable multi-user mode");
    });
  });
}
...
function renderUsersTab(body) {
  ...
  // Show migration callout if single-user mode with PIN is active
  if (meInfo && meInfo.singleUserMigrationAvailable && !meInfo.multiUser) {
    html += '<div class="migration-callout">' +
      '<strong>Migrate to multi-user mode</strong>' +
      '<p>You\'re currently using single-user PIN mode. Multi-user mode lets you create named accounts with better security. Your existing PIN will continue to work during transition.</p>' +
      '<button id="migrate-to-multiuser-btn" class="admin-action-btn">Set up multi-user mode</button>' +
      '</div>';
  }
  ...
  var migrateBtn = body.querySelector("#migrate-to-multiuser-btn");
  if (migrateBtn) {
...
function renderSmtpTab(body, cfg) {
  ...
  html += '<div class="admin-smtp-row">' +
    ...
    '<input type="text" id="smtp-from" class="admin-smtp-input" placeholder="Clagentic &lt;noreply@example.com&gt;" value="' + escapeHtml((cfg && cfg.from) || "") + '">' +
    '</div>';
