...
function buildModelItem(m, currentModel, listEl, opts) {
  ...
  // lr-f22787 (coordinator follow-up): a retired catalog entry (status
  // "retired" from the deprecations page — see model-catalog.js's
  // applyRetiredMarking) is disabled from selection, not just badged. It
  // provably cannot run; offering it as an ordinary clickable row would
  // read as the picker being broken when the operator's click fails.
  var isRetired = !!(m && m.isRetired);
  item.className = "settings-model-item"
    + (value === currentModel ? " active" : "")
    + (isRetired ? " settings-model-item-disabled" : "");
  item.dataset.model = value;
  if (isRetired) item.setAttribute("aria-disabled", "true");
  ...
  // lr-f22787: entries sourced from the release-time-generated catalog
  // (not the vendor's live enumeration) get a small non-blocking marker —
  // shown, never hidden, so the operator can still pick an older version
  // while knowing it wasn't independently confirmed by this session's live
  // vendor check.
  if (m && m.fromCatalog) {
    var catalogBadge = document.createElement("span");
    catalogBadge.className = "settings-model-catalog-badge";
    catalogBadge.textContent = "Catalog";
    item.appendChild(catalogBadge);
  }

  // lr-f22787 (coordinator follow-up): status-derived markers, decided
  // behavior — see model-catalog.js's header comment for the full
  // rationale. Retired: disabled + "Retired" marker. Deprecated: still
  // selectable (it still runs), "Deprecated" marker only.
  if (isRetired) {
    var retiredBadge = document.createElement("span");
    retiredBadge.className = "settings-model-retired-badge";
    retiredBadge.textContent = "Retired";
    item.appendChild(retiredBadge);
  } else if (m && m.isDeprecated) {
    var deprecatedBadge = document.createElement("span");
    deprecatedBadge.className = "settings-model-deprecated-badge";
    deprecatedBadge.textContent = "Deprecated";
    item.appendChild(deprecatedBadge);
  }
  ...
  item.addEventListener("click", function () {
    // lr-f22787: a retired row never dispatches a selection — this is the
    // "disabled" half of the marking (the badge alone is not a real UI
    // gate; a click handler that does nothing is).
    if (isRetired) return;
