...
function buildModelItem(m, currentModel, listEl, opts) {
  ...
  // lr-d3817f: a retired catalog entry is filtered out entirely upstream
  // (model-catalog.js's filterSelectableCatalogModels) and never reaches
  // this function — there is no isRetired/disabled row rendering here
  // anymore. Only deprecated (still-runnable) entries carry a marker.
  ...
  // lr-f22787: deprecated (not retired) entries still run — marker only,
  // no disabling. Retired entries never reach this function (lr-d3817f).
  if (m && m.isDeprecated) {
