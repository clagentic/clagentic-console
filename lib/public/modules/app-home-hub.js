...
//
// KNOWN LIMITATION (lr-0aa7b6): this is PER-PROJECT, not per-session — a
// row lights red if ITS PROJECT has unread>0, even when only one sibling
// session in that project is actually notifying. A genuine per-session
// cross-project unread signal does not exist server- or client-side today
// (ws._clayUnread in lib/sessions.js is per-connection and keyed by a
// session localId that is only unique WITHIN one project's SessionManager —
// looking it up for sessions pulled from getAllProjectSessions(), which
// spans every project, would misattribute unread across projects with
// colliding localIds; the cross-project aggregate in lib/server.js
// (crossProjectUnread / onSessionDone) is project-slug-only with no
// session id threaded through at all). Extending either mechanism to be
// genuinely per-session touches the same aggregate the project-badge
// unread count depends on today. TODO(lr-0aa7b6): thread a real per-session
// cross-project unread signal (session-level lastReadAt, or restructure
// crossProjectUnread to key by {slug, localId}) so this can key off the
// individual session instead of the project aggregate.
...
export function handleHubRecentSessions(msg) {
  ...
  sessions.forEach(function (sess) {
    ...
    // alert(red) > processing(green, pulsing) > live(green) > idle(grey).
    // Alert wins over an active/processing session so the notification is
    // always visible. Previously alert rendered as a SEPARATE right-side dot
    // (.hub-recent-alert-dot); that's removed — alert is now just another
    // color state of the one left dot.
    var hasAlert = projectHasAlert(sess.projectSlug);
    var dotClass = "hub-recent-dot" +
      (hasAlert ? " alert" : (sess.isProcessing ? " processing" : ""));
    var dotTitle = hasAlert ? "Unread activity" : (sess.isProcessing ? "Active" : "Idle");
    item.innerHTML =
