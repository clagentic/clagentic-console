...
import { getProjectAbbrev } from './sidebar-projects.js';
...
//
// This remains the PROJECT-LEVEL aggregate, intentionally — it still backs
// the Projects-list badge. It is NOT used for the Recent Sessions per-row
// dot below; that now keys off each session's own sess.unread (lr-0aa7b6),
// carried end-to-end from lib/server.js's per-session crossProjectUnread
// restructure through lib/project-loop.js's hub_recent_sessions_list.
  ...
  if (projectsList && projects) {
    ...
    for (var p = 0; p < hubProjects.length; p++) {
      (function (proj) {
        ...
        // lr-66c118: project-level dot — isProcessing-only, no alert tone
        // (the Projects-list unread badge owns that affordance).
        var projActivityCls = indicatorClass(sessionActivity(proj, null, { ignoreUnread: true }));
        dot.className = "hub-project-dot" + (projActivityCls ? " " + projActivityCls : "");
...
export function handleHubRecentSessions(msg) {
  ...
  sessions.forEach(function (sess) {
    ...
    // projectIconHtml() -> safe <img>/escaped text (lr-a6da). No icon ->
    // getProjectAbbrev() fallback, matching sidebar-projects.js /
    // sidebar-mobile.js, instead of a blank slot (lr-e5faff).
    var iconHtml = sess.projectIcon
      ? '<span class="hub-recent-project-icon">' + projectIconHtml(sess.projectIcon) + '</span>'
      : '<span class="hub-recent-project-icon hub-recent-project-icon--abbrev">' +
        escapeHtml(getProjectAbbrev(sess.projectTitle || sess.projectSlug)) + '</span>';
    ...
    var agentBadge = sess.agentName
      ...
      : '';
    // Single left status dot — color precedence LOCKED (andy, lr-0aa7b6),
    // now the CANONICAL derivation for every render surface via
    // activity-state.js's sessionActivity/indicatorClass (lr-66c118): this
    // was the only surface with a real precedence chain before, so it is
    // promoted here rather than left a local special case.
    //
    // Alert state is keyed on THIS session's own unread count (sess.unread,
    // populated server-side per-session — see hub_recent_sessions_list in
    // lib/project-loop.js) — NOT the whole-project alert helper above. That
    // per-project helper would light every sibling row in a notifying
    // project's session list even when only one of them is actually
    // notifying — that was the bug lr-0aa7b6 required fixing, not just
    // documenting.
    var recentActivityState = sessionActivity(sess, store.get('myUserId'));
    var recentActivityCls = indicatorClass(recentActivityState);
    var dotClass = "hub-recent-dot" + (recentActivityCls ? " " + recentActivityCls : "");
    var dotTitle = recentActivityState.label;
    item.innerHTML =
