...
var wtCollapsed = {};
try {
  wtCollapsed = JSON.parse(localStorage.getItem("clagentic-wt-collapsed") || localStorage.getItem("clay-wt-collapsed") || "{}");
} catch (e) {}
...
function showProjectAccessPopover(anchorEl, slug) {
  ...
  requestAnimationFrame(function () {
    positionPopover(popover, anchorEl, { placement: "right", gap: 8 });
...
function renderAccessPopover(popover, slug, access, allUsers) {
  ...
  popover.querySelectorAll(".project-access-vis-btn").forEach(function (btn) {
    btn.addEventListener("click", function () {
      ...
      var prevBtn = popover.querySelector(".project-access-vis-btn.active");
      ...
      fetch("/api/admin/projects/" + encodeURIComponent(slug) + "/visibility", {
        ...
        // Roll back the optimistic UI update so the popover reflects reality
        // instead of showing a visibility that was never actually applied.
        visibility = prevVis;
        popover.querySelectorAll(".project-access-vis-btn").forEach(function (b) { b.classList.remove("active"); });
        if (prevBtn) prevBtn.classList.add("active");
        if (usersSection) usersSection.style.display = prevVis === "private" ? "" : "none";
        showToast("Failed to update project visibility", "warn");
      });
    });
  });
  ...
  popover.querySelectorAll('.project-access-user-item input[type="checkbox"]').forEach(function (cb) {
    cb.addEventListener("change", function () {
      ...
      // The browser already flipped this checkbox's checked state before
      // "change" fires — capture that as the optimistic value so a failed
      // request can be reverted below.
      var thisCb = cb;
      var wasChecked = !thisCb.checked;
      fetch("/api/admin/projects/" + encodeURIComponent(slug) + "/users", {
        ...
        thisCb.checked = wasChecked;
        showToast("Failed to update allowed users", "warn");
      });
    });
  });
}
...
function showIconCtxMenu(anchorEl, slug, name) {
  ...
  requestAnimationFrame(function () {
    positionPopover(menu, anchorEl, { placement: "right" });
  });
}
...
function showProjectCtxMenu(anchorEl, slug, name, icon, position) {
  ...
  requestAnimationFrame(function () {
    // "below" kept its original 4px gap; "right" keeps the shared default (6px).
    positionPopover(menu, anchorEl, position === "below" ? { placement: "below", gap: 4 } : { placement: "right" });
  });
}

// --- Emoji picker ---

// Position an emoji-picker popover relative to an anchor element, keeping it
// fully on-screen. Delegates to the shared viewport-clamp util (lr-a10a,
// generalized from the original lr-149e fix here): desktop is
// anchor-relative (right of the anchor, flipped left + four-edge clamped);
// mobile centers the picker in the viewport and clamps both axes — this is
// what icon-strip.css's mobile block expects (it no longer sets left/right
// itself, so there is nothing for these inline styles to fight).
function positionEmojiPicker(picker, anchorEl) {
  positionPopover(picker, anchorEl, { placement: "right" });
}
...
function showEmojiPicker(slug, anchorEl) {
  ...
  requestAnimationFrame(function () {
    positionEmojiPicker(picker, anchorEl);
  });
}
...
function showMoveFolderMenu(anchorEl, slug, folders, position) {
  ...
  for (var i = 0; i < folders.length; i++) {
    (function (fn) {
      ...
      // LOCKED SCOPE (Andy, lr-a6da): folder icons support custom :slug: images
      // at parity with project icons. projectIconHtml() routes custom icons to
      // a safe <img> tag and emoji to escaped text; the prior raw fmeta2.icon
      // interpolation showed ':slug:' as literal text.
      var ficon = fmeta2.icon ? ('<span style="margin-right:4px">' + projectIconHtml(fmeta2.icon) + '</span>') : (iconHtml("folder") + " ");
...
function setupDragHandlers(el, slug) {
  ...
  el.addEventListener("dragover", function (e) {
    ...
    // Suppress merge zone entirely when the target item is inside a folder.
    // Folders only have one level — merging within a folder makes no sense.
    // Reorder-only (above/below) applies whether the dragged item is in the
    // same folder, a different folder, or no folder at all.
    var targetInFolder = el.classList.contains("in-folder");
    var allowMerge = !targetInFolder;
  ...
  el.addEventListener("drop", function (e) {
    ...
    // Named distinctly from the module-level `draggedFolderName` (set when a
    // whole folder is dragged) — a `var` here previously shared that name and
    // hoisted to the top of this same drop handler, so the whole-folder-drop
    // check earlier in this function (`if (draggedFolderName)`) always read
    // this local's initial `undefined`/`null` instead of the module state,
    // silently disabling folder drag-and-drop reorder onto a project item.
    var draggedItemFolderName = null;
    for (var dfi = 0; dfi < cachedProjectList.length; dfi++) {
      if (cachedProjectList[dfi].slug === draggedSlug) {
        draggedItemFolderName = cachedProjectList[dfi].folderName || null;
        break;
      }
    }
    ...
    var ws2 = getWs();
    if (draggedItemFolderName && draggedItemFolderName !== targetFolderName2) {
    ...
    } else if (!draggedItemFolderName && targetFolderName2) {
      // Entering a folder — add dragged item to the target's folder
      if (ws2 && store.get('connected')) {
        ws2.send(JSON.stringify({ type: "set_project_folder", slug: draggedSlug, folderName: targetFolderName2 }));
      }
    }

    var container = document.getElementById("icon-strip-projects");
    // Collect all project items regardless of nesting depth (includes in-folder items)
...
function setWtCollapsed(slug, collapsed) {
  ...
  try { localStorage.setItem("clagentic-wt-collapsed", JSON.stringify(wtCollapsed)); } catch (e) {}
}
...
// activityOverride (optional): a raw project/rollup-shaped object to derive
// the dot from INSTEAD of p itself, without mutating p — see
// renderWorktreeGroup, which used to fold worktree processing state directly
// onto the shared project object in place (lr-66c118; same disease as the
// July phantom-badge bug, engram 7574673: a later read of p could no longer
// distinguish "this project is processing" from "some worktree was
// processing at last render").
function createIconItem(p, currentSlug, activityOverride) {
  ...
  if (p.icon) {
    ...
    renderProjectIcon(p.icon, emojiSpan, parseEmojis);
  ...
  // lr-66c118: project-level dot — only ever a processing/idle signal, never
  // alert (the project unread badge below owns that affordance) — ignoreUnread
  // so sessionActivity never derives an 'alert' tone here even though p (or
  // activityOverride) carries .unread for the badge above/below.
  var pActivityCls = indicatorClass(sessionActivity(activityOverride || p, null, { ignoreUnread: true }));
  if (pActivityCls) statusDot.classList.add(pActivityCls);
...
function showWorktreeModal(parentSlug, parentName) {
  ...
  // Tracks the message listener + timeout for the in-flight create_worktree
  // request, so a modal close (or a second create attempt) can tear down the
  // previous listener instead of leaking it. Without this, a stacked, never-
  // removed handler from an earlier attempt could still be listening when a
  // later, unrelated create_worktree_result arrives and fire a stale
  // switchProject() for a worktree the user is no longer creating.
  var pendingCreate = null; // { dirName, handler, timeoutId } or null

  function clearPendingCreate() {
    if (!pendingCreate) return;
    var ws = getWs();
    if (ws) ws.removeEventListener("message", pendingCreate.handler);
  ...
  function doCreate() {
    ...
    var ws = getWs();
    if (!ws || !store.get('connected')) {
      errorDiv.textContent = "Not connected — cannot create worktree";
      errorDiv.classList.add("visible");
      return;
    }
    // Discard any previous in-flight attempt before starting a new one.
    clearPendingCreate();
    ...
    var handler = function (event) {
      ...
      if (msg.type !== "create_worktree_result") return;
      // Correlate on dirName — drop replies for a superseded request.
      if (msg.dirName && msg.dirName !== dirName) return;
      clearPendingCreate();
      if (msg.ok) {
        closeModal();
        if (msg.slug && switchProject) switchProject(msg.slug);
      } else {
        onFailure(msg.error || "Failed to create worktree");
      }
    };
    var timeoutId = setTimeout(function () {
      onFailure("Timed out waiting for worktree creation");
    }, 30000);
    pendingCreate = { dirName: dirName, handler: handler, timeoutId: timeoutId };
...
function renderUserFolder(unit, wtByParent, currentSlug, currentDmUserId) {
  ...
  for (var fi = 0; fi < folderProjects.length; fi++) {
    ...
    for (var fwi = 0; fwi < fwt.length; fwi++) {
      ...
      folderActivityItems.push(fwt[fwi]);
      if (fwt[fwi].slug === currentSlug) containsActive = true;
    }
  }
  // lr-66c118: replaces the open-coded anyProcessing OR-loop above. Project
  // dots never carry alert tone (see createIconItem) — ignoreUnread so this
  // stays isProcessing-only, matching, even though the raw items carry
  // .unread for totalUnread above.
  var folderActivityCls = indicatorClass(rollupActivity(folderActivityItems, null, { ignoreUnread: true }));
  ...
  if (fmeta.icon) {
    // LOCKED SCOPE (Andy, lr-a6da): folder icons support custom :slug: images
    // at parity with project icons. renderProjectIcon() routes custom icons to
    // <img> and emoji to text; the prior plain textContent assignment showed
    // ':slug:' as literal text.
    var emojiSpan = document.createElement("span");
    emojiSpan.className = "icon-strip-folder-emoji-main";
    renderProjectIcon(fmeta.icon, emojiSpan, parseEmojis);
  ...
  if (folderActivityCls) fStatusDot.classList.add(folderActivityCls);
...
function renderWorktreeGroup(p, worktrees, currentSlug, currentDmUserId) {
  ...
  // lr-66c118: replaces the prior in-place mutation of the shared project
  // object's processing flag (see createIconItem's activityOverride doc
  // comment for why that was wrong). activityOverride carries whichever raw
  // item (parent or a worktree) is the rollup's own winner, so
  // createIconItem's own sessionActivity() call still does the real
  // derivation — no double-derivation, no raw property read here.
  var worktreeGroupItems = [p].concat(worktrees);
  var worktreeWinner = worktreeGroupItems[0];
  for (var wpi = 1; wpi < worktreeGroupItems.length; wpi++) {
    if (sessionActivity(worktreeGroupItems[wpi], null, { ignoreUnread: true }).active) {
      worktreeWinner = worktreeGroupItems[wpi];
      break;
    }
  }
  var header = createIconItem(p, currentSlug, worktreeWinner);
  ...
  for (var wi = 0; wi < worktrees.length; wi++) {
    (function (wt) {
      ...
      if (wt.icon) {
        ...
        renderProjectIcon(wt.icon, wtEmoji, parseEmojis);
      ...
      var wtActivityCls = indicatorClass(sessionActivity(wt, null, { ignoreUnread: true }));
      if (wtActivityCls) wtStatus.classList.add(wtActivityCls);
...
function createMobileProjectItem(p, currentSlug, isWorktree) {
  ...
  if (p.icon) {
    renderProjectIcon(p.icon, abbrev, parseEmojis);
  } else {
  ...
  var mProjActivityCls = indicatorClass(sessionActivity(p, null, { ignoreUnread: true }));
  if (mProjActivityCls) {
    var dot = document.createElement("span");
    dot.className = "mobile-project-processing " + mProjActivityCls;
    el.appendChild(dot);
  }
