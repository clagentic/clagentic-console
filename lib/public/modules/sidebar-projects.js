...
// Mobile breakpoint mirrors the @media (max-width: 768px) block in icon-strip.css.
var EMOJI_PICKER_MOBILE_BREAKPOINT = 768;

function isMobileViewport() {
  return window.innerWidth <= EMOJI_PICKER_MOBILE_BREAKPOINT;
}

// Position an emoji-picker popover relative to an anchor element, keeping it
// fully on-screen. Desktop: anchor-relative (existing behavior, clamped to
// the right/bottom edges). Mobile: the anchor rect is often near the bottom
// of the viewport (e.g. the mobile action-sheet's synthetic contextmenu), so
// instead of anchoring we center the picker in the viewport and clamp both
// axes — this is what icon-strip.css's mobile block expects (it no longer
// sets left/right itself, so there is nothing for these inline styles to
// fight).
function positionEmojiPicker(picker, anchorEl) {
  if (isMobileViewport()) {
    var pRect = picker.getBoundingClientRect();
    var left = Math.max(8, (window.innerWidth - pRect.width) / 2);
    var top = Math.max(8, (window.innerHeight - pRect.height) / 2);
  ...
  var pRect2 = picker.getBoundingClientRect();
  if (pRect2.right > window.innerWidth - 8) {
    picker.style.left = (rect.left - pRect2.width - 6) + "px";
  }
  if (pRect2.bottom > window.innerHeight - 8) {
    picker.style.top = (window.innerHeight - pRect2.height - 8) + "px";
  }
}
...
function setupDragHandlers(el, slug) {
  ...
  el.addEventListener("drop", function (e) {
    ...
    } else if (!draggedFolderName && targetFolderName2) {
