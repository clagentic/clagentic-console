...
function renderUserFolder(unit, wtByParent, currentSlug, currentDmUserId) {
  ...
  // Folder header pill (the clickable icon)
  var headerEl = document.createElement("div");
  headerEl.className = "icon-strip-folder-header" + (containsActive && isCollapsed ? " active" : "");

  // Folder icon with optional emoji overlay
  var iconWrap = document.createElement("span");
  iconWrap.className = "icon-strip-folder-icon-wrap";
  iconWrap.innerHTML = iconHtml("folder");
  if (fmeta.icon) {
    var overlay = document.createElement("span");
    overlay.className = "icon-strip-folder-emoji-overlay";
    overlay.textContent = fmeta.icon;
    parseEmojis(overlay);
