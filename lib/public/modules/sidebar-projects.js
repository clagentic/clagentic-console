...
function setupDragHandlers(el, slug) {
  ...
  el.addEventListener("dragover", function (e) {
    ...
    var zone = pct < 0.25 ? "above" : pct > 0.75 ? "below" : "merge";
...
function renderUserFolder(unit, wtByParent, currentSlug, currentDmUserId) {
  ...
  // Folder header is a drop target — dragging a project onto it adds it to this folder
  (function (fn) {
    headerEl.addEventListener("dragover", function (e) {
      if (!draggedSlug) return;
      e.preventDefault();
      e.dataTransfer.dropEffect = "move";
      headerEl.classList.add("drag-over-folder");
    });
    headerEl.addEventListener("dragleave", function () {
      headerEl.classList.remove("drag-over-folder");
    });
    headerEl.addEventListener("drop", function (e) {
      ...
      headerEl.classList.remove("drag-over-folder");
      if (!draggedSlug) return;
      // Find if the dragged project is already in this folder
      var alreadyIn = false;
