...
// lr-255e: the parent turn's SDK stream ends with a normal 'result' message
// as soon as a Task tool call is dispatched -- the Task itself keeps running
// as a background child. app-messages.js's 'result' handler unconditionally
// called setActivity(null), which killed the "thinking dots" indicator even
// though a subagent was still actively working. Tracking live subagents here
// (keyed by parentToolId, the same id used for the stop-button dedup guard)
// lets callers ask hasActiveSubagents() before clearing the indicator.
...
export function renderPermissionRequest(requestId, toolName, toolInput, decisionReason, mateId, vendor) {
  ...
  // On WS reconnect the DOM node may survive a pendingPermissions reset.
  // Guard against duplicating the card in that case.
  if (ctx && ctx.messagesEl && ctx.messagesEl.querySelector('[data-request-id="' + requestId + '"]')) return;
  ctx.finalizeAssistantBlock();
  stopThinking();
  closeToolGroup();
  ...
  // Channel layout: conversational "Can I ...?" style
...
function renderConversationalPermission(requestId, toolName, toolInput, mateId, vendor) {
  ...
  var allowBtn = document.createElement("button");
  allowBtn.className = "perm-reply perm-allow";
  ...
  alwaysBtn.className = "perm-reply perm-always";
  ...
  var denyBtn = document.createElement("button");
  denyBtn.className = "perm-reply perm-deny";
...
// lr-255e: whether any backgrounded Task subagent is still running for the
// current session. Callers (e.g. app-messages.js's 'result' handler) use
// this to decide whether it's safe to clear the "thinking dots" indicator —
// the parent turn's own stream can end while a dispatched subagent keeps
// working, and the indicator must stay up until the LAST live subagent
// actually finishes.
export function hasActiveSubagents() {
  for (var id in activeSubagentToolIds) {
    if (activeSubagentToolIds.hasOwnProperty(id)) return true;
  }
  return false;
}
...
export function markSubagentDone(parentToolId, status, summary, usage) {
  delete activeSubagentToolIds[parentToolId];
  // lr-255e: the parent turn's 'result' already fired (and, per hasActiveSubagents(),
  // may have skipped clearing the indicator because this subagent was still live).
  // Once the last live subagent finishes, clear the indicator here too so it
  // doesn't linger until some unrelated later message happens to null it out.
