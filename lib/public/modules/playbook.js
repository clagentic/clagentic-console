...
function markCompleted(id) {
  ...
  try { localStorage.setItem("clagentic-playbooks-done", JSON.stringify(completedPlaybooks)); } catch (e) {}
}

function loadCompleted() {
  try {
    var data = JSON.parse(localStorage.getItem("clagentic-playbooks-done") || localStorage.getItem("clay-playbooks-done") || "{}");
...
function registerBuiltinPlaybooks() {
    ...
    steps: [
      {
        ...
        body: "When Claude is working on a long task, you can walk away and get a push notification the moment it's done. Works even when Clagentic: Console is in the background.",
        actions: [
        ...
        body: function () {
          ...
          if (isInsecure) {
            return "⚠️ <strong>Insecure context detected.</strong><br><br>" +
              ...
              "You're accessing Clagentic: Console over <strong>" + location.protocol + "//" + location.hostname + "</strong>, so the browser automatically blocks notifications.<br><br>" +
              "<strong>How to fix:</strong><br>" +
              "<strong>1.</strong> Access Clagentic: Console via <strong>https://</strong> (set up a TLS certificate)<br>" +
      ...
      },
      {
        id: "done",
        title: "All set! 🎉",
        body: "You'll now get a notification whenever Claude finishes processing. This works even when Clagentic: Console is minimized or in another tab.",
  ...
  if (location.protocol === "https:") {
    ...
    function certInstallBody() {
      var os = detectOS();
      var html = "The certificate has been downloaded as <strong>clagentic-ca.pem</strong>.<br><br>";

      if (os === "mac") {
        html += "<strong>macOS — run in Terminal:</strong><br>";
        html += certCodeBlock(
          "sudo security add-trusted-cert -d -r trustRoot -k /Library/Keychains/System.keychain ~/Downloads/clagentic-ca.pem"
        );
      } else if (os === "windows") {
        html += "<strong>Windows — run in PowerShell (Admin):</strong><br>";
        html += certCodeBlock(
          "certutil -addstore -f \"Root\" %USERPROFILE%\\Downloads\\clagentic-ca.pem"
        );
      } else {
        html += "<strong>Linux — run in terminal:</strong><br>";
        html += certCodeBlock(
          "sudo cp ~/Downloads/clagentic-ca.pem /usr/local/share/ca-certificates/clagentic-ca.crt && sudo update-ca-certificates"
        );
      }
      ...
      html += "<br><div style=\"padding:8px 10px;background:var(--bg-deeper);border-radius:6px;border-left:3px solid var(--accent)\">" +
        ...
        "Install ~/Downloads/clagentic-ca.pem as a trusted root certificate</code></div>";
      ...
      steps: [
        {
          ...
          body: "Clagentic: Console generates a local CA certificate for secure HTTPS connections. " +
  ...
  function fireTestNotification(cb) {
    try {
      var n = new Notification("Clagentic: Console", {
        body: "Notifications are working! 🎉",
        tag: "clagentic-test-" + Date.now(),
      });
