...
export function updateDaemonConfig(config) {
  ...
  if (memAvailEl && config.memAvailableMinMB !== undefined) {
    ...
    // A fallback default is intentionally NOT marked as last-sent: clicking
    // Save on "Using default — not yet saved" must still persist the key.
    lastSentMemAvailableMin = config.memAvailableMinMBIsDefault === true ? null : config.memAvailableMinMB;
  }
  ...
  if (tpmEl && config.tokensPerMbHeadroom !== undefined) {
    ...
    // A fallback default must remain persistable by clicking Save.
    lastSentTokensPerMbHeadroom = config.tokensPerMbHeadroomIsDefault === true ? null : config.tokensPerMbHeadroom;
  }
...
var memoryFieldStatus = {
  memAvail: { text: "", isError: false, seq: 0 },
  tpm: { text: "", isError: false, seq: 0 },
};
var memoryStatusSeq = 0;
...
function renderMemorySaveStatus() {
  ...
  var latest = memAvail.seq >= tpm.seq ? memAvail : tpm;
  var other = latest === memAvail ? tpm : memAvail;
  el.textContent = latest.text || other.text || "";
}

function setFieldSaveStatus(field, text, isError) {
  memoryFieldStatus[field] = { text: text, isError: !!isError, seq: ++memoryStatusSeq };
  renderMemorySaveStatus();
  if (!isError && text) {
    setTimeout(function () {
      if (memoryFieldStatus[field].text === text && !memoryFieldStatus[field].isError) {
        memoryFieldStatus[field] = { text: "", isError: false, seq: memoryFieldStatus[field].seq };
        renderMemorySaveStatus();
      }
    }, 2000);
  }
