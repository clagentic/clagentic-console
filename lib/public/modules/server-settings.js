...
var SETTINGS_SECTIONS = [
  ...
  { section: 'lite',           label: 'Lite',          group: 'Config',   icon: 'zap',            danger: false, conditionalId: 'settings-lite-nav' },
  ...
  { section: 'keep-awake',     label: 'Keep Awake',    group: 'Server',   icon: 'zap',            danger: false, conditionalId: 'settings-keep-awake-nav' },
...
function renderPaletteResults(query) {
  ...
  if (filtered.length === 0) {
    resultsEl.innerHTML = '<div class="settings-palette-empty">No matches for "' + escapeHtmlPalette(query) + '"</div>';
    paletteHighlight = -1;
    return;
  }
  // Group results
  ...
  for (var gi = 0; gi < groupOrder.length; gi++) {
    ...
    html += '<div class="settings-palette-group-header">' + escapeHtmlPalette(gName) + '</div>';
    ...
    for (var ii = 0; ii < items.length; ii++) {
      ...
      html += '<button class="' + cls + '" data-section="' + item.section + '" data-index="' + itemIndex + '">' +
        ...
        '<span class="settings-palette-item-label">' + escapeHtmlPalette(item.label) + '</span>' +
...
function escapeHtmlPalette(str) {
...
/**
 * Always show the Lite nav entry and section in server settings.
 * Toggle between the installed and not-installed sub-states based on liteInstalled.
 * Called from app.js when the /info response is received.
 */
export function updateSsLiteVisibility(liteInstalled) {
  var navItem = document.getElementById("settings-lite-nav");
  var section = document.getElementById("settings-lite-section");
  var installedEl = document.getElementById("settings-lite-installed");
  var notInstalledEl = document.getElementById("settings-lite-not-installed");
  // Always visible — the section is a discovery surface even when Lite is absent.
