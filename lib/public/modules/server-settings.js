...
export function initServerSettings(appCtx) {
  ...
  if (memAvailInput) {
    memAvailInput.addEventListener("change", function () {
      ...
      if (isNaN(val) || val < 0) {
        setMemorySaveStatus("Invalid value — must be 0 or greater", true);
        return;
      }
      var ws = ctx.ws;
      if (ws && ws.readyState === 1) {
        setMemorySaveStatus("Saving...", false);
        ws.send(JSON.stringify({ type: "set_mem_available_threshold", value: val }));
      } else {
        setMemorySaveStatus("Not connected — change not saved", true);
      }
    });
  }
  ...
  if (tpmInput) {
    tpmInput.addEventListener("change", function () {
      ...
      if (isNaN(val) || val < 10 || val > 500) {
        setMemorySaveStatus("Invalid value — must be 10-500", true);
        return;
      }
      var ws = ctx.ws;
      if (ws && ws.readyState === 1) {
...
function setMemorySaveStatus(text, isError) {
  var el = document.getElementById("ss-memory-save-status");
  if (!el) return;
  el.textContent = text;
  el.classList.toggle("ps-save-status-error", !!isError);
  if (!isError && text) {
    setTimeout(function () {
