...
var SETTINGS_SECTIONS = [
  ...
  { section: 'advanced-memory', label: 'Advanced Memory', group: 'Server', icon: 'cpu',           danger: false },
...
export function initServerSettings(appCtx) {
  ...
  if (memAvailInput) {
    memAvailInput.addEventListener("change", function () {
  ...
  // Advanced Memory: tokens per MB headroom input
  var tpmInput = document.getElementById("settings-tokens-per-mb-headroom");
  if (tpmInput) {
    tpmInput.addEventListener("change", function () {
      var val = parseInt(this.value, 10);
