...
export function initPanels() {
  ...
  // --- Reactive UI sync ---
  store.subscribe(['availableVendors', 'installedVendors', 'currentVendor', 'richContextUsage', 'ctxPopoverVisible', 'currentModel', 'currentMode', 'currentEffort', 'currentBetas', 'currentThinking'], function (state, prev) {
    // Vendor toggle state
...
export function resolveContextWindow(model, sdkValue) {
  // If the 1M context beta is active, the real context window is always 1M.
  // The SDK reports the model's base window (200K) in both ModelUsage.contextWindow
  // and getContextUsage().maxTokens — it does not reflect the beta-extended limit.
  // Check active betas first so we override the stale SDK value.
  if (hasBeta("context-1m")) return 1000000;
