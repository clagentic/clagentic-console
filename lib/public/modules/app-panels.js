...
var contextData = { contextWindow: 0, maxOutputTokens: 0, model: "-", cost: 0, input: 0, output: 0, cacheRead: 0, cacheWrite: 0, reasoningOutput: 0, turns: 0 };
...
function buildConfigModelItem(item, currentModel) {
  ...
  // lr-d3817f: retired entries are filtered out upstream (model-catalog.js's
  // filterSelectableCatalogModels) and never reach this function — no
  // disabled/retired-badge rendering here anymore. Deprecated (still-
  // runnable) entries still get a marker.
  var isDeprecated = !!(item && typeof item === "object" && item.isDeprecated);
  var btn = document.createElement("button");
  btn.className = "config-radio-item";
  if (value === currentModel) btn.classList.add("active");
  btn.dataset.model = value;
  btn.textContent = label;
  // lr-f22787: same non-blocking "Catalog" marker as the settings picker
  // (settings-defaults.js buildModelItem) for entries sourced from the
  // release-time-generated catalog rather than the vendor's live list.
  if (item && typeof item === "object" && item.fromCatalog) {
    var catalogBadge = document.createElement("span");
    catalogBadge.className = "config-model-catalog-badge";
    catalogBadge.textContent = " (Catalog)";
    btn.appendChild(catalogBadge);
  }
  if (isDeprecated) {
    var deprecatedBadge = document.createElement("span");
    deprecatedBadge.className = "config-model-deprecated-badge";
    deprecatedBadge.textContent = " (Deprecated)";
    btn.appendChild(deprecatedBadge);
  }
...
export function initPanels() {
  ...
  contextReasoningEl = $("context-reasoning");
...
export function accumulateUsage(cost, usage, lastStreamInputTokens) {
  ...
  // Prefer lastStreamInputTokens (accurate per-call); fall back to input+cache_read+cache_creation
  // (lr-3af675: the three usage fields are disjoint and all occupy the window).
  if (lastStreamInputTokens) {
    ...
    var cacheCreate = usage.cache_creation_input_tokens || usage.cacheCreationInputTokens || 0;
    if (directIn + cacheIn + cacheCreate > 0) sessionUsage.context = directIn + cacheIn + cacheCreate;
  }
  sessionUsage.turns++;
...
/**
 * lr-3af675: vendor-first context-window resolution. The hardcoded
 * model-name -> window table (lib/model-context-windows.js) is gone — both
 * providers report their own context window, and that report is the only
 * value this function ever returns.
 *
 * vendorWindow: the window size as reported by the vendor for the CURRENT
 *   turn/session (Claude: modelUsage.contextWindow or getContextUsage()'s
 *   maxTokens; Codex: modelUsage.contextWindow, sourced from
 *   ThreadTokenUsage.modelContextWindow on thread/tokenUsage/updated —
 *   lr-872f94 — absent until the app-server reports one for the session).
 *
 * Returns the vendor-reported window, or 0 when none is available (no
 * completed turn yet, a resumed session before its first result, a daemon
 * restart, or a vendor that hasn't reported one). Callers MUST treat 0 as
 * "unknown" and render an indeterminate state — never substitute a guess.
 */
export function resolveContextWindow(vendorWindow) {
  return (typeof vendorWindow === "number" && vendorWindow > 0) ? vendorWindow : 0;
}
...
/**
 * lr-3af675: the effective (used, window) pair the meter renders from.
 * Prefers the rich context-usage breakdown (store.richContextUsage) when
 * present — it is the freshest vendor read, including immediately after a
 * compaction re-read (see the 'status' handler re-fetch in
 * sdk-message-processor.js) — and falls back to the per-turn contextData
 * accumulated from 'result' events. Both sources are vendor-first; neither
 * ever substitutes a guessed window.
 *
 * Exported so every consumer of the meter (header bar, popover, panel, and
 * the tools.js clear-context label) reads the exact same (used, win) pair
 * instead of each recomputing its own view and risking disagreement.
 */
export function getEffectiveContextFill() {
  var rich = store.get('richContextUsage');
  if (rich && typeof rich.totalTokens === "number") {
    var richWin = resolveContextWindow(rich.maxTokens);
    if (richWin > 0) return { used: rich.totalTokens, win: richWin };
  }
  return { used: contextData.input, win: resolveContextWindow(contextData.contextWindow) };
}
...
export function updateContextPanel() {
  ...
  // lr-3af675: unknown window -> "-", never a fabricated "0%".
  contextBarPct.textContent = win > 0 ? pct.toFixed(0) + "%" : "-";
  // Mini bar
  if (contextMiniFill) {
  ...
  // lr-872f94: reasoning tokens are a real Codex-specific spend category
  // (thread/tokenUsage/updated's reasoningOutputTokens) that was previously
  // invisible to users. Row is hidden (not just "-") when a vendor hasn't
  // reported any, so Claude sessions don't show a permanently-zero row.
  if (contextReasoningEl) {
    var reasoningRow = contextReasoningEl.closest(".usage-row");
    if (contextData.reasoningOutput > 0) {
      contextReasoningEl.textContent = formatTokens(contextData.reasoningOutput);
      if (reasoningRow) reasoningRow.classList.remove("hidden");
    } else if (reasoningRow) {
...
export function accumulateContext(cost, usage, modelUsage, lastStreamInputTokens) {
  ...
  if (usage) {
    ...
    if (lastStreamInputTokens) {
      ...
      // lr-3af675: input_tokens, cache_read_input_tokens, and
      // cache_creation_input_tokens are disjoint and all three occupy the
      // context window -- cache_creation_input_tokens was previously
      // omitted here, under-reporting fill on cache-write-heavy turns.
    ...
    // lr-872f94: Codex-specific reasoning token spend (previously discarded).
    // No cumulative concept here, matching the other per-turn fields above.
    contextData.reasoningOutput = usage.reasoning_output_tokens || usage.reasoningOutputTokens || 0;
  }
  contextData.turns++;
  if (modelUsage) {
    ...
    if (models.length > 0) {
      ...
      // lr-3af675: vendor-first, no hardcoded table. mu.contextWindow is the
      // vendor's own reported window for this model; resolveContextWindow no
      // longer does anything but validate it's a positive number. Absent a
      // vendor value, contextWindow stays 0 ("unknown") -- render blank, not
      // a guess.
      contextData.contextWindow = resolveContextWindow(mu.contextWindow);
...
export function resetContextData() {
  contextData = { contextWindow: 0, maxOutputTokens: 0, model: "-", cost: 0, input: 0, output: 0, cacheRead: 0, cacheWrite: 0, reasoningOutput: 0, turns: 0 };
...
export function renderCtxPopover() {
  ...
  // lr-3af675: vendor-first -- d.maxTokens is the SDK's own reported window
  // (already reflects any active beta, e.g. context-1m) and needs no local
  // override. resolveContextWindow now only validates it's a positive number.
  var max = resolveContextWindow(d.maxTokens);
  ...
  // lr-3af675: unknown window -> "-", never a fabricated "0%"/0 denominator.
  html += '<span class="ctx-pop-pct">' + (max > 0 ? pct.toFixed(0) + '%' : '-');
  html += '<span class="ctx-pop-tokens">' + formatTokens(total) + ' / ' + (max > 0 ? formatTokens(max) : '-') + '</span>';
  html += '</span>';
  html += '</div>';
