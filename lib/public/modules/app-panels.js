...
/**
 * Build the "Use a specific version" free-text model ID form for the
 * session-chip picker. Mirrors buildCustomModelForm in settings-defaults.js
 * (project/server settings pickers) — see that function's doc comment for
 * why a free-text escape hatch is required rather than optional: the
 * vendor's model-enumeration API structurally cannot report prior versions,
 * but setModel()/the set_model message accept any string with no
 * client-side allowlist, which is the exact mechanism the CLI's own /model
 * command relies on to switch to an older version (lr-f22787).
 */
function buildConfigCustomModelForm(currentModel) {
  var wrap = document.createElement("div");
  wrap.className = "config-custom-model-row";

  var input = document.createElement("input");
  input.type = "text";
  input.className = "config-budget-input config-custom-model-input";
  ...
  var btn = document.createElement("button");
  btn.type = "button";
  btn.className = "config-radio-item config-custom-model-btn";
  btn.textContent = "Use";
...
function rebuildModelList() {
  ...
  // lr-f22787: always offer the free-text escape hatch — see
  // buildConfigCustomModelForm's doc comment.
  var customToggle = document.createElement("button");
  customToggle.type = "button";
  customToggle.className = "config-older-models-toggle";
  ...
  customBody.className = "config-older-models-body hidden";
  customBody.appendChild(buildConfigCustomModelForm(s.currentModel));
  ...
  configModelList.appendChild(customToggle);
  configModelList.appendChild(customBody);
