...
function buildConfigModelItem(item, currentModel) {
  ...
  // lr-f22787 (coordinator follow-up): same disabled-not-hidden behavior as
  // the settings picker (settings-defaults.js buildModelItem) — see
  // model-catalog.js's header comment for the full decided-behavior
  // rationale.
  var isRetired = !!(item && typeof item === "object" && item.isRetired);
  var isDeprecated = !!(item && typeof item === "object" && item.isDeprecated);
  var btn = document.createElement("button");
  btn.className = "config-radio-item" + (isRetired ? " config-radio-item-disabled" : "");
  ...
  if (isRetired) {
    ...
    retiredBadge.className = "config-model-retired-badge";
    retiredBadge.textContent = " (Retired)";
    btn.appendChild(retiredBadge);
  ...
  btn.addEventListener("click", function () {
    // lr-f22787: disabled=true on the <button> already prevents most click
    // paths, but this guard makes the no-op explicit and independent of
    // that DOM property (defense-in-depth, matches the settings-picker
    // guard in buildModelItem).
    if (isRetired) return;
