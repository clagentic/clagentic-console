...
import { escapeHtml, copyToClipboard, relativeTime } from './utils.js';
...
var pendingFileAt = null;    // { hash, cb } for the in-flight fs_file_at request, or null
...
var pendingOpenMode = null; // { path, type: "diff", oldStr, newStr } or null
var pendingReadPath = null; // path of the in-flight fs_read request, for correlating the reply

export function openFile(filePath, opts) {
  if (!filePath) return;
  if (opts && opts.diff) {
    pendingOpenMode = { path: filePath, type: "diff", oldStr: opts.diff.oldStr, newStr: opts.diff.newStr };
  } else {
    pendingOpenMode = null;
  }
...
export function handleFsRead(msg) {
  // Correlate against the in-flight request — a reply for a path we're no
  // longer waiting on (e.g. the user opened a different file, or the
  // request raced a fs_watch-triggered handleFileChanged) must be dropped
  // rather than silently applied to whatever is currently open.
  if (msg.path !== pendingReadPath) return;
  pendingReadPath = null;
  showFileContent(msg);
}
...
function showFileContent(msg) {
  ...
  // Guard against a stale pendingOpenMode meant for a different path (e.g.
  // the diff-open request was superseded by a plain openFile() before the
  // fs_read reply arrived).
  if (pendingOpenMode && pendingOpenMode.path === msg.path && pendingOpenMode.type === "diff" && currentContent != null) {
...
export function handleFileHistory(msg) {
  // Drop replies for a file that is no longer open — otherwise a slow
  // fs_file_history_result for the previous file can land after the user
  // has already switched to a different one and get applied to it.
  if (msg.path !== currentFilePath) return;
...
export function handleFileAt(msg) {
  ...
  // Only resolve the pending callback if this reply matches the hash it is
  // waiting on — otherwise a reply for an earlier, superseded request (e.g.
  // the user picked a different compare entry before the first fs_file_at
  // came back) would resolve the wrong callback with the wrong content.
  if (pendingFileAt && pendingFileAt.hash === msg.hash) {
    var cb = pendingFileAt.cb;
    pendingFileAt = null;
    cb(msg);
  }
}
...
function renderHistoryPanel() {
  ...
  for (var i = 0; i < currentHistoryEntries.length; i++) {
    ...
    if (item.source === "git") {
      ...
      sub.textContent = item.hash.substring(0, 7) + " by " + (item.author || "unknown") + (item.timestamp ? ", " + relativeTime(item.timestamp) : "");
...
// Registers this module's own result/event handlers directly with the
// app-messages.js registry instead of app-messages.js importing and
// switch-casing them itself. Runs once at module load — app.js imports
...
// arrive (see lib/public/app.js).
// fs_read_result is NOT registered here even though handleFsRead lives in
// this module: the wire message is shared with the CLAUDE.md instructions
// editor in project-settings.js (msg.path === "CLAUDE.md" &&
// isProjectSettingsOpen() routes there instead). That routing decision
// needs both modules and stays a core case in app-messages.js.
registerHandlers({
  fs_list_result: handleFsList,
  fs_search_result: handleFsSearch,
  fs_file_changed: handleFileChanged,
  fs_dir_changed: handleDirChanged,
  fs_file_history_result: handleFileHistory,
  fs_git_diff_result: handleGitDiff,
  fs_file_at_result: handleFileAt,
});
