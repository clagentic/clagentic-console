...
import { fetchCustomIconList, uploadCustomIcon, deleteCustomIcon, slugifyFilename, matchesCustomIconQuery } from './custom-icons.js';
...
var filterQuery = ""; // lr-0847: current text in #custom-icons-filter-input
...
function renderList() {
  ...
  // lr-0847: filter by hyphen/underscore-segment match (shared with the
  // emoji-picker Custom tab search via matchesCustomIconQuery) so this list
  // and the picker never drift on tokenization the way upload error
  // handling drifted between the two surfaces.
  var visible = [];
  for (var fi = 0; fi < cachedList.length; fi++) {
    if (matchesCustomIconQuery(cachedList[fi].slug, filterQuery)) visible.push(cachedList[fi]);
  }

  if (visible.length === 0) {
    var noMatch = document.createElement("div");
    noMatch.className = "settings-hint";
    noMatch.textContent = "No custom icons match \"" + filterQuery + "\".";
  ...
  for (var i = 0; i < visible.length; i++) {
    list.appendChild(buildRow(visible[i], usage));
  }
...
/**
 * Wire the Upload button + hidden file input, and the slug filter box
 * (lr-0847). Called once from server-settings.js's initServerSettings
 * (idempotent — safe if elements are absent, e.g. in tests without the full
 * DOM).
 */
export function initCustomIconsUpload() {
  ...
  var filterInput = document.getElementById("custom-icons-filter-input");
  if (filterInput) {
