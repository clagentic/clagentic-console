...
// defaultHidden: true — tool starts in the hidden section for new users.
// Users who have saved preferences keep their own state.
var SESSION_TOOLS = [
  ...
  { id: "import-cli-btn",          icon: "import",         label: "Import CLI" },
  { id: "scheduler-btn",           icon: "calendar-clock", label: "Scheduled Tasks", defaultHidden: true },
  { id: "loop-tool-btn",           icon: "repeat",         label: "Loop",            defaultHidden: true },
  { id: "mcp-btn",                 icon: "cable",          label: "MCP Servers",     countId: "mcp-sidebar-count", defaultHidden: true },
  { id: "skills-btn",              icon: "puzzle",         label: "Skills",          defaultHidden: true },
];
...
function buildPalette(name) {
  ...
  // Scaffold the hidden section before building buttons so defaultHidden
  // tools have somewhere to go.
  ...
  if (hiddenSection) {
    ...
    hiddenGrid.className = 'tool-palette-hidden-grid';
    hiddenSection.appendChild(labelEl);
    hiddenSection.appendChild(hiddenGrid);
  }

  // Create buttons with stable IDs. defaultHidden tools go into the
  // hidden grid; others go into the active grid. applyPreferences()
  // will rearrange for users who have saved their own layout.
  for (var i = 0; i < palette.tools.length; i++) {
    var tool = palette.tools[i];
    var btn = buildToolButton(tool, name);
    if (tool.defaultHidden && hiddenGrid) {
      btn.draggable = false;
...
document.addEventListener('keydown', function (e) {
  ...
  // file-picker dialog the app never wants, so taking it over costs the
