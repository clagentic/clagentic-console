...
export function buildEmojiPicker(opts) {
  ...
  function buildCustomPanel() {
    ...
    function doUpload() {
      ...
      function proceed() {
        // Capture pendingFile before hideSlugConfirm() clears it (lr-6c9930)
        // — hideSlugConfirm() nulls pendingFile, so reading it after that
        // call always passes null into uploadCustomIcon(), which then fails
        // silently (see custom-icons.js hardening below).
