...
import { getWs, sendWsQuiet } from './ws-ref.js';
import { store } from './store.js';
import { getMessagesEl } from './dom-refs.js';
import { registerTooltip } from './tooltip.js';
import { getSessionPresenceUsers } from './sidebar-sessions.js';
...
function hasOtherSessionViewers() {
  var sessionId = store.get('activeSessionId');
  if (!sessionId) return false;
  var viewers = getSessionPresenceUsers(sessionId);
  if (!viewers || viewers.length === 0) return false;
  var myUserId = store.get('myUserId');
  for (var i = 0; i < viewers.length; i++) {
    if (!myUserId || viewers[i].id !== myUserId) return true;
  }
  return false;
}

function shouldBroadcastCollab() {
  return cursorSharingEnabled && hasOtherSessionViewers();
}

// Cursor sharing toggle button in user island
export function initCursorToggle() {
  ...
  btn.addEventListener("click", function () {
    ...
    updateToggleStyle();
    if (!cursorSharingEnabled) {
      sendWsQuiet({ type: "cursor_leave" });
      sendWsQuiet({ type: "text_select", ranges: [] });
    }
  });
}
...
export function initCursors() {
  ...
  document.addEventListener("selectionchange", function () {
    ...
    if (activeEl && activeEl.id === "input" && lastSelectionKey === "") return;
    ...
    sendWsQuiet({ type: "text_select", ranges: ranges });
  });
}
