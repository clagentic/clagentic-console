...
export function uploadCustomIcon(slug, file) {
  return new Promise(function (resolve) {
    // Defense-in-depth (lr-6c9930): FileReader.readAsArrayBuffer(null/undefined)
    // throws synchronously inside the Promise executor rather than firing
    // onerror, so a caller bug that passes a missing file previously left
    // this Promise permanently unresolved — no .then(), no failure toast,
    // upload looked like it did nothing. Reject the bad input explicitly
    // instead of relying on FileReader to fail safely.
    if (!file) {
...
/**
 * Test whether a custom-icon slug matches a search query (lr-0847).
 *
 * Matches if the query is a substring of the whole slug OR a substring of
 * any hyphen/underscore-delimited segment. Whole-slug substring match alone
 * already covers segment prefixes (e.g. "clage" -> "clagentic-console"),
 * but NOT a later segment on its own (e.g. "console" was previously unable
 * to match "clagentic-console" since custom icons had no search wiring at
 * all — this is the shared match rule for both the emoji-picker Custom tab
 * search and the Server Settings > Custom Icons list filter, so the two
 * surfaces can't drift on tokenization the way they drifted on upload
 * error handling).
 *
 * @param {string} slug
 * @param {string} query
 * @returns {boolean}
 */
export function matchesCustomIconQuery(slug, query) {
  ...
  for (var i = 0; i < segments.length; i++) {
    if (segments[i].indexOf(q) !== -1) return true;
  }
  return false;
}
