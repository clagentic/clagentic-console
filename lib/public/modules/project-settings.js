...
var PS_SECTIONS = [
  ...
  { section: "lite",         label: "Lite",         group: "Config",  icon: "zap", conditionalId: "ps-nav-lite" },
];
...
function renderPsPaletteResults(query) {
  ...
  for (var i = 0; i < PS_SECTIONS.length; i++) {
    ...
    if (s.conditionalId) {
      var condEl = document.getElementById(s.conditionalId);
      if (!condEl || condEl.classList.contains("hidden")) continue;
    }
  ...
  for (var j = 0; j < visible.length; j++) {
    ...
    if (sec.group !== lastGroup) {
      html += '<div class="settings-palette-group-header">' + escapeHtml(sec.group) + '</div>';
      lastGroup = sec.group;
    }
    ...
    html += '<button class="settings-palette-item' +
      ...
      '" data-section="' + escapeHtml(sec.section) + '">' +
      '<span class="settings-palette-item-icon"><i data-lucide="' + escapeHtml(sec.icon) + '"></i></span>' +
      '<span class="settings-palette-item-label">' + escapeHtml(sec.label) + '</span>' +
...
function updateIconPreview(icon) {
  ...
  if (preview) {
    renderProjectIcon(icon || null, preview, parseEmojis);
  }
  if (removeBtn) {
...
// ===== Clagentic: Lite section =====

/**
 * Show or hide the Lite nav entry and section based on whether Lite is installed.
 * Called from app.js when the /info response is received.
 */
export function updateLiteVisibility(liteInstalled) {
  var navItem = document.getElementById("ps-nav-lite");
  var section = document.getElementById("ps-section-lite");
  if (navItem) navItem.classList.toggle("hidden", !liteInstalled);
  if (section) section.classList.toggle("hidden", !liteInstalled);
}

function loadLiteSection() {
  if (ctx.ws && ctx.connected) {
    ctx.ws.send(JSON.stringify({ type: "get_lite_project_status" }));
  }
}

export function handleLiteProjectStatus(msg) {
  var statusEl = document.getElementById("ps-lite-status-text");
  var enrollBtn = document.getElementById("ps-lite-enroll-btn");
  var unenrollBtn = document.getElementById("ps-lite-unenroll-btn");
  var errorEl = document.getElementById("ps-lite-error");

  if (statusEl) statusEl.textContent = msg.enrolled ? "Enrolled" : "Not enrolled";
  if (enrollBtn) enrollBtn.classList.toggle("hidden", !!msg.enrolled);
  if (unenrollBtn) unenrollBtn.classList.toggle("hidden", !msg.enrolled);
  if (errorEl) errorEl.textContent = "";
}

export function handleLiteEnrollResult(msg) {
  var errorEl = document.getElementById("ps-lite-error");
  if (!msg.ok) {
    if (errorEl) errorEl.textContent = msg.error || "Enroll failed";
    showToast("Lite enroll failed: " + (msg.error || "unknown error"));
  }
}

export function handleLiteUnenrollResult(msg) {
  var errorEl = document.getElementById("ps-lite-error");
  if (!msg.ok) {
    if (errorEl) errorEl.textContent = msg.error || "Unenroll failed";
    showToast("Lite unenroll failed: " + (msg.error || "unknown error"));
  }
}

export function initLiteButtons() {
  var enrollBtn = document.getElementById("ps-lite-enroll-btn");
  var unenrollBtn = document.getElementById("ps-lite-unenroll-btn");
  ...
  if (enrollBtn) {
    enrollBtn.addEventListener("click", function () {
      if (ctx.ws && ctx.connected) {
        ctx.ws.send(JSON.stringify({ type: "lite_enroll_project" }));
      }
    });
  }
  ...
  if (unenrollBtn) {
    unenrollBtn.addEventListener("click", function () {
      if (ctx.ws && ctx.connected) {
        ctx.ws.send(JSON.stringify({ type: "lite_unenroll_project" }));
      }
    });
  }
}
