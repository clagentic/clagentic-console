import { avatarUrl, userAvatarUrl } from './modules/avatar.js';
...
import {
  ...
  updateSessionBadge, populateCliSessionList, resetSessionList
...
import { openMobileSheet, updateMobileProjectsTabBadge } from './modules/sidebar-mobile.js';
...
import { initFileBrowser, loadRootDirectory, refreshTree, handleFsList, handleFsRead, handleDirChanged, refreshIfOpen, handleFileChanged, handleFileHistory, handleGitDiff, handleFileAt, closeFileViewer, resetFileBrowser } from './modules/filebrowser.js';
import { getPendingNavigate } from './modules/pending-navigate.js';
...
import { initServerSettings, updateSettingsStats, updateSettingsModels, updateDaemonConfig, handleSetPinResult, handleKeepAwakeChanged, handleAutoContinueChanged, handleRestartResult, handleShutdownResult, handleSharedEnv, handleSharedEnvSaved, handleGlobalClaudeMdRead, handleGlobalClaudeMdWrite, isSettingsOpen, closeSettings, updateSsLiteVisibility, initSsLiteToggle } from './modules/server-settings.js';
import { initProjectSettings, handleInstructionsRead, handleInstructionsWrite, handleProjectEnv, handleProjectEnvSaved, isProjectSettingsOpen, closeProjectSettings, handleProjectSharedEnv, handleProjectSharedEnvSaved, handleProjectOwnerChanged, updateLiteVisibility, initLiteButtons, handleLiteProjectStatus, handleLiteEnrollResult, handleLiteUnenrollResult } from './modules/project-settings.js';
...
import { initConsoleLogo, startLogoAnimation, stopLogoAnimation } from './modules/console-logo.js';
...
import { initCommandPalette, handlePaletteSessionSwitch, setPaletteVersion, isCommandPaletteOpen, closeCommandPalette } from './modules/command-palette.js';
import { consumeOverlayPopstate, hasOverlayState } from './modules/overlay-history.js';
import { initAgentPicker } from './modules/agent-picker.js';
...
// --- DOM refs ---
  ...
  // cachedDmConversations, dmRemovedUsers, dmTargetUser, dmKey -> store
  var dmMessageCache = [];
  ...
  if (projectHintDismiss) {
    projectHintDismiss.addEventListener("click", function () {
      ...
      try { localStorage.setItem("clagentic-project-hint-dismissed", "1"); } catch (e) {}
    });
  }
  ...
  // loopActive, loopAvailable, loopIteration, loopMaxIterations, loopBannerName, loopCurrentSessionId,
  ...
  try {
    var _cachedProjectName = localStorage.getItem("clagentic-project-name-" + (currentSlug || "default")) || localStorage.getItem("clay-project-name-" + (currentSlug || "default"));
    ...
    var _cachedProjectIcon = localStorage.getItem("clagentic-project-icon-" + (currentSlug || "default")) || localStorage.getItem("clay-project-icon-" + (currentSlug || "default"));
  ...
  // Agent picker (lr-7db0). No global hotkey — opened from the project
  // session-list "Start agent chat" button.
  initAgentPicker();
  ...
  // --- Connect overlay (animated console logo) ---
  var consoleLogoSvg = $("console-logo-svg");
  initConsoleLogo(consoleLogoSvg);
  startLogoAnimation();
  ...
  messagesEl.addEventListener("scroll", function () {
    // While sticky-bottom is armed, suppress "user scrolled up" detection.
    if (getStickyBottom()) return;
    // Coalesce rapid scroll events into one rAF callback so we don't force
    // a layout read (scrollHeight/scrollTop/clientHeight) on every event.
  ...
  initTools({
    ...
    getContextPercent: function() {
      // lr-3af675: route through the same (used, win) pair the header bar
      // and popover render from, so this label can never disagree with them.
      var fill = _panGetEffectiveContextFill();
      return fill.win > 0 ? Math.round((fill.used / fill.win) * 100) : 0;
    },
  ...
  window.addEventListener("popstate", function () {
    // Mobile swipe-back / browser back: if an overlay pushed a sentinel history
    // entry, close that overlay and consume the event — do not navigate.
    if (hasOverlayState()) {
      consumeOverlayPopstate();
      // Close whichever overlay is open (priority order: most-modal first).
      if (isCommandPaletteOpen()) { closeCommandPalette(); return; }
      if (isSettingsOpen()) { closeSettings(); return; }
      if (isProjectSettingsOpen()) { closeProjectSettings(); return; }
      if (isSearchOpen()) { closeSearch(); return; }
      if (isArchiveOpen()) { closeArchive(); return; }
      if (isSchedulerOpen()) { closeScheduler(); return; }
      return;
    }
    ...
    if (newSlug && newSlug !== store.get('currentSlug')) {
      ...
      // Clear session list before connecting so the sidebar doesn't show
      // the previous project's sessions. Must be called here (not inside
      // resetClientState) because resetClientState is also called from
      // session_switched where the list must be preserved.
      resetSessionList();
      resetClientState();
      connect();
    }
  ...
  initInput({
    ...
    disarmStickyBottom: disarmStickyBottom,
    setSendBtnMode: setSendBtnMode,
    isDmMode: function () { return store.get('dmMode'); },
    ...
    myDisplayName: function () {
      var u = null;
      try { u = JSON.parse(localStorage.getItem("clagentic_my_user") || localStorage.getItem("clay_my_user") || "null"); } catch (e) {}
  ...
  // --- Projects module ---
  initProjects();
  onProjectListUpdated(function (projects) { updateMobileProjectsTabBadge(projects); });
  onResetClientState(function () { resetSessionList(); });
  ...
  // lr-5edd64: the lr-58c813 client-side activity-edge accept/reject ledger
  // (window.__clagenticActivityLedger(), activity-latch.js) is removed along
  // with the client-local 'processing' latch it existed to observe —
  // 'processing' is a pure projection of server-authoritative per-session
  // state now (see app-messages.js's session_list/session_switched
  // handlers), so there is no edge-accept/reject decision left to ledger.
  // The SEPARATE server-side lr-58c813 divergence probe (lib/sdk-bridge.js,
  // session.isProcessing vs the token registry) is untouched by this
  // removal — it is reachable via `clagentic-console --activity-diagnostics`
  // (lr-8b476f), not this devtools hook.
