...
function attachNotifications(ctx) {
  ...
  var isMultiUser = ctx.isMultiUser || function () { return false; };
  ...
  function startReminder() {
    ...
    reminderTimer = setInterval(function () {
      // In multi-user mode, skip the broadcast reminder push.
      // The body (unread count) is per-project state that would leak to all
      // push subscribers regardless of which user owns the notifications.
      if (isMultiUser()) return;
      var count = getUnreadCount();
      if (count > 0 && pushModule) {
        pushModule.sendPush({
          type: "reminder",
          title: "Clagentic:Console",
