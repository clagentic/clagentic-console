...
function attachConnection(ctx) {
  ...
  function handleConnection(ws, wsUser, handleMessage, handleDisconnection) {
    ...
    // Send cached state
    var _userId = ws._clagenticUser ? ws._clagenticUser.id : null;
    ...
    // lr-041af8: hydrate from the session this client is about to restore
    // into (restoredActive, resolved above), not the shared project/global
    // default. Every reconnect/page-load previously hydrated the chip from
    // sm.currentModel regardless of which model the restored session had
    // actually persisted (sessions.js's buildMetaLine/loadSessions round-trip
    // it correctly to disk — only this display path never consulted it) —
    // the root cause of "picked opus 4.7, session ran on it, chip still says
    // sonnet after a reconnect." See effectiveSessionModel() in sessions.js.
    var _initialModel = sm.effectiveSessionModel(restoredActive);
    if (_initialModel) {
      sendTo(ws, { type: "model_info", model: _initialModel, models: initialModels, vendor: initialVendor, availableVendors: sm.availableVendors || [], installedVendors: sm.installedVendors || [] });
    }
    sendTo(ws, { type: "config_state", model: _initialModel, mode: sm.currentPermissionMode || "default", effort: sm.currentEffort || "medium", betas: sm.currentBetas || [], thinking: sm.currentThinking || "adaptive", thinkingBudget: sm.currentThinkingBudget || 10000 });
    ...
    if (active && !autoCreated) {
      ws._clagenticActiveSession = active.localId;
      ...
      // lr-f36626: console-driven AUTO-RESUME for a session that was force-
      // evicted under memory pressure while it had a live background child
      // (see lib/memory-shed.js's liveChildReclaims handling). The eviction
      // itself never killed queryInstance — the child kept running — so
      // "resume" here means: this is the first console reconnect since the
      // reclaim, the durable checkpoint (the on-disk session file, already
      // reloaded by loadSessionHistory above) is what the operator sees, and
      // the flag is cleared so this fires exactly once, not on every
      // subsequent reconnect to the same session.
      if (active.pendingAutoResume) {
        active.pendingAutoResume = false;
        sm.saveSessionFile(active);
      }
      // lr-2ea2a7: hasHistory uses the ABSOLUTE count (_historyBaseIndex +
      // heap length), not the raw heap length — with a bounded tail-loaded
      // session the heap length alone is still correct here (a session with
      // ANY persisted history always has a non-empty tail once loaded), but
      // the absolute form is the more defensible invariant to depend on and
      // matches how every other index/count in the wire contract is derived
      // post-lr-2ea2a7.
      var _hasHistory = ((active._historyBaseIndex || 0) + active.history.length) > 0;
  ...
  function handleDisconnection(ws) {
    if (ws._clagenticActiveSession) {
      var dcPresKey = ws._clagenticUser ? ws._clagenticUser.id : "_default";
      var dcExisting = userPresence.getPresence(slug, dcPresKey);
      userPresence.setPresence(slug, dcPresKey, ws._clagenticActiveSession, dcExisting ? dcExisting.mateDm : null);
    }
    tm.detachAll(ws);
    clients.delete(ws);
    if (clients.size === 0) {
