...
function createSDKBridge(opts) {
  ...
  async function startQuery(session, text, images, linuxUser) {
    ...
    // lr-2d91: cgroup context token guard — applies to ALL turns because long-running
    ...
    if (contextTokens > 0) {
      ...
      if (cgHeadroomMB !== null) {
        ...
        var _getConfig = opts.getConfig;
        if (typeof _getConfig === "function") {
          var _cgCfg = _getConfig();
          if (_cgCfg && typeof _cgCfg.tokensPerMbHeadroom === "number" && _cgCfg.tokensPerMbHeadroom > 0) {
            tokensPerMb = _cgCfg.tokensPerMbHeadroom;
          }
        }
        var allowedTokens = cgHeadroomMB * tokensPerMb;
        var warnTokens = Math.floor(allowedTokens * 0.8);
        if (contextTokens >= allowedTokens) {
          ...
          sm.broadcastSessionList();
          return;
        } else if (contextTokens >= warnTokens) {
    ...
    if (session.agentName && sessionAdapter.vendor !== 'codex') {
      ...
      // lr-5bd7 — inject AGENT.md body as system prompt via the agents map.
      // The SDK's `agent` option alone only discovers the agent listing; it does
      // not deliver the body to the model. Passing claudeOpts.agents with the
      // body ensures the model actually receives the agent's persona/instructions.
      ...
      if (agentRaw) {
        ...
        if (agentBody) {
          var agentEntry = {};
          if (agentParsed.meta && agentParsed.meta.description) {
            agentEntry.description = agentParsed.meta.description;
          }
          agentEntry.prompt = agentBody;
          if (agentTools) {
            agentEntry.tools = agentTools;
          }
          claudeOpts.agents = {};
          claudeOpts.agents[session.agentName] = agentEntry;
          console.log("[sdk-bridge] agent body injected for " + session.agentName + " (" + agentBody.length + " chars)");
        }
      }
    }
...
module.exports = { createSDKBridge, createMessageQueue, readMemAvailableMB, readCgroupHeadroomMB, DEFAULT_MEM_AVAILABLE_MIN_MB, DEFAULT_TOKENS_PER_MB_HEADROOM };
