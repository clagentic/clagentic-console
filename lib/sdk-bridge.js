  ...
  async function processQueryStream(session) {
    ...
    try {
      ...
      if (session.isProcessing && session.taskStopRequested) {
        session.isProcessing = false;
        onProcessingChanged();
        // lr-5450ef: an interrupted turn can leave a Task activity token
        // acquired (block_stop, sdk-message-processor.js) with no later
      ...
      if (session.isProcessing) {
        ...
        // branch above \u2014 an aborted/errored stream can also leave a Task
        // token acquired with no terminal event left to release it.
        sessionActivity.bumpGeneration(session);
