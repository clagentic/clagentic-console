...
// Known context windows for models the backend may encounter (lr-1f7e).
// Mirrors KNOWN_CONTEXT_WINDOWS in app-panels.js — keep in sync when new models are added.
// The guard degrades cleanly (skips window-based warn) when a model is not found here.
var BACKEND_KNOWN_CONTEXT_WINDOWS = {
  "opus-4-6": 1000000,
...
// Resolve the context window for a given model identifier.
// activeBetas is an optional array of active beta strings (sm.currentBetas).
// Returns the window size in tokens, or 0 when unknown.
// Unknown → caller skips the window-based warn (degrade cleanly).
//
// Mirrors app-panels.js resolveContextWindow() logic exactly:
//   1. If the context-1m beta is active, return 1M regardless of model.
//      The SDK reports the base window (200K) in ModelUsage — the beta overrides it.
//   2. If the model name contains "[1m]" (UI beta-toggle suffix), return 1M.
//   3. Look up the model in BACKEND_KNOWN_CONTEXT_WINDOWS.
//   4. Return 0 (unknown) so the caller skips the window-based warn cleanly.
function resolveModelContextWindow(model, activeBetas) {
  ...
  var keys = Object.keys(BACKEND_KNOWN_CONTEXT_WINDOWS);
  for (var i = 0; i < keys.length; i++) {
    if (lc.indexOf(keys[i]) !== -1) return BACKEND_KNOWN_CONTEXT_WINDOWS[keys[i]];
  }
  return 0;
}
  ...
  async function startQuery(session, text, images, linuxUser) {
    ...
    var queryOpts = {
      ...
      adapterOptions: {
        CLAUDE: claudeOpts,
        CODEX: {
          // Always use "never" (auto-approve) because Clagentic: Console handles tool
          ...
          // relayed through the web UI, causing MCP tool calls to hang.
...
module.exports = { createSDKBridge, createMessageQueue, readMemAvailableMB, readCgroupHeadroomMB, resolveModelContextWindow, DEFAULT_MEM_AVAILABLE_MIN_MB, DEFAULT_TOKENS_PER_MB_HEADROOM };
