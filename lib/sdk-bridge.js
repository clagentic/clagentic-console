    ...
    if (adapter) {
      try {
        ...
        if (result.slashCommands) {
          // result.slashCommands may contain enriched {name,desc,type} objects
          // (e.g. Codex adapter) or plain strings (legacy). Deduplicate by name,
          // preferring enriched entries over plain-string fallbacks. Any string
          // entries in sm.skillNames not already covered are appended as-is.
          var seenNames = new Set();
          var combined = [];
          for (var k = 0; k < result.slashCommands.length; k++) {
            var entry = result.slashCommands[k];
          ...
          // Append any skill names not already present (string fallback path)
          var extraNames = Array.from(sm.skillNames);
          for (var k = 0; k < extraNames.length; k++) {
            if (!seenNames.has(extraNames[k])) {
              seenNames.add(extraNames[k]);
              combined.push(extraNames[k]);
            }
          }
...
module.exports = { createSDKBridge, createMessageQueue, readMemAvailableMB, DEFAULT_MEM_AVAILABLE_MIN_MB };
