 ...
 * touch the registry. Call site (idle reaper, see startIdleReaper) observes
 * BEFORE any reaper-driven correction runs in the same tick, so a divergence
 * caused by a leaked token that the reaper is about to sweep is still
 * counted -- the point is measuring how often production state actually
 * disagrees, not how often it disagrees net of self-healing.
 */
function _recordActivityDivergenceIfAny(session) {
  var rawIsProcessing = !!session.isProcessing;
  var derivedIsActive = sessionActivity.isSessionActive(session);
  ...
  console.warn("[sdk-bridge] activity-source divergence (lr-58c813): session " + session.localId +
    " isProcessing=" + rawIsProcessing + " isSessionActive=" + derivedIsActive +
    " hasQueryInstance=" + sample.hasQueryInstance);
}
