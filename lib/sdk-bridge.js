...
function createSDKBridge(opts) {
  ...
  async function startQuery(session, text, images, linuxUser) {
    ...
    if (isNewQuery && _activeLiveCount >= MAX_CONCURRENT_SESSIONS) {
      ...
      sendAndRecord(session, {
        type: "error",
        text: "Too many active sessions (" + _activeLiveCount + "/" + MAX_CONCURRENT_SESSIONS + "). Wait for another session to finish, or increase CLAGENTIC_MAX_CONCURRENT_SESSIONS.",
      });
      sendAndRecord(session, { type: "done", code: 1 });
      sm.broadcastSessionList();
      return;
    ...
    try {
      ...
      if (isNewQuery) {
        _activeLiveCount = Math.max(0, _activeLiveCount - 1);
        console.log("[sdk-bridge] Live query count: " + _activeLiveCount + "/" + MAX_CONCURRENT_SESSIONS + " (session " + session.localId + " createQuery failed)");
      }
      session.isProcessing = false;
      onProcessingChanged();
      session.queryInstance = null;
    ...
    // lr-29f9: Mark this session as holding a live-count slot so the finally
    // block in processQueryStream can release it (and only it — pushMessage
    // continuations must not double-release).
    if (isNewQuery) session._isCountedLive = true;
