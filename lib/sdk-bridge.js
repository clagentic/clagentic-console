...
function createSDKBridge(opts) {
  ...
  async function startQuery(session, text, images, linuxUser) {
    ...
    // lr-7db0 — surface per-session agent identity into the SDK `agent` option.
    // When set, the Claude SDK applies the named agent's system prompt, tool
    // restrictions, and model for the main thread of this session. The session
    // still belongs to the project at `cwd`; only identity is shifted.
    //
    // The SDK only resolves `agent` against in-band `agents:{}` (or settings),
    // not file/plugin discovery — so we materialize the discovered agent's
    // definition and pass it alongside the name. If the agent can't be
    // resolved (renamed, missing, parse failure) we fall through silently;
    // the badge stays on the session for diagnosability but the session uses
    // default identity.
    //
    // Agent injection is Claude Code-only. The Codex adapter has no equivalent
    // API surface for in-band agent definition injection, so we skip this block
    // entirely for Codex sessions. The UI already hides the Agent Chat entry
    // point for Codex projects; this guard is belt-and-suspenders for sessions
    // that carry agentName from a prior vendor or from a direct ws message.
    if (session.agentName && sessionAdapter.vendor !== 'codex') {
      var agentDef = null;
      try { agentDef = agentsModule.getAgentDefinition(session.agentName); }
      catch (e) {
        console.error("[sdk-bridge] getAgentDefinition failed for " + session.agentName + ":", e.message);
      }
      if (agentDef) {
        claudeOpts.agent = session.agentName;
        var agentsMap = {};
        agentsMap[session.agentName] = agentDef;
        claudeOpts.agents = agentsMap;
      } else {
        console.warn("[sdk-bridge] session.agentName=" + session.agentName + " unresolved; not applying SDK agent option");
      }
    }
