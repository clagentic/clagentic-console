...
function createSessionManager(opts) {
  ...
  // (e.g. no file on disk yet) so the caller can fall back.
  ...
  function saveSessionFile(session) {
    ...
    try {
      // Session history is not resident in memory (lazy-loaded / LRU-evicted).
      // Rewriting from session.history here would serialize an empty array and
      // truncate the file to the meta line, destroying the persisted
      // conversation (lr-79c6). Rewrite only the meta line in place instead —
      // avoids paying the cost of a full history load just to change a title
      // or flag.
  ...
  function replayHistory(session, fromIndex, targetWs, transform) {
    ...
    if (typeof fromIndex !== "number") {
      if (total <= HISTORY_PAGE_SIZE) {
        ...
        fromIndex = extendWindowForVisibility(session.history, fromIndex, total);
      }
    }
