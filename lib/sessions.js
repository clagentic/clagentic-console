...
function createSessionManager(opts) {
  ...
  function findTurnBoundary(history, targetIndex) {
    // Cap the backward scan to HISTORY_PAGE_SIZE events. Without this cap,
    // sessions where one turn spans thousands of events (e.g. a long agent run
    // with 1600+ thinking_delta + tool events) expand the initial replay window
    // from the intended ~100 events to thousands — flooding the client on every
    // session switch and WS reconnect. The cap means we may start mid-turn, which
    // is acceptable: the load-more pagination lets users fetch earlier events.
    ...
    // No turn boundary found within the search window — start at targetIndex.
    // A partial turn at the top is preferable to a 36x oversized initial payload.
    return targetIndex;
  }
