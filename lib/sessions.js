...
// lr-041af8: single source of truth for "what model should THIS session's
// clients see right now" — a session's own persisted/live choice always
// wins, falling back to the project/global default (sm.currentModel) only
// when the session has never picked one of its own (e.g. brand new, or a
// pre-lr-db0437 session file with no persisted model). This is the same
// precedence every per-session config_state sender already used ad hoc
// (project-sessions.js's switch_session/set_model targeted replies); lifted
// out here so the connect/hydration path and every other sender can share
// it instead of re-deriving it (and risking a third divergence — see
// lr-db0437, which fixed the WRITE side of this same shared-mutable-state
// class but never enumerated the SENDERS, which is what lr-041af8 fixes).
function effectiveSessionModel(session, sm) {
...
function createSessionManager(opts) {
  ...
  // lr-13c047: fired whenever a session is permanently removed (single or
  // bulk delete) so the server's cross-project unread map (keyed
  // "slug::localId", lib/server.js) can prune the entry for this session
  // instead of leaking it forever — the "prune-on-session-delete" half of
  // Badge A's fix. No-op default keeps this opt-in for callers that don't
  // wire cross-project unread tracking (e.g. tests constructing a bare
  // SessionManager).
  ...
  function buildMetaLine(session) {
    ...
    // lr-db0437: persist the session's own model choice so it survives a
    // daemon restart / resume. Without this, session.model only ever lived
    // in memory — a restart silently reset every session back to whatever
    // the project/global default happened to be, indistinguishable from the
    // user never having picked a model at all.
  ...
  function loadSessions() {
    ...
    for (var i = 0; i < loaded.length; i++) {
      ...
      // lr-db0437: restore the session's own model choice across a daemon
      // restart — see buildMetaLine's write side above for why this must
      // persist (session.model, not the project/global default).
  ...
  function broadcastSessionList() {
    ...
    if (sendEach) {
      ...
      sendEach(function (ws, filterFn) {
        ...
        var clientActiveId = ws._clagenticActiveSession;
        var wsUnread = ws._clagenticUnread || {};
        if (ws.readyState === 1) {
  ...
  function switchSession(localId, targetWs, transform) {
    ...
    // references.
    //
    // No-auth / single-user connections have _clagenticUser === null (set by
    ...
    // the gate entirely for null users so that _clagenticActiveSession is still
    ...
    if (targetWs) {
      var requestingUser = targetWs._clagenticUser || null;
      if (requestingUser) {
      ...
      // null requestingUser = no-auth deployment; fall through to bind _clagenticActiveSession.
    }

    activeSessionId = localId;
    if (targetWs) {
      targetWs._clagenticActiveSession = localId;
      // Clear unread for this session (multi-user)
      if (targetWs._clagenticUnread) targetWs._clagenticUnread[localId] = 0;
    } else if (sendEach) {
      // No specific target: update all connected clients (server-initiated switch)
      sendEach(function (ws) {
        ws._clagenticActiveSession = localId;
      });
    }
    ...
    // lr-041af8: carry the session's own effective model on the wire so a
    // client hydrating from session_switched alone (no separate config_state
    // round-trip needed) never has to fall back to guessing the shared
    // project/global default — see effectiveSessionModel() above.
    _send({ type: "session_switched", id: localId, cliSessionId: session.cliSessionId || null, loop: session.loop || null, vendor: session.vendor || null, model: effectiveSessionModel(session, _instance), hasHistory: (session._historyLoaded ? session.history.length > 0 : !!session.cliSessionId), capabilities: _vendorCaps, isProcessing: !!session.isProcessing, agentName: session.agentName || null });
  ...
  function doSendToSession(session, obj) {
    ...
    if (sendEach) {
      ...
      sendEach(function (ws) {
        if (ws._clagenticActiveSession === session.localId && ws.readyState === 1) {
          ws.send(data);
        }
      });
  ...
  function doSendAndRecord(session, obj) {
    ...
    if (sendEach) {
      ...
      var ioData = null;
      sendEach(function (ws) {
        if (ws._clagenticActiveSession === session.localId) {
        ...
        if (obj.type === "done" && ws._clagenticActiveSession !== session.localId) {
          var _isMySession = !session.ownerId || (ws._clagenticUser && ws._clagenticUser.id === session.ownerId);
          if (_isMySession) {
            if (!ws._clagenticUnread) ws._clagenticUnread = {};
            ws._clagenticUnread[session.localId] = (ws._clagenticUnread[session.localId] || 0) + 1;
            if (ws.readyState === 1) {
              ws.send(JSON.stringify({ type: "session_unread", id: session.localId, count: ws._clagenticUnread[session.localId] }));
            }
          }
        }
      });
  ...
  // lr-041af8: captured once so internal functions (e.g. switchSession, which
  // may be invoked either as sm.switchSession(...) or directly by another
  // internal function like createSession — `this` is not reliably bound in
  // the latter case) can read ad hoc properties external callers set directly
  // on the returned instance (sm.currentModel, sm.modelsByVendor, etc.) that
  // were never declared as closure variables in this file.
  var _instance;
  ...
  _instance = {
    ...
    getTotalUnread: function (ws) {
      var unreadMap = (ws && ws._clagenticUnread) ? ws._clagenticUnread : {};
      var total = 0;
    ...
    // lr-041af8: exposed on the instance too (in addition to the module
    // export) so every existing `sm.<method>` call-site convention in
    // project-sessions.js / project-connection.js can call
    // `sm.effectiveSessionModel(session)` without importing this module
    // separately.
    effectiveSessionModel: function (session) { return effectiveSessionModel(session, _instance); },
