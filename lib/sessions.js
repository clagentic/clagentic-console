...
function createSessionManager(opts) {
  ...
  function createSession(sessionOpts, targetWs) {
    ...
    var session = {
      ...
      cumulativeUsage: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, turns: 0 },
    };
    sessions.set(localId, session);
  ...
  function replayHistory(session, fromIndex, targetWs, transform) {
    ...
    _send({ type: "history_done", lastUsage: lastUsage, lastModelUsage: lastModelUsage, lastCost: lastCost, lastStreamInputTokens: lastStreamInputTokens, contextUsage: session.lastContextUsage || null, cumulativeUsage: session.cumulativeUsage || null });
  }
