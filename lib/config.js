...
// Data dir: ~/.clagentic/console/  (prev: ~/.clagentic/ root → moved to console/ subdir by lr-12d0; prev: ~/.clay/ — migrated on first run)
...
if (!fs.existsSync(CLAGENTIC_HOME) && fs.existsSync(OLD_CLAY_HOME)) {
  try {
    fs.cpSync(OLD_CLAY_HOME, CLAGENTIC_HOME, { recursive: true, preserveTimestamps: true });
...
// One-time migration: copy all Console data from ~/.clagentic/ root into
// ~/.clagentic/console/ if not yet done.
//
// Sentinel: console/.migrated marker file. Using sessions/ directory existence
// as the sentinel is wrong — partial earlier migrations (lr-5dca, lr-eb5a)
// created console/sessions/ before this full sweep ran, causing the sentinel
// to fire too early and leaving any sessions written after that point orphaned.
// A marker file written only at sweep completion is the correct pattern
// (same as how ~/.clay/ → ~/.clagentic/ worked: gated on destination absence).
//
// Skip-if-dest-exists for each entry so files already in console/ from
// earlier partial migrations are preserved.
var _consoleMigrated = path.join(CLAGENTIC_HOME, "console", ".migrated");
if (!fs.existsSync(_consoleMigrated) && fs.existsSync(CLAGENTIC_HOME)) {
  try {
    fs.mkdirSync(path.join(CLAGENTIC_HOME, "console"), { recursive: true });
    // cpSync cannot copy a dir into one of its own subdirs — iterate entries
    // individually and skip the console/ subdir itself to avoid recursion.
    var _rootEntries = fs.readdirSync(CLAGENTIC_HOME);
    for (var _ri = 0; _ri < _rootEntries.length; _ri++) {
      if (_rootEntries[_ri] === "console") continue;
      var _src = path.join(CLAGENTIC_HOME, _rootEntries[_ri]);
      var _dst = path.join(CLAGENTIC_HOME, "console", _rootEntries[_ri]);
      if (!fs.existsSync(_dst)) {
        try { fs.cpSync(_src, _dst, { recursive: true, preserveTimestamps: true }); } catch (_ce) {}
      }
    }
    // Write sentinel only after the sweep completes so a crash mid-sweep
    // causes a retry on next start rather than silently leaving data behind.
    fs.writeFileSync(_consoleMigrated, "");
    console.log("[config] Migrated ~/.clagentic/ → ~/.clagentic/console/ — root files can be cleaned up when verified");
  } catch (e) {
    console.error("[config] Migration to console/ subdir failed:", e.message);
  }
}

var CONSOLE_DIR = path.join(CLAGENTIC_HOME, "console");
var CONFIG_DIR = CONSOLE_DIR;
var EXTERNAL_TRIGGERS_DIR = path.join(CONFIG_DIR, "external-triggers");
var OLD_EXTERNAL_TRIGGERS_DIR = path.join(CLAGENTIC_HOME, "external-triggers");
...
var OLD_CRASH_INFO_PATH = path.join(CLAGENTIC_HOME, "crash.json");
...
// Path used before lr-12d0 moved CONFIG_DIR to console/ (was CLAGENTIC_HOME root).
function oldConfigPath() {
  return path.join(CLAGENTIC_HOME, _devMode ? "daemon-dev.json" : "daemon.json");
}
...
// Path used by pre-lr-12d0 daemons (before CONFIG_DIR moved to console/).
// Only meaningful on Unix — Windows named pipes were unchanged.
function oldSocketPath() {
  if (process.platform === "win32") return null;
  return path.join(CLAGENTIC_HOME, _devMode ? "daemon-dev.sock" : "daemon.sock");
}
...
// Path used by pre-lr-12d0 daemons (before CONFIG_DIR moved to console/).
function oldLogPath() {
  return path.join(CLAGENTIC_HOME, _devMode ? "daemon-dev.log" : "daemon.log");
}
...
function ensureConfigDir() {
  ...
  if (!process.env.CLAGENTIC_HOME) {
    ...
    // external-triggers/
    // Use lstatSync (not existsSync) so a pre-existing symlink at the old path is
    // detected and skipped — we only act on a real directory.
    var _oldTriggersIsRealDir = false;
    try {
      var _oldStat = fs.lstatSync(OLD_EXTERNAL_TRIGGERS_DIR);
      _oldTriggersIsRealDir = _oldStat.isDirectory() && !_oldStat.isSymbolicLink();
    } catch (_) {}
    if (_oldTriggersIsRealDir) {
      if (!fs.existsSync(EXTERNAL_TRIGGERS_DIR)) {
        // Case 1: new dir absent — simple copy (existing behavior).
        try {
        ...
        // Case 2: both old and new exist — merge .json files from old that are not
        // already present in new (by filename), then replace the old dir with a
        // symlink → EXTERNAL_TRIGGERS_DIR so stale writers (e.g. an un-updated relay
        // binary still writing to the old root path) are silently redirected.
        try {
          // Merge top-level .json files.
          var _oldFiles = fs.readdirSync(OLD_EXTERNAL_TRIGGERS_DIR);
          for (var _oi = 0; _oi < _oldFiles.length; _oi++) {
            var _oldFile = _oldFiles[_oi];
            if (_oldFile === "processed") continue; // handled separately below
            if (path.extname(_oldFile) !== ".json") continue;
            var _oldFilePath = path.join(OLD_EXTERNAL_TRIGGERS_DIR, _oldFile);
            var _newFilePath = path.join(EXTERNAL_TRIGGERS_DIR, _oldFile);
            if (!fs.existsSync(_newFilePath)) {
              try {
                fs.copyFileSync(_oldFilePath, _newFilePath);
                console.warn("[config] Merged external-triggers/" + _oldFile + " → " + _newFilePath);
              } catch (_ce) {
                console.error("[config] Merge of " + _oldFile + " failed (non-fatal):", _ce.message);
              }
            }
          }
          // Merge processed/ subdir files.
          var _oldProcessedDir = path.join(OLD_EXTERNAL_TRIGGERS_DIR, "processed");
          var _newProcessedDir = path.join(EXTERNAL_TRIGGERS_DIR, "processed");
          if (fs.existsSync(_oldProcessedDir)) {
            try { fs.mkdirSync(_newProcessedDir, { recursive: true }); } catch (_) {}
            var _oldProcessed = fs.readdirSync(_oldProcessedDir);
            for (var _pi = 0; _pi < _oldProcessed.length; _pi++) {
              var _pf = _oldProcessed[_pi];
              if (path.extname(_pf) !== ".json") continue;
              var _oldPfPath = path.join(_oldProcessedDir, _pf);
              var _newPfPath = path.join(_newProcessedDir, _pf);
              if (!fs.existsSync(_newPfPath)) {
                try {
                  fs.copyFileSync(_oldPfPath, _newPfPath);
                  console.warn("[config] Merged external-triggers/processed/" + _pf + " → " + _newPfPath);
                } catch (_ce2) {
                  console.error("[config] Merge of processed/" + _pf + " failed (non-fatal):", _ce2.message);
                }
              }
            }
          }
          // Replace old dir with a symlink → EXTERNAL_TRIGGERS_DIR. Rename to a temp
          // name first, create the symlink, then remove the temp. Non-fatal: on any
          // failure we best-effort restore the original name and leave for manual cleanup.
          var _oldTriggersTmp = OLD_EXTERNAL_TRIGGERS_DIR + ".migration-tmp";
          try {
            fs.renameSync(OLD_EXTERNAL_TRIGGERS_DIR, _oldTriggersTmp);
            fs.symlinkSync(EXTERNAL_TRIGGERS_DIR, OLD_EXTERNAL_TRIGGERS_DIR);
            fs.rmSync(_oldTriggersTmp, { recursive: true, force: true });
            console.warn("[config] Replaced " + OLD_EXTERNAL_TRIGGERS_DIR + " with symlink → " + EXTERNAL_TRIGGERS_DIR);
          } catch (_se) {
            console.error("[config] Symlink replacement of external-triggers/ failed (non-fatal):", _se.message);
            try {
              if (!fs.existsSync(OLD_EXTERNAL_TRIGGERS_DIR)) {
                fs.renameSync(_oldTriggersTmp, OLD_EXTERNAL_TRIGGERS_DIR);
              }
            } catch (_re) {}
          }
        } catch (e) {
          console.error("[config] Migration merge of external-triggers/ failed (non-fatal):", e.message);
        }
      }
    }
    // daemon.json / daemon-dev.json (lr-eb5a: moved into console/)
    var oldCfg = oldConfigPath();
    var newCfg = configPath();
    if (fs.existsSync(oldCfg) && !fs.existsSync(newCfg)) {
      try {
        fs.copyFileSync(oldCfg, newCfg);
        try { fs.chmodSync(newCfg, 0o600); } catch (_) {}
        console.warn("[config] Migrated " + oldCfg + " → " + newCfg);
      } catch (e) {
        console.error("[config] Migration of daemon.json failed (non-fatal):", e.message);
      }
    }
  }
