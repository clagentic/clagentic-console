...
// ~/.clagentic/console/ if not yet done. Sentinel: console/sessions/ absent.
// Uses force:false so files already in console/ (from lr-5dca, lr-eb5a) are
// not overwritten — preserves any newer versions already migrated there.
// Same pattern as the clay→clagentic migration above.
var _consoleSentinel = path.join(CLAGENTIC_HOME, "console", "sessions");
if (!fs.existsSync(_consoleSentinel) && fs.existsSync(CLAGENTIC_HOME)) {
  try {
