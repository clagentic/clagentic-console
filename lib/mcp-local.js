...
// mcp.json lives under the Console data dir (CONFIG_DIR = ~/.clagentic/console/).
// One-time migration: if the old root-level file exists and the new one does not,
// copy it so existing server configs are preserved on upgrade.
var CLAY_CONFIG_PATH = path.join(CONFIG_DIR, "mcp.json");
(function migrateMcpJson() {
  var oldPath = path.join(os.homedir(), ".clagentic", "mcp.json");
  try {
    if (fs.existsSync(oldPath) && !fs.existsSync(CLAY_CONFIG_PATH)) {
      fs.mkdirSync(path.dirname(CLAY_CONFIG_PATH), { recursive: true });
      fs.copyFileSync(oldPath, CLAY_CONFIG_PATH);
      console.log("[mcp-local] Migrated mcp.json → " + CLAY_CONFIG_PATH + " — root copy can be removed when verified");
    }
  } catch (e) {
    // Non-fatal — mcp.json will be created fresh on first use
  }
}());
...
// Returns merged server definitions from ~/.clagentic/mcp.json + includes.
