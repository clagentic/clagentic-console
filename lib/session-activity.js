...
function createRegistry() {
  return {
    // lr-66c118 (fold-in, flagged by BOBBIE on 2/4 and 3/4): tokens is
    // written by bracket assignment (registry.tokens[token] = ...) keyed by
    // SDK/Codex-supplied ids with no prototype guard. Object.create(null)
    // removes the Object.prototype chain so a crafted token id (e.g.
    // "__proto__" or "constructor") cannot collide with an inherited
    // property/method. Session-scoped, no demonstrated exploit — cheap
    // hardening, not a response to a live vulnerability.
    tokens: Object.create(null),   // token -> { source, label, startedAt, generation }
    generation: 0,
  };
}
 ...
 * the viewing client only (ws._clagenticActiveSession-style filtering) rather
