...
function performUpdate(channel) {
  var tag = channel === "beta" ? "beta" : "latest";
  try {
    execFileSync("npm", ["install", "-g", "@clagentic/console@" + tag], { stdio: "pipe" });
    return true;
  } catch (e) {
    return false;
  }
...
async function checkAndUpdate(currentVersion, skipUpdate, channel) {
  ...
  log(sym.bar + "  " + a.dim + "Run manually: npm install -g @clagentic/console@" + tag + a.reset);
  log("");
  return false;
}
