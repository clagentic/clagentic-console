...
async function handleQueryStart(msg) {
  ...
  if (process.env.CLAGENTIC_DEBUG === "1") {
    // worker-logs lives under CONFIG_DIR (= ~/.clagentic/console/); old path was ~/.clagentic/worker-logs/.
    // No migration needed — these are ephemeral debug logs, not persistent data.
    var _configDir = process.env.CLAGENTIC_HOME ? path.join(process.env.CLAGENTIC_HOME, "console") : path.join(os.homedir(), ".clagentic", "console");
    var _logDir = path.join(_configDir, "worker-logs");
  ...
  try {
    ...
    for (var k in pendingMcpCalls) {
      try { pendingMcpCalls[k].reject(new Error("Worker shutting down")); } catch(e) {}
    }
    for (var k in pendingAskUser) {
      try { pendingAskUser[k]({ behavior: "deny", message: "Worker shutting down" }); } catch(e) {}
    }
    for (var k in pendingElicitations) {
      try { pendingElicitations[k]({ action: "reject" }); } catch(e) {}
    }
    queryInstance = null;
    messageQueue = null;
    abortController = null;
