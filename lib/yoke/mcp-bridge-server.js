...
// list/call requests to the project-scoped HTTP endpoint
...
//   3. We respond, then fetch tool list from the server
//   4. Codex sends "tools/list" -> we return cached tools
//   5. Codex sends "tools/call" -> we proxy to the server and return result
...
function postJson(urlPath, body) {
  return new Promise(function (resolve, reject) {
    ...
    // Include auth cookie if available (required for authenticated servers)
    if (clayAuthToken) {
...
function fetchTools() {
  return postJson(CLAY_MCP_PATH, { action: "list_tools" }).then(function (resp) {
    ...
    log("Fetched " + _tools.length + " tools: " + Object.keys(serverNames).map(function(s) { return s + "(" + serverNames[s] + ")"; }).join(", "));
    return _tools;
  }).catch(function (err) {
