...
// with the main daemon over a Unix domain socket using JSON lines.
//
...
var fs = require("fs");
var os = require("os");
var parseDiagnosticLine = require("./diagnostic-patterns").parseDiagnosticLine;
...
// The daemon. Each user may have their own `claude` install (nvm, asdf,
...
var conn = null;
var buffer = "";
// Reference to the claude CLI child process spawned by spawnClaudeCodeProcess.
// Stored so cleanup() can send SIGTERM/SIGKILL to it on worker shutdown.
...
function canUseTool(toolName, input, opts) {
  ...
  return new Promise(function(resolve) {
    var timeoutId = setTimeout(function() {
      if (pendingPermissions[requestId]) {
        delete pendingPermissions[requestId];
        resolve({ behavior: "deny", message: "Permission request timed out after 30s" });
      }
    }, 30000);
    pendingPermissions[requestId] = function(result) {
...
async function handleQueryStart(msg) {
  ...
  if (process.env.CLAGENTIC_DEBUG === "1") {
    var _logDir = path.join(os.homedir(), ".clagentic", "worker-logs");
    fs.mkdirSync(_logDir, { recursive: true, mode: 0o700 });
    var _logFile = path.join(_logDir, process.pid + ".log");
    // Create with 0o600 before passing to the SDK so the file is never world-readable
    fs.writeFileSync(_logFile, "", { flag: "a", mode: 0o600 });
    options.debug = true;
    options.debugFile = _logFile;
  }
  ...
  options.spawnClaudeCodeProcess = function(spawnOpts) {
    ...
    // Track the child so cleanup() can terminate it on worker shutdown.
    _claudeChildProcess = cp;
    // Capture ALL CLI stderr. Known diagnostic lines are also emitted as
    // 'diagnostic' YOKE events (stage 2/5, lr-28ee) so they can be routed to
    // the Console UI. The raw [CLI-STDERR] log is always kept regardless.
    if (cp.stderr) {
      cp.stderr.on("data", function(chunk) {
        ...
        for (var li = 0; li < lines.length; li++) {
          ...
          if (!line) continue;
          console.log("[CLI-STDERR] " + line.substring(0, 500));
          var diag = parseDiagnosticLine(line);
          if (diag) {
            // Emit as sdk_event so the existing IPC path (createWorkerQueryHandle
            // → flattenEvent) carries it through unchanged. flattenEvent will
            // tag it yokeType:"unknown"/rawType:"diagnostic" until stage 3/5
            // (lr-0868) adds the explicit branch in flattenEvent().
            sendToDaemon({ type: "sdk_event", event: diag });
          }
        }
      });
    }
  ...
  try {
    ...
    for (var k in pendingPermissions) {
      try { pendingPermissions[k]({ behavior: "deny", message: "Worker shutting down" }); } catch(e) {}
    }
    for (var k in pendingMcpToolCalls) {
      try { pendingMcpToolCalls[k].reject(new Error("Worker shutting down")); } catch(e) {}
    }
    for (var k in pendingAskUser) {
      try { pendingAskUser[k](new Error("Worker shutting down")); } catch(e) {}
    }
    for (var k in pendingElicitations) {
      try { pendingElicitations[k](new Error("Worker shutting down")); } catch(e) {}
    }
    queryInstance = null;
    messageQueue = null;
    abortController = null;
...
async function handleSetModel(msg) {
  ...
  try {
    ...
    // lr-f22787: echo requestId so the daemon-side setModel() promise
    // (claude.js pendingSetModel) can be correlated and resolved — without
    // it, the daemon has no way to distinguish this reply from any other
    // model_changed broadcast.
    sendToDaemon({ type: "model_changed", model: msg.model, requestId: msg.requestId });
  } catch (e) {
    sendToDaemon({ type: "worker_error", error: "Failed to set model: " + (e.message || e), requestId: msg.requestId });
  }
}
...
function cleanup() {
  ...
  // Terminate the claude CLI child process. Without this, it outlives the worker
  // and becomes orphaned when the daemon sends SIGTERM to the process group.
  if (_claudeChildProcess && !_claudeChildProcess.killed) {
    try { _claudeChildProcess.kill("SIGTERM"); } catch (e) {}
    // SIGKILL fallback after brief grace period to allow the CLI to flush state.
