  ...
  async function runQueryLoop(initialMessage) {
    ...
    try {
      ...
      while (!isCancelled()) {
        ...
        // (command execution ~line 728, file change ~line 751, MCP
        // elicitation ~line 770) already routes through canUseTool; this
        // brings $skill-name into the same gate using the same vendor-neutral
        // "Skill" tool name + { skill: <name> } input shape the Claude Skill
        // tool uses (lib/utils.js permissionGrantKey), so a Codex skill grant
        // behaves identically to a Claude one — same key shape, same
        // per-skill session-grant semantics, same persistence round-trip.
