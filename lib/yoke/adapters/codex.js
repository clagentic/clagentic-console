...
// Enrich a raw string model ID into a rich descriptor, or passthrough if already rich.
function enrichCodexModel(model) {
  if (model && typeof model === "object" && model.value) {
    // Already a rich object — ensure all fields are present
    return {
      value: model.value,
      displayName: model.displayName || model.value,
      supportsEffort: Object.prototype.hasOwnProperty.call(model, "supportsEffort") ? model.supportsEffort : true,
      supportedEffortLevels: model.supportedEffortLevels || CODEX_EFFORT_LEVELS,
      supportsThinking: Object.prototype.hasOwnProperty.call(model, "supportsThinking") ? model.supportsThinking : false,
    };
  }
  // Plain string — look up in defaults table, fall back to generic enrichment
  var id = String(model || "");
  for (var i = 0; i < CODEX_DEFAULT_MODELS.length; i++) {
    if (CODEX_DEFAULT_MODELS[i].value === id) return CODEX_DEFAULT_MODELS[i];
  }
  // Unknown model — assign reasonable defaults
  return {
    value: id,
    displayName: id,
    supportsEffort: true,
  ...
  var adapter = {
    ...
    init: function(initOpts) {
      ...
      _initPromise = (async function() {
        ...
        // Use curated default model list. CODEX_DEFAULT_MODELS entries are already
        // rich descriptor objects with capability flags; no further enrichment needed.
