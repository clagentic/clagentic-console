...
var { discoverSkillsWithMeta, resolveContainedSkillDir } = require("../../sdk-skill-discovery");
var codexLocalImage = require("../codex-local-image");
...
//
// homeOverride: optional. On a shared multi-user daemon, the caller resolves
// the SESSION OWNER's home (os-users.getLinuxUserHome(linuxUser)) and passes
// it here so the global skills dir is the invoking user's own ~/.claude/skills,
// not the daemon process's REAL_HOME (lr-3ccc78). Falls back to REAL_HOME
// (config.js) / os.homedir() when absent — single-user case, unchanged.
function discoverClaudeSkills(cwd, homeOverride) {
  var skills = {};
  var globalHome = homeOverride;
  if (!globalHome) {
    try { globalHome = require("../../config").REAL_HOME; } catch (e) { globalHome = require("os").homedir(); }
  }
  var dirs = [
    path.join(globalHome, ".claude", "skills"),
  ...
  for (var d = 0; d < dirs.length; d++) {
    ...
    if (!base) continue;
    var entries;
    try { entries = fs.readdirSync(path.resolve(base), { withFileTypes: true }); } catch (e) { continue; }
    for (var i = 0; i < entries.length; i++) {
      var entry = entries[i];
      if (!entry.isDirectory() && !entry.isSymbolicLink()) continue;
      // Two-stage containment (lexical + realpath) — see
      // sdk-skill-discovery.js's resolveContainedSkillDir doc comment.
      // entry.name comes from readdirSync and may be a symlink; a lexical
      // check alone does not stop fs.accessSync below from following a
      // symlinked entry outside base.
      var skillDir = resolveContainedSkillDir(base, entry.name);
...
// lr-3ccc78 — resolve a linuxUser into its home directory for skill
// discovery. Mirrors sdk-bridge.js's resolveEffectiveHome; duplicated here
// (rather than imported) because the two modules deliberately do not share
// runtime state — os-users is the only cross-module dependency needed, and
// it is already a peer-module import pattern used elsewhere in this file.
function resolveLinuxUserHome(linuxUser) {
...
function flattenEvent(notification, state) {
  ...
  if (method === "turn/completed") {
    // lr-872f94 — schema-verified (`codex app-server generate-json-schema`,
    // codex-cli 0.124.0): TurnCompletedNotification is { threadId, turn },
    // and Turn carries no usage field at all. params.usage/params.status
    // were already always undefined on the real wire protocol; the actual
    // token breakdown and status live on thread/tokenUsage/updated (state.last*)
    // and params.turn.status respectively. Kept the params.usage/params.status
    // reads as a defensive fallback in case a future server version adds them.
    ...
    var outputTokens = state.lastOutputTokens || (usage ? (usage.output_tokens || 0) : 0);
    var cachedTokens = state.lastCachedInputTokens || (usage ? (usage.cached_input_tokens || 0) : 0);
    // reasoningOutputTokens has no params.usage fallback -- it's Codex-specific
    // and only ever arrives via thread/tokenUsage/updated.
    var reasoningTokens = state.lastReasoningOutputTokens || 0;
    var hasTokenData = inputTokens > 0 || outputTokens > 0;
    var resultModelUsage = {};
    // lr-872f94 — vendor-first contract (lr-3af675): state.lastContextWindow
    // is set only from ThreadTokenUsage.modelContextWindow, the vendor's own
    // reported value. Never a hardcoded per-model constant; stays null (=
    // unknown, renders the indeterminate meter state) when the server hasn't
    // reported one yet for this session.
    ...
    state.lastCachedInputTokens = null;
    state.lastReasoningOutputTokens = null;
    return events;
  }
  ...
  // Token usage update - track input tokens for context bar, plus the
  // previously-discarded breakdown fields and vendor context window
  // (lr-872f94; schema-verified via `codex app-server generate-json-schema`
  // against ThreadTokenUsageUpdatedNotification -> ThreadTokenUsage ->
  // TokenUsageBreakdown, codex-cli 0.124.0).
  if (method === "thread/tokenUsage/updated") {
    var tu = params.tokenUsage;
    if (tu && tu.total) {
      state.lastInputTokens = tu.total.inputTokens || 0;
      state.lastOutputTokens = tu.total.outputTokens || 0;
      state.lastCachedInputTokens = tu.total.cachedInputTokens || 0;
      state.lastReasoningOutputTokens = tu.total.reasoningOutputTokens || 0;
    }
    if (tu && typeof tu.modelContextWindow === "number" && tu.modelContextWindow > 0) {
...
// Testable seam: the exact expression used to resolve model in
// createCodexQueryHandle (both state.model and threadParams.model).
// Exported so tests import and call the real function rather than
// inlining a copy — reverting the || queryOpts.defaultModel term
// in codex.js will break those tests. (lr-a7e7)
//
// The "gpt-5.5" literal is a fallback-only last resort (lr-d91ecf): used
// only when neither the query nor the adapter supplies a model at all,
// which should not happen in normal operation (the adapter always resolves
// a default from CODEX_DEFAULT_MODELS or the live model/list response).
function _test_resolveModel(queryOpts) {
  return queryOpts.model || queryOpts.defaultModel || "gpt-5.5";
}

// Selects the model used for background title generation.
//
// The live "model/list" RPC's Model objects (see the model/list handler
// below, and _cachedModels' shape) carry no cost-tier field — only
// value/displayName/supportedEffortLevels/isLatest ("isLatest" means "not
// superseded by an upgrade", not "cheap"). There is no robust signal in the
// live catalog to identify a mini/small-tier model, and this codebase does
// not substring-match displayName/value for a tier (that class of fragile
// string matching was deliberately removed elsewhere — see the Context
// meter section of docs/guides/architecture.md).
//
// So titling — a trivial 3-8 word job — falls back to _defaultModel, the
// adapter's only non-staling model identifier (refreshed from the live
// model/list response). This is a deliberate cost/staleness tradeoff: every
// auto-title now runs on the user's main model instead of a cheaper
// mini-tier one, in exchange for auto-titling never breaking silently when
// OpenAI retires a pinned mini snapshot. See PR body for the reasoning.
function resolveTitleModel(defaultModel) {
...
function createCodexQueryHandle(appServer, queryOpts) {
  ...
  var state = {
    ...
    // lr-872f94 — thread/tokenUsage/updated's TokenUsageBreakdown carries
    // outputTokens/cachedInputTokens/reasoningOutputTokens too (verified
    // against `codex app-server generate-json-schema`, codex-cli 0.124.0);
    // previously only .total.inputTokens was read here, discarding the
    // rest. Captured on state so turn/completed's result event can surface
    // them without re-deriving from the (usage-less) turn/completed params.
    lastOutputTokens: null,
    lastCachedInputTokens: null,
    lastReasoningOutputTokens: null,
    // lr-872f94 — ThreadTokenUsage.modelContextWindow (nullable int64) is the
    // vendor's own per-session context window, schema-verified present on
    // thread/tokenUsage/updated (NOT on turn/started as originally
    // hypothesized from degraded research — see PR body). Feeds
    // modelUsage[model].contextWindow on the next result event, keeping the
    // vendor-first contract from lr-3af675: no hardcoded per-model table.
    lastContextWindow: null,
    done: false,
    aborted: false,
    loopStarted: false,
    model: _test_resolveModel(queryOpts),
    // lr-79ee — live model/effort, mutable via handle.setModel()/setEffort().
    // The app-server has no thread-level "change model" RPC, but turn/start
    // accepts a per-turn model/effort override (see docs/guides/codex-reference/
    // llms-full.txt around "Start a turn"), so mutating these here and passing
    // them into every turn/start call switches the active thread's model on
    // its next turn — no thread recreation required.
    modelReasoningEffort: queryOpts.modelReasoningEffort || null,
    ...
    // lr-f7a4 — live tool policy, mutable via handle.setToolPolicy(). The
    // app-server's own approvalPolicy is fixed at thread creation, but every
    // requestApproval/requestUserInput notification is resolved here in JS
    // via canUseTool, so a mid-thread policy change genuinely takes effect
    // on the next approval request even though the underlying thread was
    // started with a different approvalPolicy.
    toolPolicy: queryOpts.toolPolicy || null,
    // lr-de04 — absolute paths of localImage temp files written for the turn
    // currently in flight. Cleared (and files deleted) once that turn's
    // input has been sent, since the app-server has read the file by then.
    pendingImagePaths: [],
  };
  ...
  function handleServerEvent(msg) {
    ...
    // lr-f7a4 — live "allow-all" tool policy, settable mid-thread via
    // handle.setToolPolicy("allow-all"). Bypasses the canUseTool round-trip
    // entirely so an operator-triggered full-auto toggle takes effect on the
    // very next approval request, without waiting for a new thread.
    function isAllowAll() {
      return state.toolPolicy === "allow-all";
    }
    ...
    if (method === "item/commandExecution/requestApproval") {
      ...
      if (isAllowAll()) {
        ...
        // No approval callback wired (e.g. warmup/init queries) — fail
        // closed rather than silently executing an unreviewed command
        // (lr-f969dc: brings this site in line with the Skill injection
        // gate below, which already fails closed in the same situation).
        console.log("[yoke/codex] no canUseTool wired, declining command execution:", cmdParams.command);
        appServer.respond(msg.id, { decision: "decline" });
      }
      return;
    }
    ...
    if (method === "item/fileChange/requestApproval") {
      ...
      if (isAllowAll()) {
        ...
        // No approval callback wired — fail closed (lr-f969dc), same as the
        // command-execution site above and the Skill injection gate below.
        console.log("[yoke/codex] no canUseTool wired, declining file change:", fcParams.path);
        appServer.respond(msg.id, { decision: "decline" });
      }
      return;
    }
    ...
    if (method === "item/tool/requestUserInput" || method === "mcpServer/elicitation/request") {
      ...
      if (onElicitation) {
        ...
        // Neither an elicitation UI nor a canUseTool callback is wired —
        // fail closed (lr-f969dc), same as the command/file approval sites
        // above and the Skill injection gate below.
        console.log("[yoke/codex] no canUseTool/onElicitation wired, declining MCP approval:", (mcpMeta.tool || "?"));
        appServer.respond(msg.id, { action: "decline" });
      }
      return;
    }
  ...
  async function runQueryLoop(initialMessage) {
    ...
    // Register with the multiplexed event handler so concurrent queries on the
    // same app-server instance don't clobber each other's handler.  Start with
    // threadId=null (receives all events) and narrow to the specific thread once
    // we know it, so events that arrive before thread/started are not dropped.
    var _handlerEntry = appServer.addEventHandler(handleServerEvent, null);
    ...
    try {
      ...
      if (threadResult && threadResult.thread) {
        ...
        // Narrow the handler to only receive events for this thread so other
        // concurrent queries (e.g. auto-title) are not routed to us.
        appServer.updateHandlerThreadId(_handlerEntry, state.threadId);
      }
      ...
      while (!isCancelled()) {
        ...
        //
        // lr-4a13c3 — this path previously appended matched skills straight
        // onto the turn/start RPC input with no approval step at all: skill
        // files carry arbitrary instructions, so injecting one unprompted is
        // equivalent to injecting arbitrary agent instructions with zero
        // operator sign-off. Every other Codex approval surface in this file
        // (command execution, file change, MCP elicitation — see the
        // item/*/requestApproval and mcpServer/elicitation/request branches
        // above) already routes through canUseTool; this brings $skill-name
        // into the same gate using the same vendor-neutral "Skill" tool name
        // + { skill: <name> } input shape the Claude Skill tool uses
        // (lib/utils.js permissionGrantKey), so a Codex skill grant behaves
        // identically to a Claude one — same key shape, same per-skill
        // session-grant semantics, same persistence round-trip.
        //
        // lr-f969dc — at the time this gate was added, the three older sites
        // above still fell back to auto-accept when no canUseTool callback
        // was wired (e.g. warmup/init queries), while this newer site failed
        // closed in the same situation — an inconsistency BOBBIE flagged on
        // the lr-4a13c3 PR. All four sites now fail closed identically.
        var availableSkills = discoverClaudeSkills(queryOpts.cwd, queryOpts.homeOverride);
        ...
        for (var ii = 0; ii < input.length; ii++) {
          if (input[ii].type === "text" && input[ii].text) {
            ...
            for (var si = 0; si < parsed.skills.length; si++) {
              if (!seenSkillNames[parsed.skills[si].name]) {
                seenSkillNames[parsed.skills[si].name] = true;
                candidateSkills.push(parsed.skills[si]);
              }
            }
          }
        }
        if (candidateSkills.length > 0) {
          if (state.toolPolicy === "allow-all") {
            skillItemsToInject = candidateSkills.map(function(s) { return { type: "skill", name: s.name, path: s.path }; });
          } else if (canUseTool) {
            for (var ci = 0; ci < candidateSkills.length; ci++) {
              var candidate = candidateSkills[ci];
              var decision;
              try {
                decision = await canUseTool("Skill", { skill: candidate.name }, {});
              } catch (err) {
                console.error("[yoke/codex] Skill canUseTool error:", err.message);
                decision = null;
              }
              if (decision === true || (decision && decision.behavior === "allow")) {
                skillItemsToInject.push({ type: "skill", name: candidate.name, path: candidate.path });
              } else {
                console.log("[yoke/codex] Skill denied, not injected:", candidate.name);
              }
            }
          } else {
            // No approval callback wired (e.g. warmup/init queries) — fail
            // closed rather than silently injecting ungated instructions.
            console.log("[yoke/codex] no canUseTool wired, skipping skill injection:", candidateSkills.map(function(s) { return s.name; }).join(", "));
          }
        }
        ...
        if (state.modelReasoningEffort) {
          turnParams.effort = state.modelReasoningEffort;
        }
        await appServer.send("turn/start", turnParams, 60000);

        // Wait for turn to complete
        await turnPromise;

        // lr-de04 — the app-server has finished reading any localImage temp
        // files for this turn (whether it completed or failed), so they can
        // be removed now rather than lingering until process exit.
        if (state.pendingImagePaths.length > 0) {
          codexLocalImage.removeTempImages(state.pendingImagePaths);
      ...
      // Always deregister this query's handler so it doesn't receive stale events
      // from a future query on the same app-server thread ID.
      appServer.removeEventHandler(_handlerEntry);
      // lr-de04 — catch-all for images belonging to a turn that never
      // reached the post-turnPromise cleanup above (abort/error mid-turn).
      if (state.pendingImagePaths.length > 0) {
        codexLocalImage.removeTempImages(state.pendingImagePaths);
        state.pendingImagePaths = [];
      }
  ...
  var handle = {
    ...
    pushMessage: function(text, images) {
      ...
      if (images && images.length > 0) {
        ...
        for (var i = 0; i < images.length; i++) {
          var img = images[i];
          var imgPath = codexLocalImage.writeTempImage(img.mediaType, img.data);
          if (imgPath) {
            state.pendingImagePaths.push(imgPath);
        ...
        if (failedCount > 0) {
          // Acceptance criterion (lr-de04): an image must never be dropped
          // silently. Surface a visible error event so the UI shows it,
          // matching the shape flattenEvent already produces for turn/failed.
          pushEvent({
            yokeType: "error",
            text: failedCount === 1
              ? "1 image could not be sent to Codex (failed to write temp file) and was not included in this message."
              : failedCount + " images could not be sent to Codex (failed to write temp file) and were not included in this message.",
          });
        }
    ...
    setModel: function(model) {
      // lr-79ee — genuinely live: turn/start (see runQueryLoop above) sends
      // state.model on every turn, so this takes effect starting with the
      // next turn on the current thread. No thread recreation needed.
      if (model) state.model = model;
      return Promise.resolve();
    },

    setEffort: function(effort) {
      // lr-79ee — same live mechanism as setModel: state.modelReasoningEffort
      // is read by turn/start on every turn.
      state.modelReasoningEffort = effort || null;
      return Promise.resolve();
    },

    setToolPolicy: function(policy) {
      // lr-f7a4 — the app-server's approvalPolicy is fixed at thread
      // creation (no known live-update RPC), but every approval
      // notification is resolved here in JS via canUseTool, so mutating
      // state.toolPolicy genuinely changes behavior for the next approval
      // request on this thread (see isAllowAll() in handleServerEvent).
      state.toolPolicy = policy || null;
      return Promise.resolve();
    },
    ...
    getContextUsage: function() {
      // lr-3af675 / lr-872f94: Codex's app-server has no equivalent of
      // Claude's rich context-usage API (categories/totalTokens/maxTokens
      // breakdown) — state.lastUsage is a flat turn-usage object
      // (input_tokens etc), not that shape. Returning it here used to make
      // the frontend's richContextUsage truthiness check pass, rendering a
      // confident "0 / 200K · 0%" popover built from data that was never a
      // context-window measurement at all. Returning null keeps the
      // frontend on its per-turn 'result' event fallback instead, which is
      // honest and (as of lr-872f94) now carries a real vendor context
      // window via modelUsage[model].contextWindow, sourced from
      // ThreadTokenUsage.modelContextWindow (thread/tokenUsage/updated) —
      // schema-verified there is no turn/started model_context_window field
      // in the installed protocol (codex-cli 0.124.0); that was a research
      // hypothesis, not a real field. See PR body for the schema dump.
...
// --- Model enrichment ---
// Curated model descriptors for Codex/OpenAI models. These are used as the
// authoritative list after init because the Codex app-server does not expose a
// models/list endpoint. Update this table when OpenAI releases new models.
//
// supportsEffort: whether the model accepts an effort/reasoning-effort parameter
// supportedEffortLevels: ordered list of valid effort values for the model
// supportsThinking: whether extended thinking UI should be shown (Codex uses effort
//   as its equivalent of thinking depth; the thinking toggle itself is Claude-only)

var CODEX_EFFORT_LEVELS = ["minimal", "low", "medium", "high", "xhigh"];
var CODEX_EFFORT_LEVELS_STANDARD = ["low", "medium", "high"]; // models without xhigh/minimal

// Fallback table used only when the live "model/list" RPC fails or returns
// nothing (see catch block below). This static list carries no "upgrade"
// signal from the app-server, so isLatest defaults true for every entry —
// nothing is hidden behind the older-models disclosure when we have no basis
// to tier it (lr-e03635: absence of signal must never manifest as a hidden
// model).
var CODEX_DEFAULT_MODELS = [
  ...
  },
  {
    ...
    supportsEffort: true,
    supportedEffortLevels: CODEX_EFFORT_LEVELS_STANDARD,
    supportsThinking: false,
    isLatest: true,
  },
  {
...
function createCodexAdapter(opts) {
  ...
  var _cachedModels = [];
  // Fallback-only (lr-d91ecf): the initial value before init()/warmup
  // populates _cachedModels from the live model/list RPC or
  // CODEX_DEFAULT_MODELS. Overwritten as soon as either succeeds.
  var _defaultModel = "gpt-5.5";
  var _appServer = null;
  ...
  // Build the enriched slash_commands array from skill names and workflow discovery.
  // Returns {name, desc, type}[] with skills first (type:'skill'), then workflows
  // (type:'workflow'), deduplicated by name. This mirrors the enrichment done in
  // sdk-message-processor for the Claude adapter path.
  // homeOverride (lr-3ccc78): session owner's home, see discoverClaudeSkills doc comment.
  function buildEnrichedSlashCommands(skillNames, cwd, homeOverride) {
    var effectiveCwd = cwd || _cwd;
    var seen = {};
    var combined = [];

    // Skills: enrich with descriptions from SKILL.md where available
    var fsSkillsMeta;
    try {
      fsSkillsMeta = discoverSkillsWithMeta(effectiveCwd, homeOverride);
    } catch (e) {
      fsSkillsMeta = [];
    }
    var skillMetaMap = {};
    for (var si = 0; si < fsSkillsMeta.length; si++) {
      skillMetaMap[fsSkillsMeta[si].name] = fsSkillsMeta[si];
    }
    ...
    for (var ni = 0; ni < names.length; ni++) {
      ...
      if (seen[name]) continue;
      seen[name] = true;
      var meta = skillMetaMap[name];
      combined.push({ name: name, desc: (meta && meta.description) || "", type: "skill" });
    }

    // Workflows: discovered from filesystem, deduplicated against skills by name
    var workflows;
    try {
      workflows = discoverWorkflows(effectiveCwd);
    } catch (e) {
      workflows = [];
    }
    for (var wi = 0; wi < workflows.length; wi++) {
      var wf = workflows[wi];
      if (seen[wf.name]) continue;
      seen[wf.name] = true;
      combined.push({ name: wf.name, desc: wf.description || "", type: "workflow" });
    }

    return combined;
  }

  function buildReadyResponse(skillNames, cwd, homeOverride) {
    return {
      models: _cachedModels,
      defaultModel: _defaultModel,
      skills: skillNames || [],
      slashCommands: buildEnrichedSlashCommands(skillNames, cwd, homeOverride),
      fastModeState: null,
      capabilities: {
        thinking: true,
        betas: false,
  ...
  var adapter = {
    ...
    init: function(initOpts) {
      ...
      if (_appServer && _appServer.started && _cachedModels.length > 0) {
        return Promise.resolve(buildReadyResponse([], _cwd));
      }
      ...
      _initPromise = (async function() {
        ...
        // Track 1: Read local MCP server definitions from ~/.clagentic/mcp.json
        ...
        await _appServer.send("initialize", {
          clientInfo: { name: "clagentic", title: "Clagentic: Console", version: "1.0.0" },
        ...
        // Query the app-server for the live model list. The v2 protocol exposes
        // "model/list" which returns rich Model objects with displayName, isDefault,
        // supportedReasoningEfforts, and upgrade (non-null when a newer model exists).
        // Fall back to CODEX_DEFAULT_MODELS if the call fails or returns nothing.
        try {
          var modelListResult = await _appServer.send("model/list", {}, 10000);
          // Response shape: { data: [Model, ...], nextCursor }
          if (modelListResult && Array.isArray(modelListResult.data) && modelListResult.data.length > 0) {
            _cachedModels = modelListResult.data
              .filter(function(m) { return !m.hidden; })
              .map(function(m) {
                var efforts = Array.isArray(m.supportedReasoningEfforts)
                  ? m.supportedReasoningEfforts.map(function(r) { return r.reasoningEffort; })
                  : CODEX_EFFORT_LEVELS;
                return {
                  value: m.model || m.id,
            ...
            // Derive default model: first isDefault entry, or first non-hidden entry
            var defaultEntry = null;
            for (var mi = 0; mi < modelListResult.data.length; mi++) {
              if (!modelListResult.data[mi].hidden && modelListResult.data[mi].isDefault) {
                defaultEntry = modelListResult.data[mi];
                break;
              }
            }
            _defaultModel = (defaultEntry && (defaultEntry.model || defaultEntry.id)) || _defaultModel;
            console.log("[codex] model/list: " + _cachedModels.length + " models, default=" + _defaultModel);
          } else {
            throw new Error("empty or missing data");
          }
        } catch (e) {
          console.warn("[codex] model/list failed (" + e.message + "); using built-in model table");
          _cachedModels = CODEX_DEFAULT_MODELS.slice();
          // _defaultModel stays "gpt-5.5" (set at init)
        }
        console.log("[codex] App-server initialized, models: " + _cachedModels.map(function(m) { return m.value; }).join(", "));

        // Discover skills: built-in Codex skills + Claude skills
        // lr-3ccc78 — resolve the session owner's home so a shared
        // multi-user daemon reports THIS user's skills, not the daemon
        // operator's. Falls back to REAL_HOME (config.js) when no
        // linuxUser was passed to init() — the single-user case.
        var skillNames = [];
        try {
          var initHomeOverride = resolveLinuxUserHome(effectiveInitOpts.linuxUser);
          var claudeSkillsHome = initHomeOverride;
          if (!claudeSkillsHome) {
            try { claudeSkillsHome = require("../../config").REAL_HOME; } catch (e) { claudeSkillsHome = require("os").homedir(); }
          }
          var claudeSkillsDir = require("path").join(claudeSkillsHome, ".claude", "skills");
          ...
          var claudeSkills = discoverClaudeSkills(_cwd, initHomeOverride);
        ...
        return buildReadyResponse(skillNames, _cwd, initHomeOverride);
      })();
    ...
    createQuery: async function(queryOpts) {
      ...
      var model = queryOpts.model || _defaultModel;
      ...
      var handleOpts = {
        ...
        // lr-3ccc78 — session owner's home for discoverClaudeSkills below,
        // resolved once per query so a shared multi-user daemon shows each
        // user their own ~/.claude/skills, not the daemon operator's.
        homeOverride: resolveLinuxUserHome(codexOpts.linuxUser),
      };
      ...
      // Tool policy -> approval mode. handleOpts.toolPolicy seeds
      // state.toolPolicy in createCodexQueryHandle so isAllowAll() (lr-f7a4)
      // is already correct on the very first approval request, not just
      // after a later setToolPolicy() call.
      handleOpts.toolPolicy = queryOpts.toolPolicy || null;
    ...
    generateTitle: async function(messages, opts) {
      ...
      var handle = await adapter.createQuery({
        ...
        model: resolveTitleModel(_defaultModel),
        abortController: ac,
...
module.exports = {
  ...
  _test_resolveModel: _test_resolveModel,
  // Exported so tests exercise the real title-generation model selection
  // (including the catalog-unavailable fallback) rather than reimplementing
  // a copy of it.
  _test_resolveTitleModel: resolveTitleModel,
  // lr-f7a4 — exported so tests can drive the real approval-routing and
  // setToolPolicy logic against a fake appServer, rather than reimplementing
  // a copy of handleServerEvent's approval branches inline.
  _test_createCodexQueryHandle: createCodexQueryHandle,
  // lr-3ccc78 — exported so tests can exercise the real per-user home
  // resolution in discoverClaudeSkills directly, rather than driving it
  // indirectly through a full query/init round trip.
  _test_discoverClaudeSkills: discoverClaudeSkills,
};
