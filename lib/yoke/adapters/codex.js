...
var { discoverSkillsWithMeta } = require("../../sdk-skill-discovery");
...
function discoverClaudeSkills(cwd, homeOverride) {
  ...
  for (var d = 0; d < dirs.length; d++) {
    ...
    var entries;
    try { entries = fs.readdirSync(resolvedBase, { withFileTypes: true }); } catch (e) { continue; }
    for (var i = 0; i < entries.length; i++) {
      var entry = entries[i];
      if (!entry.isDirectory() && !entry.isSymbolicLink()) continue;
      // Containment check (semgrep path-traversal): entry.name comes from
      // readdirSync, so a crafted/symlinked directory entry could otherwise
      // resolve outside resolvedBase. Reject anything that escapes it before
      // ever touching the filesystem again.
      var skillDir = path.resolve(resolvedBase, entry.name);
      if (skillDir !== resolvedBase && skillDir.indexOf(resolvedBase + path.sep) !== 0) continue;
