...
function createCodexQueryHandle(appServer, queryOpts) {
  ...
  var handle = {
    ...
    getContextUsage: function() {
      // lr-3af675: Codex's app-server has no equivalent of Claude's rich
      // context-usage API (categories/totalTokens/maxTokens breakdown) —
      // state.lastUsage is a flat turn-usage object (input_tokens etc), not
      // that shape. Returning it here used to make the frontend's
      // richContextUsage truthiness check pass, rendering a confident
      // "0 / 200K · 0%" popover built from data that was never a context-
      // window measurement at all. Returning null instead lets the frontend
      // fall through to its unknown/unsupported state, which is honest.
      // Consuming Codex's real per-turn model_context_window (turn/started)
      // is tracked separately (lr-872f94), not solved here.
