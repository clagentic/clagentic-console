...
function mergeStaticCatalog(models) {
  ...
  var merged = modelCatalog.mergeModelCatalog(models, catalog.models);
  // lr-f22787 (coordinator follow-up): applies the retired/deprecated
  // marking documented in model-catalog.js's header comment — a retired
  // catalog entry (e.g. claude-1.0) is disabled from selection rather than
  // rendering as an ordinary clickable row indistinguishable from a
  // currently-runnable version.
  return modelCatalog.applyRetiredMarking(merged);
}

// --- Alias-vs-concrete-ID reconciliation (lr-f22787 Finding 4) ---
//
// EMPIRICAL BUG: a bare vendor alias (value: "opus", no version digits in
// the ID itself) carries a `description` string ("Opus 4.8 with 1M
// context") that claudeDisplayName uses to derive a version-bearing label —
// so the picker row reads "Opus 4.8". But the row's VALUE is still the bare
// alias "opus", so clicking it sends {model:"opus"}, not a pinned ID. What
// "opus" actually resolves to server-side is whatever Anthropic's alias
// mapping says at call time — a moving target this codebase has no
// visibility into, and (per lore session engrams from this project) the
// operator's own recorded runs land on claude-opus-4-6, not a model
// matching "4.8" at all. So the description-derived label and the alias's
// real runtime resolution can disagree, and a user picking "Opus 4.8" has
// no way to know they are actually sending an unpinned alias.
//
// FIX: once the static catalog (mergeStaticCatalog above) has supplied real
// versioned IDs for a family, prefer sending one of THOSE concrete IDs over
// the bare alias whenever the family+version the alias's description claims
// matches a catalog entry exactly. This is a per-entry, non-destructive
// substitution — it only fires when parseClaudeModelVersion resolves the
// alias's description to a version that has an exact-match catalog ID
// available, and it never invents an ID that is not already present in the
// merged list. If no such match exists the alias entry is left untouched
// (still selectable, just still alias-shaped) — never silently dropped.
function reconcileAliasesWithCatalogIds(models) {
  var byFamilyVersion = {};
  for (var i = 0; i < models.length; i++) {
    var m = models[i];
    var value = (m && typeof m === "object") ? (m.value || "") : String(m || "");
    if (!value) continue;
    var parsedId = modelFamilies.parseClaudeModelVersion(value);
    // Only concrete IDs (version digits present IN THE ID ITSELF, not
    // borrowed from description) are trusted as reconciliation targets —
    // an alias resolving against another alias would just move the
    // ambiguity, not remove it.
    if (parsedId && parsedId.version.length > 0) {
      var key = parsedId.family + ":" + parsedId.version.join(".");
      byFamilyVersion[key] = value;
    }
  }
  ...
  for (var j = 0; j < models.length; j++) {
    var entry = models[j];
    var val = (entry && typeof entry === "object") ? (entry.value || "") : String(entry || "");
    var description = (entry && typeof entry === "object") ? entry.description : null;
    var idParsed = modelFamilies.parseClaudeModelVersion(val);
    var isBareAlias = idParsed && idParsed.version.length === 0 && description;
    if (isBareAlias) {
      var withDesc = modelFamilies.parseClaudeModelVersion(val, description);
      if (withDesc && withDesc.version.length > 0) {
        var lookupKey = withDesc.family + ":" + withDesc.version.join(".");
        var concreteId = byFamilyVersion[lookupKey];
        if (concreteId && concreteId !== val) {
          // Substitute the alias's value with the matching concrete ID so
          // the picker row's click handler sends the pinned ID — the label
          // it already displays ("Opus 4.8") is now what actually runs.
          var substituted = (entry && typeof entry === "object") ? Object.assign({}, entry) : {};
          substituted.value = concreteId;
...
// lr-f22787: the real adapter-facing entry point. Merges the release-time
// static catalog and reconciles bare aliases against it BEFORE handing off
// to the pure enrichClaudeModels above, so catalog-only entries still flow
// through identical enrichment/tiering logic — just via composition instead
// of a filesystem read baked into the pure function itself.
function enrichClaudeModelsWithCatalog(models) {
  if (!models || !models.length) return models;
  var withCatalog = mergeStaticCatalog(models);
  var reconciled = reconcileAliasesWithCatalogIds(withCatalog);
  return enrichClaudeModels(reconciled);
}
...
module.exports = {
  ...
  // Test seams for lr-f22787: static catalog merge + alias/description
  // reconciliation, exercised directly without touching the filesystem-
  // backed catalog loader or spinning up the SDK.
  _test_mergeStaticCatalog: mergeStaticCatalog,
  _test_reconcileAliasesWithCatalogIds: reconcileAliasesWithCatalogIds,
