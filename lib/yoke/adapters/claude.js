...
function flattenEvent(raw) {
  ...
  if (raw.type === "stream_event" && raw.event) {
    ...
    if (evt.type === "message_start") {
      ...
      if (evt.message && evt.message.usage) {
        ...
        // cache_creation_input_tokens are disjoint (Anthropic prompt-caching
        // docs) and all three occupy the context window. The input_tokens +
        // cache_read_input_tokens sum was correct as far as it went;
        // cache_creation_input_tokens was simply omitted, under-reporting
        // context fill on cache-write-heavy turns.
        base.inputTokens = (u.input_tokens || 0) + (u.cache_read_input_tokens || 0) + (u.cache_creation_input_tokens || 0);
      }
      return base;
    }
  ...
  if (raw.type === "system") {
    ...
    if (raw.subtype === "task_started") {
      ...
      // lr-1317b8: parity fields (sdk.d.ts:4060-4082), not yet rendered
      // anywhere (frontend derivation lands in a later task) — carried
      // through now as substrate for CLI-parity labels (e.g. subagent_type
      // is the data behind "PRAX researching...").
      base.subagentType = raw.subagent_type || null;
      base.taskType = raw.task_type || null;
      base.workflowName = raw.workflow_name || null;
      base.skipTranscript = !!raw.skip_transcript;
      return base;
    }
    if (raw.subtype === "task_progress") {
      ...
      // lr-1317b8: parity field (sdk.d.ts:4038-4058).
      base.subagentType = raw.subagent_type || null;
      return base;
    }
    // lr-1317b8 (Defect 3): the SDK emits task_notification as a SYSTEM
    // SUBTYPE (sdk.d.ts:4020-4036), not a top-level message type. The old
    // top-level raw.type === "task_notification" check below never matched
    // anything real, so this event fell through to the generic catch-all,
    // was silently relabeled yokeType:"system", and the drain path in
    // sdk-message-processor.js (subagent_done + taskIdMap cleanup) never
    // ran on the Claude path. See lr-255e comment #8.
    if (raw.subtype === "task_notification") {
      ...
      base.usage = raw.usage || null;
      // lr-1317b8: parity field (sdk.d.ts:4033).
      base.skipTranscript = !!raw.skip_transcript;
      return base;
    }
    // lr-1317b8 (Defect 4): task_updated was branched on by
    // sdk-message-processor.js but no adapter ever produced it — the SDK
    // emits it as {type:'system', subtype:'task_updated'} (sdk.d.ts:4084-
    // 4101) carrying a wire-safe patch (status/description/end_time/
    // total_paused_ms/error/is_backgrounded). Without this branch,
    // updateSubagentTaskStatus's failed/killed drain (tools.js:2152-2154)
    // was unreachable.
  ...
  // Note: task_notification has no top-level raw.type — the SDK emits it as
  // {type:'system', subtype:'task_notification'} (sdk.d.ts:4020-4036), and
  // that case is handled in the system-events branch above (lr-1317b8).

  // --- tool_progress ---
  // lr-1317b8 (Defect 1): SDKToolProgressMessage (sdk.d.ts:4115-4124) has NO
  // `content` field — it carries tool_use_id, tool_name, parent_tool_use_id,
  // elapsed_time_seconds, task_id. The old `raw.content || ""` read was
  // always undefined, so text was always "". Downstream, subagent_activity
  // was emitted with text:"" (sdk-message-processor.js), updateSubagentActivity
  // called ctx.setActivity(""), and "" is falsy, so setActivity took its
  // clear branch and destroyed the indicator — a raise-site that was
  // secretly a clear-site. Fix: derive text from tool_name + elapsed_time_seconds.
  if (raw.type === "tool_progress") {
    base.yokeType = "tool_progress";
    base.parentToolId = raw.parent_tool_use_id;
    base.toolName = raw.tool_name || null;
    base.elapsedTimeSeconds = typeof raw.elapsed_time_seconds === "number" ? raw.elapsed_time_seconds : null;
    base.taskId = raw.task_id || null;
    if (raw.tool_name && typeof raw.elapsed_time_seconds === "number") {
      base.text = raw.tool_name + " (" + Math.round(raw.elapsed_time_seconds) + "s)";
    } else if (raw.tool_name) {
      base.text = raw.tool_name;
    } else {
      base.text = "";
    }
    return base;
