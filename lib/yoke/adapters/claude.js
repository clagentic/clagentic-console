 ...
 * family-substring heuristic ONLY when the vendor field is genuinely absent.
 *
...
function enrichClaudeModel(model) {
  ...
  if (model && typeof model === "object") {
    ...
    var enriched = {
      ...
      // supportedEffortLevels: reported list wins outright when present and
      // non-empty; CLAUDE_EFFORT_LEVELS is the documented fallback-only
      // static table (lr-af9d66 tertiary item — see its declaration above).
      supportedEffortLevels: (model.supportedEffortLevels && model.supportedEffortLevels.length > 0)
