...
// --- Model enrichment ---
// Maps raw string model IDs returned by stream.supportedModels() into rich
// objects with displayName, supportsEffort, supportedEffortLevels, and
// supportsThinking capability flags. The downstream model_info WS message
// carries these objects so the UI can drive visibility without vendor checks.

var CLAUDE_EFFORT_LEVELS = ["low", "medium", "high", "xhigh", "max"];

// Canonical display-name table. Keys are substrings of the model ID (lowercased).
// Order matters: more-specific keys (e.g. "opus-4") must precede family keys (e.g. "opus").
var CLAUDE_MODEL_DISPLAY_NAMES = [
  // Opus 4.x family
  { key: "claude-opus-4-5",     label: "Opus 4.5" },
  { key: "claude-opus-4-6",     label: "Opus 4.6" },
  { key: "claude-opus-4",       label: "Opus 4" },
  { key: "claude-opus-3-7",     label: "Opus 3.7" },
  { key: "claude-opus-3-5",     label: "Opus 3.5" },
  { key: "claude-opus-3",       label: "Opus 3" },
  // Sonnet 4.x
  { key: "claude-sonnet-4-5",   label: "Sonnet 4.5" },
  { key: "claude-sonnet-4-6",   label: "Sonnet 4.6" },
  { key: "claude-sonnet-4",     label: "Sonnet 4" },
  { key: "claude-sonnet-3-7",   label: "Sonnet 3.7" },
  { key: "claude-sonnet-3-5",   label: "Sonnet 3.5" },
  { key: "claude-sonnet-3",     label: "Sonnet 3" },
  // Haiku 4.x
  { key: "claude-haiku-3-5",    label: "Haiku 3.5" },
  { key: "claude-haiku-3",      label: "Haiku 3" },
...
function claudeDisplayName(modelId) {
  ...
  // Strip pinned-version suffix like "[1m]"
  var clean = lower.replace(/\[.*?\]$/, "").trim();
  for (var i = 0; i < CLAUDE_MODEL_DISPLAY_NAMES.length; i++) {
    if (clean.indexOf(CLAUDE_MODEL_DISPLAY_NAMES[i].key) !== -1) {
      return CLAUDE_MODEL_DISPLAY_NAMES[i].label;
    }
  }
...
function claudeModelSupportsEffort(modelId) {
  var lower = (modelId || "").toLowerCase();
  if (lower.indexOf("haiku") !== -1) return false;
  return true;
}
...
function enrichClaudeModel(model) {
  ...
  if (model && typeof model === "object") {
    ...
    var enriched = {
      value: v,
      displayName: model.displayName || claudeDisplayName(v),
      supportsEffort: Object.prototype.hasOwnProperty.call(model, "supportsEffort") ? model.supportsEffort : claudeModelSupportsEffort(v),
      supportedEffortLevels: model.supportedEffortLevels || CLAUDE_EFFORT_LEVELS,
      supportsThinking: Object.prototype.hasOwnProperty.call(model, "supportsThinking") ? model.supportsThinking : claudeModelSupportsThinking(v),
    };
...
// pattern (family substring + numeric version ordering), same shape as
// CLAUDE_MODEL_DISPLAY_NAMES above but comparison-based instead of a lookup
// table. A model release that lands with a higher version number in an
// already-known family, or as a brand-new family, is automatically "latest"
// with zero code changes here.
//
// lr-d91ecf (sibling chore, hardcoded-model-metadata purge) will consolidate
// shared family/version parsing across this file and settings-defaults.js/
// app-panels.js's duplicated substring family detection — this is the shared
// location it should build on; do not add a third copy elsewhere.

var CLAUDE_MODEL_FAMILIES = ["opus", "sonnet", "haiku"];
 ...
 */
function parseClaudeModelVersion(modelId) {
  var lower = (modelId || "").toLowerCase();
        ...
        for await (var msg of stream) {
          if (msg.type === "system" && msg.subtype === "init") {
            ...
            try {
              ...
              result.models = enrichClaudeModels(models || []);
    ...
    return new Promise(function(resolve, reject) {
      ...
      worker.onMessage(function(msg) {
        if (msg.type === "warmup_done" && !warmupDone) {
          warmupDone = true;
          var r = msg.result || {};
          var enrichedModels = enrichClaudeModels(r.models || []);
