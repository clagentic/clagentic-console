...
/**
 * Sanitize a Clagentic: Console username into a valid Linux username.
...
/**
 * Provision a Linux user account for a Clagentic: Console user.
...
function provisionLinuxUser(clayUsername) {
  ...
  if (linuxUserExists(linuxName)) {
    // Check if this is a managed Linux user (reuse it)
  ...
  try {
    ...
    console.log("[os-users] Provisioned Linux user: " + linuxName + " (username: " + clayUsername + ")");
    installClaudeCli(linuxName);
    audit.log("osuser.create", {
      actorId: "system",
      metadata: { username: clayUsername, linuxUser: linuxName },
    });
    return { ok: true, linuxUser: linuxName };
  } catch (e) {
    var msg = (e.stderr || e.message || "").trim();
...
/**
 * Provision Linux accounts for all users that don't have one yet.
...
function provisionAllUsers(usersModule) {
  ...
  for (var i = 0; i < users.length; i++) {
    ...
    if (provision.ok) {
      // Update the user record with the Linux username
...
/**
 * Grant ACL access on a project directory to ALL users with linuxUser mappings.
...
function ensureProjectsDir() {
  var dir = "/var/clagentic/projects";
