      ...
      Promise.all(tabPromises).then(function(results) {
        ...
        if (screenshotImages.length > 0) {
          ...
          for (var si = 0; si < screenshotImages.length; si++) {
            ...
            // (unlike every other .history.push site in this file), so
            // sm.saveSessionFile()'s historyMatchesDisk() length check below
            // will detect the mismatch and still do a full rewrite.
