...
var { CONFIG_DIR, REAL_HOME } = require("./config");

// Lite home: <CONFIG_DIR>/lite/ — mirrors the path contract established in lr-dc6c.
// CONFIG_DIR is the canonical clagentic data directory (resolves CLAGENTIC_HOME,
// sudo users, and legacy ~/.clay/ migration the same way as every other daemon path).
function getLiteHome() {
  return path.join(CONFIG_DIR, "lite");
}
