...
// Identity header: X-Relay-Caller-Lead: "andy" (operator bypass).
//
      ...
      res.on("end", function () {
        ...
        if (res.statusCode !== 200) {
          ...
          try {
            ...
            if (parsed.message) msg = parsed.error + ": " + parsed.message;
          } catch (e) {}
...
//
// Map from session.localId -> { agentName, claims: Map<convId, claimId>, timer }
// where timer is the heartbeat setInterval handle.

var _sessionClaims = new Map(); // localId -> { agentName, claims, timer }
...
// 3. For each open conv: registers a live claim (catches 409 silently).
// 4. Starts heartbeat loop.
// 5. Surfaces open conv list to operator via sendToSessionFn (system message).
//    Does NOT auto-claim new conversations here — that is handled by onNewConversation.
//
...
function attachAgentSession(session, agentName, sendToSessionFn) {
  ...
  isRelayReachable().then(function (reachable) {
    ...
    // Surface open conversations to operator (display-only, per task spec).
    if (openConvs.length > 0 && typeof sendToSessionFn === "function") {
      var lines = ["Open conversations where " + agentName + " is a participant:"];
      ...
      lines.push("To claim: clagentic claim register --conv <id> --participant " + agentName);
      sendToSessionFn({
        type: "system_message",
        text: lines.join("\n"),
        role: "system",
    ...
    // Register a live claim for each open conversation.
    var sessionLabel = "console-ui-" + agentName + "-" + Date.now();
    var registerPromises = openConvs.map(function (c) {
      ...
      return registerClaim(agentName, convId, sessionLabel).then(function (claimId) {
        ...
        console.log("[relay-agent-claims] claimed conv " + convId.substring(0, 8) + " for session " + localId + " (" + agentName + ")");
      });
    });
    ...
    Promise.all(registerPromises).then(function () {
      var s = _sessionClaims.get(localId);
      if (!s) return;
      // Start heartbeat loop (even if no claims yet — new convs may be added).
...
// Releases all live claims and stops the heartbeat loop.
