...
function attachTeam(ctx) {
  ...
  function activateTeam(teamId, cliSessionId, cwd) {
    ...
    // sessionsBase must match lib/sessions.js which reads from CONFIG_DIR/sessions.
    // The old path (REAL_HOME/.clagentic/sessions) was a bug: team tails read from a
    // different directory than where sessions were actually written. (lr-6204)
    if (cliSessionId && cwd) {
