...
function dmFilePath(key) {
  ...
  var resolved = path.join(DM_DIR, key.replace(/:/g, "_") + ".jsonl");
  // Defense-in-depth: assert the resolved path stays within DM_DIR.
  // path.resolve is used to canonicalize both sides before comparison so that
  // any surviving .. segments (e.g. from a malformed key that slipped through
  // caller-side validation) cannot escape the directory.
  var dmDirResolved = path.resolve(DM_DIR);
  if (!path.resolve(resolved).startsWith(dmDirResolved + path.sep) &&
      path.resolve(resolved) !== dmDirResolved) {
    throw new Error("dm: path traversal detected for key: " + key);
  }
  return resolved;
}
