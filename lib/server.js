  ...
  var appHandler = function (req, res) {
    ...
    if (req.method === "GET" && fullUrl === "/api/me") {
      if (!users.isMultiUser()) {
        res.writeHead(200, { "Content-Type": "application/json" });
        var singleUserData = { multiUser: false, singleUserMigrationAvailable: !!pinHash };
        res.end(JSON.stringify(singleUserData));
        return;
      }
        ...
        projects.forEach(function (pCtx, pSlug) {
          ...
          pSm.sessions.forEach(function (s) {
            if (!s.hidden && s.history && s.history.length > 0) {
              ...
              s._projectSlug = pSlug;
              s._projectIcon = projectIcon;
              allSessions.push(s);
            }
          });
        });
