...
function createServer(opts) {
  ...
  // --- WebSocket ---
  // handleProtocols echoes back the offered ticket subprotocol on a
  // ticket-authed upgrade (WS spec requires the 101 response to echo an
  // accepted subprotocol when one was offered). req._clayAcceptedProtocol is
  // set by the upgrade handler below after it validates the ticket, BEFORE
  ...
  server.on("upgrade", function (req, socket, head) {
    ...
    var offeredTicket = null;
    if (!wsCookieUser) {
      offeredTicket = extractWsTicketFromHeader(req.headers["sec-websocket-protocol"]);
      if (offeredTicket) wsTicketUser = auth.consumeWsTicket(offeredTicket);
    }
    ...
    if (wsTicketUser) {
      // Echo the accepted ticket subprotocol back in the 101 response (WS
      // spec requires the server to select one of the offered subprotocols
      // when the client lists any). See the WebSocketServer handleProtocols
      // hook above.
