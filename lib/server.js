...
function createServer(opts) {
  ...
  var appHandler = function (req, res) {
    ...
    if (fullUrl === "/setup" && req.method === "GET") {
      ...
      // lr-20e71c: tlsOptions alone is daemon-internal-TLS-only and reports a
      // false http:// link behind a TLS-terminating reverse proxy — resolve
      // the real external protocol for this request instead.
      var protocol = resolveEffectiveProtocol({
        tlsOptions: tlsOptions,
    ...
    if (fullUrl === "/pwa" && req.method === "GET") {
      var host = req.headers.host || "localhost";
      var hostname = host.split(":")[0];
      // lr-20e71c: same effective-protocol resolution as /setup above.
      var protocol = resolveEffectiveProtocol({
        tlsOptions: tlsOptions,
        trustedProxy: trustedProxy,
        forwardedProtoHeader: req.headers["x-forwarded-proto"],
    ...
    // lr-20e71c: tlsOptions alone under-reports HTTPS behind a trusted
    // TLS-terminating proxy (Caddy) — resolve the effective protocol so the
    // Secure attribute is set whenever the browser actually sees HTTPS, not
    // only when the daemon itself terminates TLS.
    var cookieEffectiveProtocol = resolveEffectiveProtocol({
      tlsOptions: tlsOptions,
      trustedProxy: trustedProxy,
      forwardedProtoHeader: req.headers["x-forwarded-proto"],
    }).protocol;
    res.setHeader("Set-Cookie", "clagentic_last_project=" + slug + "; Path=/; SameSite=Strict; Max-Age=31536000" + (cookieEffectiveProtocol === "https" ? "; Secure" : ""));
    ...
    req._clagenticUser = getMultiUserFromReq(req);
  ...
  // subprotocol when one was offered, on any auth path. req._clagenticAcceptedProtocol
  ...
  var wss = new WebSocketServer({
    ...
    handleProtocols: function (protocols, req) {
      if (req._clagenticAcceptedProtocol && protocols.has(req._clagenticAcceptedProtocol)) {
        return req._clagenticAcceptedProtocol;
      }
      return false;
    },
  });
  ...
  server.on("upgrade", function (req, socket, head) {
    ...
    wss.handleUpgrade(req, socket, head, function (ws) {
      ...
      ws._clagenticUser = wsUser; // attach user context
      var remoteAddr = req.socket.remoteAddress || "";
      ws._clagenticLocal = (remoteAddr === "127.0.0.1" || remoteAddr === "::1" || remoteAddr === "::ffff:127.0.0.1");
      ...
      ws._clagenticEffectiveProtocol = resolveEffectiveProtocol({
        tlsOptions: tlsOptions,
        trustedProxy: trustedProxy,
        forwardedProtoHeader: req.headers["x-forwarded-proto"],
      });
  ...
  // unread is tracked per-session via ws._clagenticUnread[localId] (the existing
  ...
  function getSessionUnread(ws, ownSlug, sessSlug, localId) {
    if (sessSlug === ownSlug) {
      var ownMap = ws._clagenticUnread || {};
  ...
  // same-project ws._clagenticUnread zeroing in switchSession. If the session that
        ...
        ctx.forEachClient(function (ws) {
          ...
          if (ws.readyState === 1) {
            ws.send(JSON.stringify({
              ...
              removedProjects: getRemovedProjects(ws._clagenticUser ? ws._clagenticUser.id : null),
  ...
  function addProject(cwd, slug, title, icon, projectOwnerId, worktreeMeta, extraOpts) {
    ...
    var ctx = createProjectContext({
      ...
      // ws._clagenticUser can ever be set, including in single-user/PIN mode
      ...
      // this must distinguish "own project" (ws._clagenticUnread) from
  ...
  function broadcastPresenceChange() {
    ...
    presenceTimer = setTimeout(function () {
      ...
      projects.forEach(function (ctx) {
        ctx.forEachClient(function (ws) {
          var userId = ws._clagenticUser ? ws._clagenticUser.id : null;
  ...
  function sendToUser(userId, msg) {
    ...
    projects.forEach(function (ctx) {
      ...
      ctx.forEachClient(function (ws) {
        if (ws._clagenticUser && ws._clagenticUser.id === userId && ws.readyState === 1) {
          ws.send(data);
        }
      });
    });
