    ...
    var ctx = createProjectContext({
      ...
      getAllProjectSessions: function (includeSelf, userId) {
        ...
        projects.forEach(function (pCtx, pSlug) {
          ...
          // per-session inner loop below, not re-resolved per session) and
          // skip the whole project up front if the user cannot access it at
          // all -- avoids an O(projects * sessions) repeated-resolution
          // pattern for a value that does not vary per session.
