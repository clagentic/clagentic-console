...
// Two-stage containment check mirroring project.js's safePath: a directory
// entry name from readdirSync can be a symlink, so a lexical path.resolve
// check alone (entry.name embedded in a path string that lexically stays
// under base) is NOT sufficient — the subsequent fs.accessSync/readFileSync
// on SKILL.md follows the symlink at read time, escaping base entirely.
// Stage 1 (lexical) rejects a literal ".." style escape before any syscall.
// Stage 2 (fs.realpathSync) resolves the symlink and re-checks containment
// against the REAL target, which is the check that actually defeats a
// symlinked directory entry pointing outside base. Returns the resolved
// real path on success, or null if entry escapes base or doesn't exist.
function resolveContainedSkillDir(base, entryName) {
  var resolvedBase = path.resolve(base);
  var lexical = path.resolve(resolvedBase, entryName);
  if (lexical !== resolvedBase && lexical.indexOf(resolvedBase + path.sep) !== 0) return null;
  try {
    var real = fs.realpathSync(lexical);
    if (real !== resolvedBase && real.indexOf(resolvedBase + path.sep) !== 0) return null;
    return real;
  } catch (e) {
    return null;
  }
...
// not the daemon process's REAL_HOME (lr-3ccc78). Falls back to REAL_HOME when
// absent — the single-user / no-OS-isolation case, unchanged behavior.
function discoverSkillsWithMeta(cwd, homeOverride) {
  var globalHome = homeOverride || require("./config").REAL_HOME;
  var dirs = [
...
function attachSkillDiscovery(ctx) {
  ...
  // homeOverride: optional per-call override (see discoverSkillsWithMeta
  // above) so a caller can resolve the session owner's home instead of the
  // daemon's REAL_HOME on a shared multi-user daemon (lr-3ccc78).
  function discoverSkillDirs(homeOverride) {
    var skills = {};
    var globalHome = homeOverride || require("./config").REAL_HOME;
    var dirs = [
      path.join(globalHome, ".claude", "skills"),
...
module.exports = {
  ...
  // Exported so other skill-discovery consumers (yoke/adapters/codex.js)
  // reuse the same two-stage (lexical + realpath) containment check instead
  // of approximating it — see the function's own doc comment.
  resolveContainedSkillDir: resolveContainedSkillDir,
};
