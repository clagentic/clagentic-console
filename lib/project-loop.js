...
function attachLoop(ctx) {
  ...
  function handleLoopMessage(ws, msg) {
    ...
    if (msg.type === "ralph_wizard_complete") {
      ...
      // Default: "draft" mode — crafts files via the clay-ralph skill
    ...
    if (msg.type === "hub_recent_sessions_list") {
      ...
      // Augment with slug + icon from the server-side closure, which doesn't set those
      // (getAllProjectSessions in server.js sets _projectTitle but not _projectSlug/_projectIcon).
      // We re-hydrate those by re-iterating — but since we only have cross-project sessions here,
      // just add self sessions inline with self metadata.
