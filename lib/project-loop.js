...
function attachLoop(ctx) {
  ...
  function handleLoopMessage(ws, msg) {
    ...
    if (msg.type === "hub_recent_sessions_list") {
      // Other-project sessions (already have _projectTitle set by getAllProjectSessions)
      // getAllProjectSessions returns annotated shallow copies (not live objects).
      // Each copy carries _projectTitle, _projectSlug, _projectIcon from server.js.
      // We then append the current project's own sessions below with self metadata.
      var otherSessions = getAllProjectSessions ? getAllProjectSessions() : [];
      var selfStatus = getStatus ? getStatus() : {};
      var selfSlug = selfStatus.slug || slug;
      var selfTitle = selfStatus.title || selfStatus.project || selfSlug;
      var selfIcon = selfStatus.icon || null;
      var allSessions = otherSessions.slice();
      sm.sessions.forEach(function (s) {
        if (!s.hidden) {
          allSessions.push(Object.assign({}, s, {
            _projectSlug: selfSlug,
            _projectTitle: selfTitle,
            _projectIcon: selfIcon,
          }));
        }
      });
      ...
      var top10 = allSessions.slice(0, 10);
      var mapped = top10.map(function (s) {
        return {
          id: s.localId,
          title: s.title || "New Session",
          lastActivity: s.lastActivity || s.createdAt || 0,
          projectSlug: s._projectSlug || selfSlug,
          projectTitle: s._projectTitle || selfTitle,
