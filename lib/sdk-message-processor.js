  ...
  function processSDKMessage(session, parsed) {
    ...
    if (parsed.yokeType === "turn_start") {
      ...
      if (session.turnCount >= AUTO_TITLE_TURN_THRESHOLD
        ...
        // lr-efdab6 (DRUMMER decision, comment #1): the taken branch above
        // already logs on every path (sdk-bridge.js [auto-title] sites) — but
        // logging existed on NOTHING that recorded the gate NOT firing, so
        // silence read identically for "ran fine and logged nothing notable",
        // "never invoked", and "invoked and died before the first log". That
        // ambiguity is exactly what let lr-62157d's dead auto-title feature
        // survive ~3 months / ~160 sessions undetected. This branch makes the
        // negative case unambiguous: every turn where the gate evaluates
        // false gets exactly one line naming why.
        //
        // already_auto_generated is the expected steady-state reason for the
        // overwhelming majority of turns in a mature session (fires every
        // turn after the session's title is set) and is logged at a lower
        // level (console.debug) so a normal journald view isn't dominated by
        // it; every other reason is diagnostic of a gate that should be
        // firing but isn't, and stays at console.log so it is visible by
        // default. Nothing here transmits session content — only flags/counts.
        var _autoTitleReason;
        if (!onAutoTitle) {
          _autoTitleReason = "no_callback";
        } else if (session.loop) {
          _autoTitleReason = "loop_running";
        } else if (session.titleManuallySet) {
          _autoTitleReason = "manually_set";
        } else if (session.titleAutoGenerated) {
          _autoTitleReason = "already_auto_generated";
        } else if (session.turnCount < AUTO_TITLE_TURN_THRESHOLD) {
          _autoTitleReason = "turn_threshold_not_met";
        } else {
          _autoTitleReason = "unknown";
        }
        var _autoTitleLogFn = (_autoTitleReason === "already_auto_generated") ? console.debug : console.log;
        _autoTitleLogFn(
          "[auto-title] gate did not fire: reason=" + _autoTitleReason
      ...
      var parentId = parsed.parentToolId;
      // lr-b75006: compute this BEFORE any cleanup below so both the
      // activeTaskToolIds release and the subagentToolOwners prune (further
      // down) can consult the SAME live-ownership snapshot. Previously only
      // subagentToolOwners was guarded against the race described below;
      // activeTaskToolIds was released unconditionally a few lines earlier,
      // which silently reintroduced the identical hazard through the other
      // half of partitionSubagentOwnedPermissions's check (lib/sdk-permission-
      // ownership.js: `owningTaskId && activeTaskToolIds[owningTaskId]`) --
      // see the exception note below for the full mechanism.
      ...
      var parentOwnsLivePermission = false;
      if (parentId && session.subagentToolOwners) {
        for (var polk in session.subagentToolOwners) {
          if (session.subagentToolOwners[polk] === parentId && liveOwnedToolIds[polk]) {
            parentOwnsLivePermission = true;
            break;
          }
        }
      }
      if (parentId) {
        ...
        //
        // lr-b75006 EXCEPTION: do not release activeTaskToolIds[parentId]
        // while the sub-agent's own permission request (e.g. its last Bash
        // call) is still awaiting the operator's click. A later 'result' (or
        // processQueryStream's finally) decides whether to preserve that
        // pending permission via partitionSubagentOwnedPermissions, which
        // requires activeTaskToolIds[owningTaskId] to still be truthy. If
        // this fired unconditionally here, the permission would be force-
        // denied and purged by that later cleanup while the UI card is still
        // rendered and clickable -- every subsequent Allow/Allow-for-Session/
        // Deny click then finds no pendingPermissions entry to resolve
        // (project-sessions.js's permission_response handler no-ops on a
        // missing `pending`), reproducing the exact "clicks don't stick"
        // symptom lr-9d4b's sibling subagentToolOwners guard (below) was
        // already written to prevent -- just via activeTaskToolIds instead.
        if (!parentOwnsLivePermission) {
