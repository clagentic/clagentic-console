...
function attachMessageProcessor(ctx) {
  ...
  // session.vendor === "codex" so Claude's existing behavior (which already
  // has turn-wide isProcessing coverage for ordinary tools, and only acquires
  // a token for backgrounded Task tools) is completely unchanged by this.
  //
    ...
    if (parsed.yokeType === "turn_start") {
      ...
      if (block && block.type === "tool_use") {
        ...
        if (block.name === "Task") {
          ...
          // lr-9bcd7b: acquire an activity token for the lifetime of this
          // backgrounded Task — released at the tool_result / task_notification
          // drain paths below (whichever fires first for this same block.id).
          acquireActivity(session, block.id, { source: "task", label: input.description || "" });
        }
      ...
      if (Array.isArray(content)) {
        for (var i = 0; i < content.length; i++) {
          ...
          if (block.type === "tool_result" && !session.sentToolResults[block.tool_use_id]) {
            ...
            // lr-9bcd7b: release the activity token acquired at block_stop for
            // this tool id, unconditionally -- NOT gated on activeTaskToolIds
            // still containing the entry. The 'result' handler's pre-existing
            // lr-9d4b cleanup (retainPreservedTaskBookkeeping) already trims
