...
function attachMessageProcessor(ctx) {
  ...
  function processSDKMessage(session, parsed) {
    ...
    if (parsed.yokeType === "turn_start") {
      ...
      // Ownership is determined via session.subagentToolOwners (tool-use id ->
      // owning Task's tool id, recorded in processSubagentMessage above) cross-
      // referenced against session.activeTaskToolIds (still-live Task ids).
      ...
      var keptPermissions = {};
      var permissionIds = Object.keys(session.pendingPermissions);
      for (var ppi = 0; ppi < permissionIds.length; ppi++) {
        var pid = permissionIds[ppi];
        var pentry = session.pendingPermissions[pid];
        var owningTaskId = pentry && pentry.toolUseId && session.subagentToolOwners
          ? session.subagentToolOwners[pentry.toolUseId]
          : null;
        if (owningTaskId && session.activeTaskToolIds && session.activeTaskToolIds[owningTaskId]) {
          keptPermissions[pid] = pentry;
        }
      }
      session.pendingPermissions = keptPermissions;
      ...
      for (var kpi in keptPermissions) {
        var kpEntry = keptPermissions[kpi];
        var kpTaskId = session.subagentToolOwners[kpEntry.toolUseId];
      ...
      if (parentId && session.subagentToolOwners) {
        for (var stk in session.subagentToolOwners) {
          if (session.subagentToolOwners[stk] === parentId) delete session.subagentToolOwners[stk];
        }
      }
