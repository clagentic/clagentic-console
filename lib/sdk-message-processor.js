        ...
        for (var i = 0; i < content.length; i++) {
          ...
          if (block.type === "tool_result" && !session.sentToolResults[block.tool_use_id]) {
            ...
            if (session.activeTaskToolIds && session.activeTaskToolIds[block.tool_use_id]) {
              ...
              // lr-9bcd7b: release the activity token acquired at block_stop.
              // Idempotent if task_notification already released the same
              // token first (whichever terminal path fires first wins).
