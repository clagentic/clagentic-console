  ...
  function processSDKMessage(session, parsed) {
    ...
    if (parsed.yokeType === "init") {
      ...
      if (parsed.slashCommands) {
        // Build enriched {name, desc, type} objects from three sources.
        // Priority per name:
        //   1. sm.skillMeta entry  → desc from metadata, type:'skill'
        //   2. sm.workflowMeta entry → desc from metadata, type:'workflow'
        //   3. CLI-emitted name only → desc:'', type:'builtin'
        // Deduplication: skillMeta + workflowMeta win over CLI-name-only.
        var _skillMetaMap = {};
        var _skillMetaArr = sm.skillMeta || [];
        for (var k = 0; k < _skillMetaArr.length; k++) {
          _skillMetaMap[_skillMetaArr[k].name] = _skillMetaArr[k];
        }
        var _workflowMetaArr = sm.workflowMeta || [];
        var _workflowMetaMap = {};
        for (var k = 0; k < _workflowMetaArr.length; k++) {
          _workflowMetaMap[_workflowMetaArr[k].name] = _workflowMetaArr[k];
        }

        var seen = new Set();
        var combined = [];

        // Highest priority: enriched skill metadata
        for (var k = 0; k < _skillMetaArr.length; k++) {
          var _sm = _skillMetaArr[k];
          if (!seen.has(_sm.name)) {
            seen.add(_sm.name);
            combined.push({ name: _sm.name, desc: _sm.description || "", type: "skill" });
          }
        }
        // Second priority: enriched workflow metadata
        for (var k = 0; k < _workflowMetaArr.length; k++) {
          var _wm = _workflowMetaArr[k];
          if (!seen.has(_wm.name)) {
            seen.add(_wm.name);
            combined.push({ name: _wm.name, desc: _wm.description || "", type: "workflow" });
          }
        }
        // Lowest priority: CLI-emitted names not covered above — treat as builtins
        var _cliNames = parsed.slashCommands;
        for (var k = 0; k < _cliNames.length; k++) {
          var _cliEntry = _cliNames[k];
          var _cliName = typeof _cliEntry === "string" ? _cliEntry : _cliEntry.name;
          if (_cliName && !seen.has(_cliName)) {
            seen.add(_cliName);
            combined.push({ name: _cliName, desc: "", type: "builtin" });
          }
        }
    ...
    if (parsed.yokeType === "turn_start") {
      ...
      if (block && block.type === "tool_use") {
        ...
        if (pushModule && block.name === "AskUserQuestion" && input.questions) {
          ...
          if (usersModule.isMultiUser() && session.ownerId && pushModule.sendPushToUser) {
