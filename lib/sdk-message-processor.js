...
function attachMessageProcessor(ctx) {
  ...
  function processSDKMessage(session, parsed) {
    ...
    if (parsed.yokeType === "turn_start") {
      ...
      // Accumulate server-side cumulative usage for accurate history_done replay
      if (!session.cumulativeUsage) {
        session.cumulativeUsage = { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, turns: 0 };
      }
      if (parsed.usage) {
        session.cumulativeUsage.input      += parsed.usage.input_tokens  || parsed.usage.inputTokens  || 0;
        session.cumulativeUsage.output     += parsed.usage.output_tokens || parsed.usage.outputTokens || 0;
        session.cumulativeUsage.cacheRead  += parsed.usage.cache_read_input_tokens  || parsed.usage.cacheReadInputTokens  || 0;
        session.cumulativeUsage.cacheWrite += parsed.usage.cache_creation_input_tokens || parsed.usage.cacheCreationInputTokens || 0;
      }
      session.cumulativeUsage.turns++;
