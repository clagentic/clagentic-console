...
// Action-string convention (see lib/audit.js and lib/server-admin.js):
// "resource.verb", dot-separated. Logged on BOTH success and failure paths
// for custom-emoji upload/delete — the class of bug this closes (lr-fc71)
// is a silent no-op that left zero runtime trace and required a live
// browser repro to diagnose (MILLER bounce off PR #303).
var CUSTOM_EMOJI_UPLOAD_ACTION = "custom_emoji.upload";
var CUSTOM_EMOJI_DELETE_ACTION = "custom_emoji.delete";
...
function attachSettings(ctx) {
  ...
  function handleRequest(req, res, fullUrl) {
    ...
    if (req.method === "POST" && fullUrl.startsWith("/api/custom-emoji/")) {
      ...
      req.on("end", function () {
        // actorId/actorName come from the auth gate above (muPost) —
        // resolved once per request so every audit entry below shares them.
        var postActor = { actorId: muPost.id, actorName: muPost.username };

        if (postSize > postMaxSize) {
          audit.log(CUSTOM_EMOJI_UPLOAD_ACTION, Object.assign({}, postActor, {
            target: postSlug,
            metadata: { status: 413, size: postSize, reason: "file too large" },
          }));
          res.writeHead(413, { "Content-Type": "application/json" });
        ...
        if (!ct) {
          audit.log(CUSTOM_EMOJI_UPLOAD_ACTION, Object.assign({}, postActor, {
            target: postSlug,
            metadata: { status: 400, size: postSize, reason: "unsupported image format" },
          }));
          res.writeHead(400, { "Content-Type": "application/json" });
          res.end('{"error":"Unsupported image format"}');
          return;
        ...
        if (!resolvedCandidate.startsWith(customEmojiDir + path.sep)) {
          audit.log(CUSTOM_EMOJI_UPLOAD_ACTION, Object.assign({}, postActor, {
            target: postSlug,
            metadata: { status: 400, size: postSize, reason: "invalid path" },
          }));
          res.writeHead(400, { "Content-Type": "application/json" });
        ...
        try {
          ...
          audit.log(CUSTOM_EMOJI_UPLOAD_ACTION, Object.assign({}, postActor, {
            target: postSlug,
            metadata: { status: 500, size: postSize, reason: "write failed: " + e.message },
          }));
          res.writeHead(500, { "Content-Type": "application/json" });
          res.end('{"error":"Failed to save image"}');
          return;
        }

        audit.log(CUSTOM_EMOJI_UPLOAD_ACTION, Object.assign({}, postActor, {
          target: postSlug,
          metadata: { status: 200, size: postSize },
        }));
        res.writeHead(200, { "Content-Type": "application/json" });
    ...
    if (req.method === "DELETE" && fullUrl.startsWith("/api/custom-emoji/")) {
      ...
      var delActor = { actorId: muDel.id, actorName: muDel.username };

      if (!SLUG_RE.test(delSlug)) {
        audit.log(CUSTOM_EMOJI_DELETE_ACTION, Object.assign({}, delActor, {
          target: delSlug,
      ...
      // deletedSize/deletedAny distinguish an actual removal from the
      // no-op case (slug valid but no matching file) — the exact class of
      // silent failure this audit entry exists to make diagnosable (lr-fc71).
      var deletedAny = false;
      var deletedSize = null;
      var delError = null;
      try {
        ...
        for (var dfi = 0; dfi < delFiles.length; dfi++) {
          ...
          if (path.basename(df, "." + dfExt) === delSlug) {
            ...
            if (safeDelPath) {
              try { deletedSize = fs.statSync(safeDelPath).size; } catch (e2) { /* stat failed — omit size */ }
              fs.unlinkSync(safeDelPath);
              deletedAny = true;
            }
          }
        }
      ...
      audit.log(CUSTOM_EMOJI_DELETE_ACTION, Object.assign({}, delActor, {
        ...
        metadata: {
          ...
          size: deletedSize,
          deleted: deletedAny,
          reason: delError ? ("directory read failed: " + delError) : (deletedAny ? null : "no matching file (no-op)"),
        },
      }));
