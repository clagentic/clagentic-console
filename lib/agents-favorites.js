...
// chattable.json lives under the Console data dir (CONFIG_DIR = ~/.clagentic/console/).
// One-time migration: if the old root-level file exists and the new one does not,
// copy it so favorites/recents are preserved on upgrade.
var FAVORITES_PATH = path.join(CONFIG_DIR, "agents", "chattable.json");
(function migrateFavorites() {
  var _realHome = process.env.SUDO_USER
    ? path.join("/home", process.env.SUDO_USER)
    : os.homedir();
  var oldPath = path.join(_realHome, ".clagentic", "agents", "chattable.json");
  try {
    if (fs.existsSync(oldPath) && !fs.existsSync(FAVORITES_PATH)) {
      fs.mkdirSync(path.dirname(FAVORITES_PATH), { recursive: true });
      fs.copyFileSync(oldPath, FAVORITES_PATH);
      console.log("[agents-favorites] Migrated chattable.json → " + FAVORITES_PATH + " — root copy can be removed when verified");
    }
  } catch (e) {
    // Non-fatal — favorites will start fresh
  }
}());
