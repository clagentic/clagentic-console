 ...
 * opts: { cwd, send, sendTo, isMultiUser, broadcastTermList }
 *
 * Ownership enforcement is always active. Each terminal records ownerUserId
 * from ws._clayUser.id at creation time. Operations that mutate or read
 * private output (attach, write, close, rename) are restricted to the owning
 * user or admins (ws._clayUser.role === "admin"). term_list results are
...
function createTerminalManager(opts) {
  ...
  // Authorized if: no owner set, same user, caller is admin, or system context (no user).
  function isAuthorized(session, callerWs) {
    ...
    if (!caller) return true; // system context, allow
