...
function createTerminalManager(opts) {
  ...
  // system context (callerWs === null): allowed. Unauthenticated ws (_clayUser null): denied.
  ...
  function list(callerWs) {
    ...
    for (var session of terminals.values()) {
      ...
      if (callerWs && callerWs._clayUser && callerWs._clayUser.role === "admin" && session.ownerUserId) {
